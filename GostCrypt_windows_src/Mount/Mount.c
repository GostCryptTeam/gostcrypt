 ...
 *
 *	\file Mount.c
 *	\brief Manage the user interface for the first page of the program (mounting / dismounting a drive...)
 *	\version 1.3.1
 *	\date 14/12/2016
 *
 */
...
/**
 *
 *	\fn static void localcleanup (void)
 *	\brief Often is called with atexit, clean the function before it dies (at the end of the function) : free variables.
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void RefreshMainDlg (HWND hwndDlg)
 *	\brief Refresh the main window, change the combo boxes, reload drive and refresh buttons values.
...
/**
 *
 *	\fn void EndMainDlg (HWND hwndDlg)
 *	\brief Clean and end the main window
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
void EndMainDlg(HWND hwndDlg)
{
	...
	/* hide main window, and show the handled window */
	if (TaskBarIconMutex != NULL)
	{
		MainWindowHidden = TRUE;
		ShowWindow(hwndDlg, SW_HIDE);
	}
	/* if the program is not busy by other window or anything, end the main dialog window */
	else
	{
...
/**
 *
 *	\fn static void InitMainDialog (HWND hwndDlg)
 *	\brief Initialize and fill the top main menu, and its logo (enable or disable menu options)
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void EnableDisableButtons (HWND hwndDlg)
 *	\brief Enables some menu options depending on selected drive / partition
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
void EnableDisableButtons(HWND hwndDlg)
{
	...
	/*
	Enables some menu options in "Volume" section
	Enables Mount Volume ( in Volumes menu bar )
	Enables Mount Volume options ( in Volumes menu bar )
	Enables Backup and Restore Header in Tools menu bar
	Enable change password on multiples menu bars
	*/
	...
	{
		SetWindowTextW(hOKButton, GetString("UNMOUNT_BUTTON")); // in the homepage allow if a volume is NONSYS to unmount it
		EnableWindow(hOKButton, TRUE);
	...
	case GST_MLIST_ITEM_SYS_DRIVE:
		...
		SetWindowTextW(hOKButton, GetString("MOUNT_BUTTON")); //if it is a system drive the button appear to be mount
		EnableWindow(GetDlgItem(hwndDlg, IDC_VOLUME_PROPERTIES), TRUE);
	...
	default:
		SetWindowTextW(hOKButton, GetString("MOUNT_BUTTON")); // by default it is Mount
...
/**
 *
 *	\fn BOOL VolumeSelected (HWND hwndDlg)
 *	\brief Returns TRUE if the last partition/drive selected via the Select Device dialog box was the system
 ...
 *	\param[in] HWND hwndDlg
 *	\return BOOL (GetWindowTextLength (GetDlgItem (hwndDlg, IDC_VOLUME)) > 0)
 *
 */
...
/**
 *
 *	\fn BOOL ActiveSysEncDeviceSelected (void)
 *	\brief Return True if : the partition or the drive is or has to be fully encrypted. Return True if it is a partition selected. All other cases are returned false
 ...
 *			supposedly non-system device) -- instead use IsSystemDevicePath(path, hwndDlg, TRUE) for such
 *			purposes. This function can be used only for preliminary GUI checks requiring very fast responses.
 *	\return BOOL TRUE (WholeSysDriveEncryption(FALSE) ? (retCode == 2 || retCode == 1) : (retCode == 1))
 *			FALSE otherwise
 *
 */
BOOL ActiveSysEncDeviceSelected(void)
{
	try
	{
		BootEncStatus = BootEncObj->GetStatus(); //BootEncryptionStatus
		...
		if (BootEncStatus.DriveEncrypted)
		{
			...
			retCode = IsSystemDevicePath(szFileName, MainDlg, FALSE); // retcode = 

			return (WholeSysDriveEncryption(FALSE) ? (retCode == 2 || retCode == 1) : (retCode == 1)); //return True if the whole system is encrypted or is to be encrypted or
		}
	}
	catch (Exception &e)
	{
...
/**
 *
 *	\fn void LoadSettings (HWND hwndDlg)
 *	\brief Set all the variables the program need to run, some are in a xml config file.
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
{
	EnableHwEncryption((ReadDriverConfigurationFlags() & GST_DRIVER_CONFIG_DISABLE_HARDWARE_ENCRYPTION) ? FALSE : TRUE); // inutile

	WipeAlgorithmId savedWipeAlgorithm = GST_WIPE_NONE; // 

	LoadSysEncSettings(hwndDlg); // load the encryption settings
...
/**
 *
 *	\fn void SaveSettings (HWND hwndDlg)
 *	\brief Save the settings in a file
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL SysEncryptionOrDecryptionRequired (void)
 *	\brief Returns TRUE if system encryption or decryption had been or is in progress and has not been completed
 *	\return static BOOL (SystemEncryptionStatus == SYSENC_STATUS_ENCRYPTING || SystemEncryptionStatus == SYSENC_STATUS_DECRYPTING || (BootEncStatus.DriveMounted && (BootEncStatus.ConfiguredEncryptedAreaStart != BootEncStatus.EncryptedAreaStart || BootEncStatus.ConfiguredEncryptedAreaEnd != BootEncStatus.EncryptedAreaEnd)))
 *
 */
...
/**
 *
 *	\fn static BOOL SysDriveOrPartitionFullyEncrypted (BOOL bSilent)
 *	\brief Returns TRUE if the system partition/drive is completely encrypted
 *	\param[in] BOOL bSilent
 *	\return static BOOL (!BootEncStatus.SetupInProgress && BootEncStatus.ConfiguredEncryptedAreaEnd != 0 && BootEncStatus.ConfiguredEncryptedAreaEnd != -1 && BootEncStatus.ConfiguredEncryptedAreaStart == BootEncStatus.EncryptedAreaStart && BootEncStatus.ConfiguredEncryptedAreaEnd == BootEncStatus.EncryptedAreaEnd)
 *
 */
...
/**
 *
 *	\fn static BOOL SysEncDeviceActive (BOOL bSilent)
 *	\brief Returns TRUE if the system partition/drive is being filtered by the GostCrypt driver and the key data
 ...
 *	\param[in] BOOL bSilent
 *	\return static BOOL FALSE catch (Exception &e)
 *			(BootEncStatus.DriveMounted) otherwise
 *
 */
...
/**
 *
 *	\fn BOOL WholeSysDriveEncryption (BOOL bSilent)
 *	\brief Returns TRUE if the entire system drive (as opposed to the system partition only) is (or is to be) encrypted
 *	\param[in] BOOL bSilent
 *	\return BOOL FALSE catch (Exception &e)
 *			(BootEncStatus.ConfiguredEncryptedAreaStart == GST_BOOT_LOADER_AREA_SIZE && BootEncStatus.ConfiguredEncryptedAreaEnd >= BootEncStatus.BootDriveLength.QuadPart - 1) otherwise
 *
 */
...
/**
 *
 *	\fn unsigned __int64 GetSysEncDeviceSize (BOOL bSilent)
 *	\brief Returns the size of the system drive/partition (if encrypted) in bytes
 *	\param[in] BOOL bSilent
...
/**
 *
 *	\fn unsigned __int64 GetSysEncDeviceEncryptedPartSize (BOOL bSilent)
 *	\brief Returns the current size of the encrypted area of the system drive/partition in bytes
 *	\param[in] BOOL bSilent
...
/**
 *
 *	\fn static void PopulateSysEncContextMenu (HMENU popup, BOOL bToolsOnly)
 *	\brief Add in the drop down menu options
 *	\param[in] BOOL bToolsOnly
 *	\param[in] HMENU popup
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL CheckSysEncMountWithoutPBA (const char *devicePath, BOOL quiet)
 *	\brief WARNING: This function may take a long time to complete. To prevent data corruption, it MUST be called before
 ...
 *	\param[in] BOOL quiet
 *	\param[in] const char
 *	\return BOOL FALSE if (!tmpbDevice)
 *			FALSE if (LOWORD (GetSelectedLong (GetDlgItem (MainDlg, IDC_DRIVELIST))) != GST_MLIST_ITEM_FREE)
 *			FALSE if (!partionPortion || !_stricmp (partionPortion, "\\Partition0"))
 *			FALSE if (sscanf (szDevicePath, "\\Device\\Harddisk%d\\Partition", &driveNo) != 1)
 *			TRUE if (retCode != 2)
 *			FALSE else (retCode != 2)
 *			TRUE else (BootEncStatus.DriveMounted)
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn BOOL GSTBootLoaderOnInactiveSysEncDrive (void)
 *	\brief Returns TRUE if the host drive of the specified partition contains a portion of the GostCrypt Boot Loader
 ...
 *	\return BOOL FALSE if (bPrebootPasswordDlgMode)
 *			FALSE if (sscanf (szDevicePath, "\\Device\\Harddisk%d\\Partition", &driveNo) != 1)
 *			FALSE if (BootEncStatus.DriveMounted && IsSystemDevicePath (parentDrivePath, MainDlg, FALSE) == 2)
 *			FALSE catch (...)
 *			((BOOL) BootEncObj->IsBootLoaderOnDrive (parentDrivePath)) otherwise
 *
 */
...
/**
 *
 *	\fn BOOL SelectItem (HWND hTree, char nLetter)
 *	\brief Function to select a drive Letter
 *	\param[in] char nLetter
 *	\param[in] HWND hTree
 *	\return BOOL FALSE if (ListView_GetItem (hTree, &item) == FALSE)
 *			TRUE if (HIWORD (item.lParam) == nLetter)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn static void LaunchVolCreationWizard (HWND hwndDlg, const char *arg)
 *	\brief Launch the GostCrypt Format.exe that permits to create a new volume (linked to the button create new volume in the interface).
 *	\param[in] const char
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
...
/**
*
*	\fn void LoadDriveLetters (HWND hTree, int drive)
*	\brief Load all drive letters and tell their status
*	\param[in] int drive
*	\param[in] HWND hTree
*	\return void
*
...
/**
 *
 *	\fn static void PasswordChangeEnable (HWND hwndDlg, int button, int passwordId, BOOL keyFilesEnabled,int newPasswordId, int newVerifyId, BOOL newKeyFilesEnabled)
 *	\brief Verify the password and change it.
 *	\param[in] HWND hwndDlg
 *	\param[in] int button
 *	\param[in] int passwordId
 *	\param[in] BOOL keyFilesEnabled
 *	\param[in] int newPasswordId
 *	\param[in] int newVerifyId
 *	\param[in] BOOL newKeyFilesEnabled
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK PasswordChangeDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 ...
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 if (SetTimer (hwndDlg, TIMER_ID_KEYB_LAYOUT_GUARD, TIMER_INTERVAL_KEYB_LAYOUT_GUARD, NULL) == 0)
 *			0 switch (msg) case WM_INITDIALOG
 *			0 switch (msg) case WM_TIMER
 *			1 switch (wParam) case TIMER_ID_KEYB_LAYOUT_GUARD
...
/**
 *
 *	\fn BOOL CALLBACK PasswordDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Except in response to the WM_INITDIALOG message, the dialog box procedure
 *			should return nonzero if it processes the message, and zero if it does
 *			not. - see DialogProc
 *	\param[in] LPARAM lParam
 ...
 *			1 switch (msg) case GST_APPMSG_PREBOOT_PASSWORD_MODE
 *			0 case WM_TIMER
 *			1 switch (wParam) case TIMER_ID_KEYB_LAYOUT_GUARD
 *			0 case WM_COMMAND
 *			1 if case WM_COMMAND and enter in a if
 *			1 case WM_DROPFILES
 *			0 otherwise
 *
 */
...
/**
 *
 *	\fn static void PreferencesDlgEnableButtons (HWND hwndDlg)
 *	\brief Enable some buttons depending on some parameters
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK PreferencesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Send to the window handled the values to display and manage when a user click on some menu object
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (msg) case WM_INITDIALOG
 *			1 case WM_INITDIALOG : if (PreferencesDialogActive)
 *			1 if (lw == IDCANCEL)
 *			1 if (lw == IDOK)
 *			1 if (lw == IDC_MORE_SETTINGS)
...
/**
 *
 *	\fn BOOL CALLBACK MountOptionsDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Handle mount options dialogue
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (msg) case WM_INITDIALOG
 *			0 if (lw == IDC_KEYFILES_ENABLE_HIDVOL_PROT)
 *			1 if (lw == IDC_SHOW_PASSWORD_MO)
 *			1 if (lw == IDOK)
 *			1 if (lw == IDC_MOUNT_READONLY || lw == IDC_PROTECT_HIDDEN_VOL)
 *			0 otherwise
 *
 */
...
/**
 *
 *	\fn int GetCipherBlockSizeByDriveNo (int nDosDriveNo)
 *	\brief Returns the block size (in bits) of the cipher with which the volume mounted as the
...
/**
 *
 *	\fn int GetModeOfOperationByDriveNo (int nDosDriveNo)
 *	\brief Returns the mode of operation in which the volume mounted as the specified drive letter is encrypted.
 *	\param[in] int nDosDriveNo
 *	\return int prop.mode if (DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop, sizeof (prop), &prop, sizeof (prop), &dwResult, NULL))
 *			0 otherwise
 */
...
/**
 *
 *	\fn BOOL CALLBACK VolumePropertiesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief handle properies dialogue
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 catch (Exception &e)
 *			0 switch (msg) case WM_INITDIALOG
 *			0 switch (msg) case WM_COMMAND
 *			1 if (lw == IDOK)
...
/**
 *
 *	\fn BOOL CALLBACK TravelerDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Handle dialogues on the window handled. manage buttons like create that will copy the wizard
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
...
/**
 *
 *	\fn void BuildTree (HWND hTree)
 *	\brief  Destroy listview column, inialise a LvCol function, set some icons, clean objects and return.
 *	\param[in] HWND hTree
 *	\return void
 *
 */
...
/**
 *
 *	\fn LPARAM GetSelectedLong (HWND hTree)
 *	\brief Get the selected "Tree" that is a listview
 *	\param[in] HWND hTree
 *	\return LPARAM MAKELONG (0xffff, 0xffff) if (ListView_GetItem (hTree, &item) == FALSE)
 *			item.lParam else (ListView_GetItem (hTree, &item) == FALSE)
 *
 */
...
/**
 *
 *	\fn LPARAM GetItemLong (HWND hTree, int itemNo)
 *	\brief Create a LVitem ,initialise the number value and the mask. If it fails
 *	\param[in] int itemNo
 *	\param[in] HWND hTree
 *	\return LPARAM MAKELONG (0xffff, 0xffff) if (ListView_GetItem (hTree, &item) == FALSE)
 *			item.lParam else (ListView_GetItem (hTree, &item) == FALSE)
...
/**
 *
 *	\fn static int AskVolumePassword (HWND hwndDlg, Password *password, char *titleStringId, BOOL enableMountOptions)
 *	\brief Take as an argument the password and check with the Volume if it is matching
 *	\param[in] BOOL enableMountOptions
 *	\param[in] char * titleStringId
 *	\param[in] Password * password
 *	\param[in] HWND hwndDlg
 *	\return static int (result == IDOK)
 *
 */
...
/**
 *
 *	\fn static BOOL Mount (HWND hwndDlg, int nDosDriveNo, char *szFileName)
 *	\brief GUI actions
 *	\param[in] char * szFileName
 *	\param[in] int nDosDriveNo
 *	\param[in] HWND hwndDlg
 *	\return static BOOL status
 *
 */
...
/**
 *
 *	\fn static BOOL Dismount (HWND hwndDlg, int nDosDriveNo)
 *	\brief Function called when the user want to unmount a drive. Unmount a drive
 *	\param[in] int nDosDriveNo
 *	\param[in] HWND hwndDlg
 *	\return static BOOL TRUE if (UnmountVolume (hwndDlg, nDosDriveNo, bForceUnmount))
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL DismountAll (HWND hwndDlg, BOOL forceUnmount, BOOL interact, int dismountMaxRetries, int dismountAutoRetryDelay)
 *	\brief Unmount all drives.
 *	\param[in] int dismountAutoRetryDelay
 *	\param[in] int dismountMaxRetries
 *	\param[in] BOOL interact
 *	\param[in] BOOL forceUnmount
 *	\param[in] HWND hwndDlg
 *	\return static BOOL TRUE if (mountList.ulMountedDrives == 0)
 *			FALSE if (bResult == FALSE)
 *			FALSE if (unmount.nReturnCode == ERR_FILES_OPEN)
 *			status otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL MountAllDevices (HWND hwndDlg, BOOL bPasswordPrompt)
 *	\brief Mount all drives with default mount options
 *	\param[in] BOOL bPasswordPrompt
 *	\param[in] HWND hwndDlg
 *	\return static BOOL status
 *
 */
...
/**
 *
 *	\fn static void ChangePassword (HWND hwndDlg)
 *	\brief Function called when the user want to change the password, change the password by calling passwordchngeDLGPROC
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ChangeSysEncPassword (HWND hwndDlg, BOOL bOnlyChangeKDF)
 *	\brief Change password of the system partition/drive
 *	\param[in] BOOL bOnlyChangeKDF
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void EncryptSystemDevice (void)
 *	\brief Initiates or resumes encryption of the system partition/drive
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void DecryptSystemDevice (void)
 *	\brief Initiates decryption of the system partition/drive
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void CreateHiddenOS (void)
 *	\brief Initiates the process of creation of a hidden operating system
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ResumeInterruptedSysEncProcess (void)
 *	\brief Blindly attempts (without any checks) to instruct the wizard to resume whatever system encryption process
...
/**
 *
 *	\fn void CreateRescueDisk (void)
 *	\brief When an error happened try to recover the disk by checking what happened and acting, verify that the encryption is not running yet.
 *			Launch the rescue wizard.
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void VerifyRescueDisk (void)
 *	\brief Create a temporary up-to-date rescue disk image in RAM (with it the CD/DVD content will be compared)
 *		   notice if the rescue failed or succeed
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ShowSystemEncryptionStatus (void)
 *	\brief Show information relatively to the encryption status.
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ResumeInterruptedNonSysInplaceEncProcess (void)
 *	\brief Launch the creation wizard to create a new volume in a special folder : /zinplace
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static BOOL SelectContainer (HWND hwndDlg)
 *	\brief Select a container and change disable / enable buttons.
 *	\param[in] HWND hwndDlg
 *	\return static BOOL FALSE if (BrowseFiles (hwndDlg, "OPEN_VOL_TITLE", szFileName, bHistory, FALSE, NULL) == FALSE)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL SelectPartition (HWND hwndDlg)
 *	\brief When selecting a partition change the focus and enables/disable buttons.
 *	\param[in] HWND hwndDlg
 *	\return static BOOL TRUE if (nResult == IDOK)
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn static void WipeCache (HWND hwndDlg, BOOL silent)
...
/**
 *
 *	\fn static void Benchmark (HWND hwndDlg)
 *	\brief Set the dialog box parameters to benchmarks
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static BOOL CheckMountList ()
 *	\brief Check the list of drives, display some information. the function is run in hidden mode to protect from modification.
 *	\return static BOOL FALSE if (selDrive != -1 && (current.ulMountedDrives & (1 << (selDrive - 'A'))) == 0 && !IsDriveAvailable (selDrive - 'A'))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK MainDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Main dialog proc is the main function that manage first page window display and connections with GUI and execution 
 ...
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (uMsg) case WM_HOTKEY
 *			0 switch (uMsg) case WM_INITDIALOG
 *			HandleDriveListMouseWheelEvent (uMsg, wParam, lParam, FALSE) switch (uMsg) case WM_MOUSEWHEEL
 *			0 switch (uMsg) case WM_WINDOWPOSCHANGING if (MainWindowHidden)
 *			1 switch (uMsg) case WM_WINDOWPOSCHANGING
 *			1 switch (uMsg) case WM_SYSCOMMAND if (lw == IDC_ABOUT)
 *			0 switch (uMsg) case WM_SYSCOMMAND
 *			1 switch (uMsg) case WM_HELPs
 *			0 switch (uMsg) case WM_ENDSESSION
 *			1 switch (uMsg) case WM_TIMER
 *			1 switch (lParam) case WM_LBUTTONDOWN
 *			1 switch (lParam) case WM_RBUTTONUP
 *			1 switch (lParam) case WM_TIMER
 *			0 switch (uMsg) case GST_APPMSG_TASKBAR_ICON if switch (lParam) dont return
 *			1 switch (uMsg) case GST_APPMSG_CLOSE_BKG_TASK
 *			1 switch (uMsg) case GST_APPMSG_SYSENC_CONFIG_UPDATE
 *			1 switch (uMsg) case WM_DEVICECHANGE if (!IgnoreWmDeviceChange && wParam != DBT_DEVICEARRIVAL)
 *			0 switch (uMsg) case WM_DEVICECHANGE else
 *			0 switch (uMsg) case WM_NOTIFY if (((LPNMHDR)lParam)->code == NM_CUSTOMDRAW)
 *			1 switch (uMsg) case WM_NOTIFY if (((LPNMHDR) lParam)->code == LVN_ITEMCHANGED && (((LPNMLISTVIEW) lParam)->uNewState & LVIS_FOCUSED ))
 *			1 switch (uMsg) case WM_NOTIFY if (((LPNMHDR)lParam)->code == LVN_ITEMACTIVATE)
 *			1 switch (menuItem)
 *			0 switch (uMsg) case WM_NOTIFY else
 *			0 switch (uMsg) case WM_ERASEBKGND
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_UNMOUNT_VOLUME && LOWORD(GetSelectedLong(GetDlgItem(hwndDlg, IDC_DRIVELIST))) != GST_MLIST_ITEM_NONSYS_VOL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ABOUT || lw == IDC_LOGO)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ABOUT || lw == IDC_LOGO)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDHELP || lw == IDM_HELP)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDCANCEL || lw == IDC_EXIT)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_SELECT_DEVICE || lw == IDM_SELECT_DEVICE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_SELECT_FILE || lw == IDM_SELECT_FILE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_MOUNTALL || lw == IDM_MOUNTALL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_UNMOUNTALL || lw == IDM_UNMOUNTALL)
 *			1 switch (uMsg) case WM_COMMAND if ((lw == IDOK || lw == IDM_MOUNT_VOLUME || lw == IDM_MOUNT_VOLUME_OPTIONS))
 *			1 switch (uMsg) case WM_COMMAND if ((lw == IDOK || lw == IDM_MOUNT_VOLUME || lw == IDM_MOUNT_VOLUME_OPTIONS || lw == IDC_MOUNTALL || lw == IDM_MOUNTALL) && LOWORD(GetSelectedLong(GetDlgItem(hwndDlg, IDC_DRIVELIST))) == 0xffff)
 *			1 switch (uMsg) case WM_COMMAND if (CheckMountList())
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_VOLUME_TOOLS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_CHANGE_PASSWORD)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_CHANGE_HEADER_KEY_DERIV_ALGO)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_WIPE_CACHE || lw == IDM_WIPE_CACHE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_CLEAR_HISTORY)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_CREATE_VOLUME || lw == IDM_CREATE_VOLUME || lw == IDM_VOLUME_WIZARD)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ADD_REMOVE_VOL_KEYFILES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_MANAGE_TOKEN_KEYFILES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_REMOVE_ALL_KEYFILES_FROM_VOL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_CLOSE_ALL_TOKEN_SESSIONS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_KEYFILE_GENERATOR)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_LICENSE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_WEBSITE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_HOMEPAGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ONLINE_TUTORIAL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ONLINE_HELP)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_FAQ)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_GST_DOWNLOADS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_NEWS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_VERSION_HISTORY)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ANALYZE_SYSTEM_CRASH)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_CONTACT)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_PREFERENCES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_HOTKEY_SETTINGS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_PERFORMANCE_SETTINGS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_DEFAULT_KEYFILES)
 *			1 switch (uMsg) case WM_COMMAND if (LOWORD(selectedDrive) != GST_MLIST_ITEM_NONSYS_VOL && !(VolumeSelected(hwndDlg) && IsMountedVolume(volPathLower)))
 *			1 switch (uMsg) case WM_COMMAND if (!DeviceIoControl (hDriver, GST_IOCTL_GET_VOLUME_PROPERTIES, &prop, sizeof (prop), &prop, sizeof (prop), &dwResult, NULL) || dwResult == 0)
 *			1 switch (AskMultiChoice ((void **) tmp, FALSE)) default
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ADD_VOLUME_TO_FAVORITES || lw == IDM_ADD_VOLUME_TO_SYSTEM_FAVORITES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_ORGANIZE_FAVORITES || lw == IDM_ORGANIZE_SYSTEM_FAVORITES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_TOKEN_PREFERENCES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_SYSENC_SETTINGS || lw == IDM_SYS_ENC_SETTINGS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_SYS_FAVORITES_SETTINGS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_BENCHMARK)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_TRAVELER)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_BACKUP_VOL_HEADER)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_RESTORE_VOL_HEADER)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_LANGUAGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_TEST_VECTORS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_REFRESH_DRIVE_LETTERS)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_MOUNT_FAVORITE_VOLUMES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_RESUME_INTERRUPTED_PROC)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_VOLUME_PROPERTIES || lw == IDM_VOLUME_PROPERTIES)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_VOLUME && hw == CBN_EDITCHANGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_VOLUME && hw == CBN_SELCHANGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_NO_HISTORY)
 *			1 switch (uMsg) case WM_COMMAND if (lw >= GST_FAVORITE_MENU_CMD_ID_OFFSET && lw < GST_FAVORITE_MENU_CMD_ID_OFFSET_END)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_LANGUAGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDM_LANGUAGE)
 *			0 switch (uMsg) case WM_COMMAND else
 *			1 switch (uMsg) case WM_DROPFILES
 *			1 switch (uMsg) case GST_APPMSG_MOUNT_ENABLE_DISABLE_CONTROLS
 *			1 switch (uMsg) case GST_APPMSG_MOUNT_SHOW_WINDOW
 *			1 switch (uMsg) case WM_COPYDATA
 *			1 switch (uMsg) case WM_CLOSE
 *			1 switch (uMsg) default
 *			0 else
 *
 */
	...
	case WM_INITDIALOG:
		{
			...
			MainDlg = hwndDlg; //window handler
			...
			ResetWrongPwdRetryCount ();
			//reset the count of pwd retry chances
			...
			{
				BootEncStatus = BootEncObj->GetStatus();
				//check if the driver have to be decrypted before booting
			...
			if (bAuto || (Quit && szFileName[0] != 0))
			{
				...
				if (bAutoMountFavorites) //favorites drivers are auto mounted, asking the password
				{
					defaultMountOptions = mountOptions;
					if (FirstCmdKeyFile)
					{
...
/**
 *
 *	\fn void ExtractCommandLine (HWND hwndDlg, char *lpszCommandLine)
 *	\brief When launching the program in command line this function manage the option you add to the launch example /a.
...
/**
 *
 *	\fn static void SystemFavoritesServiceLogError (const string &errorMessage)
 *	\brief Register events for the favorites services and report errors to the user.
 *	\param[in] const string & errorMessage
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void SystemFavoritesServiceSetStatus (DWORD status, DWORD waitHint = 0)
 *	\brief Set variables for the favorites services
 *	\param[in] DWORD waitHint
 *	\param[in] DWORD status
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static VOID WINAPI SystemFavoritesServiceCtrlHandler (DWORD control)
 *	\brief Send a control code to a service and if it is stop, set the service status.
 *	\param[in] DWORD control
...
/**
 *
 *	\fn static VOID WINAPI SystemFavoritesServiceMain (DWORD argc, LPTSTR *argv)
 *	\brief Main called for the favorites services, auto mount favorites. Set some variables for the service status.
 *	\param[in] LPTSTR * argv
 *	\param[in] DWORD argc
 *	\return static VOID WINAPI
 *
 */
...
/**
 *
 *	\fn static BOOL StartSystemFavoritesService ()
 *	\brief Start the favorites service system.
 *	\return static BOOL FALSE if (DriverAttach() != ERR_SUCCESS)
 *			result else
 *
 */
...
/**
 *
 *	\fn int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpszCommandLine, int nCmdShow)
 *	\brief Lock some gloal variable, allocate memory, store the application name, initialize the window, and try to open the device driver
 *	\param[in] int nCmdShow
 *	\param[in] char * lpszCommandLine
 *	\param[in] HINSTANCE hPrevInstance
 *	\param[in] HINSTANCE hInstance
 *	\return int WINAPI StartSystemFavoritesService() ? 0 : 1 if (argv && argc == 2 && SingleStringToWide (GST_SYSTEM_FAVORITES_SERVICE_CMDLINE_OPTION) == argv[1])
 *			0 else
 *
 */
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpszCommandLine, int nCmdShow)
{
	...
	//set the priority shutdown for this program : here set to be the last application to be shutdown
...
/**
 *
 *	\fn BOOL TaskBarIconAdd (HWND hwnd)
 *	\brief add the program in the taskbar icon.
 *	\param[in] HWND hwnd
 *	\return BOOL FALSE if (TaskBarIconMutex == NULL || GetLastError () == ERROR_ALREADY_EXISTS)
 *			Shell_NotifyIconW (NIM_ADD, &tnid) else
 *
 */
...
/**
 *
 *	\fn BOOL TaskBarIconRemove (HWND hwnd)
 *	\brief Remove the program from the taskbar icon
 *	\param[in] HWND hwnd
 *	\return BOOL res if (TaskBarIconMutex != NULL)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn BOOL TaskBarIconChange (HWND hwnd, int iconId) 
 *	\brief Change the program from the taskbar icon
 *	\param[in] HWND hwnd
 *	\param[in] int iconId
 *	\return BOOL FALSE if (TaskBarIconMutex == NULL)
 *			Shell_NotifyIcon (NIM_MODIFY, &tnid) else
 *
 */
...
/**
 *
 *	\fn void DismountIdleVolumes ()
 *	\brief Dismount all volumes.
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL MountFavoriteVolumes (BOOL systemFavorites, BOOL logOnMount, BOOL hotKeyMount, const FavoriteVolume &favoriteVolumeToMount)
 *	\brief Load and mount favorites volumes
 *	\param[in] const FavoriteVolume
 *	\param[in] BOOL hotKeyMount
 *	\param[in] BOOL logOnMount
 *	\param[in] BOOL systemFavorites
...
/**
 *
 *	\fn static void SaveDefaultKeyFilesParam (void)
 *	\brief The user chose a file to save the Key file, this function if a file is chosen will write into it. (save)
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void KeyfileDefaultsDlg (HWND hwndDlg)
 *	\brief Dialog window for the keyfile.
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void HandleHotKey (HWND hwndDlg, WPARAM wParam)
 *	\brief Launch function associated with hotkeys.
 *	\param[in] WPARAM wParam
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
...
/**
 *
 *	\fn void ChangeMainWindowVisibility ()
 *	\brief Change if the window is visible or not.
 *	\return void
 *
 */
...
/**
 *
 *	\fn int BackupVolumeHeader (HWND hwndDlg, BOOL bRequireConfirmation, char *lpszVolume)
 *	\brief Keep a backup of the volume header to recover it when required. The header contains essential informations to find the data.
 *	\param[in] char * lpszVolume
 *	\param[in] BOOL bRequireConfirmation
 *	\param[in] HWND hwndDlg
 *	\return int 0 switch (IsSystemDevicePath (lpszVolume, hwndDlg, TRUE)) case 1
 *			0 switch (IsSystemDevicePath (lpszVolume, hwndDlg, TRUE)) case 2
 *			0 switch (IsSystemDevicePath (lpszVolume, hwndDlg, TRUE)) case 2
 *			nStatus else
 *
 */
...
/**
 *
 *	\fn int RestoreVolumeHeader (HWND hwndDlg, char *lpszVolume)
 *	\brief Restore the header of the volume to find the data inside it.
 *	\param[in] char * lpszVolume
 *	\param[in] HWND hwndDlg
 *	\return int 0 switch (IsSystemDevicePath (lpszVolume, hwndDlg, TRUE)) case 1
 *			0 switch (IsSystemDevicePath (lpszVolume, hwndDlg, TRUE)) case 2
 *			0 if (IsMountedVolume(lpszVolume))
 *			0 if (!VolumePathExists (lpszVolume))
 *			0 switch (AskMultiChoice ((void **) volTypeChoices, FALSE)) default
 *			nStatus else
 *
 */
...
/**
 *
 *	\fn void SetDriverConfigurationFlag (uint32 flag, BOOL state)
 *	\brief Set driver state and flag to the parameters sent
 *	\param[in] BOOL state
...
/**
 *
 *	\fn static BOOL CALLBACK PerformanceSettingsDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage the perfomance Options window. This contains the benchmark
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return static BOOL 0 switch (msg) case WM_INITDIALOG
 *			0 switch (msg) case WM_INITDIALOG
 *			1 switch (lw) case IDCANCEL
 *			1 switch (lw) case IDOK
 *			1 switch (lw) case IDC_LIMIT_ENC_THREAD_POOL
 *			1 switch (lw) case IDC_BENCHMARK
 *			1 switch (lw) case IDC_MORE_INFO_ON_THREAD_BASED_PARALLELIZATION
 *			0 switch (msg) case WM_COMMAND if switch (lw) didn't return
 *			0 else
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK SecurityTokenPreferencesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage the Security token window in settings > security proc. This window permits to use smart card or token.
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 ...
 *			1 switch (lw) case IDCANCEL
 *			1 switch (lw) case IDOK
 *			1 switch (lw) case IDC_AUTO_DETECT_PKCS11_MODULE
 *			1 switch (lw) case IDC_SELECT_PKCS11_MODULE
 *			0 switch (msg) case WM_COMMAND if switch (lw) didn't return
 *			0 else
 *
 */
...
/**
 *
 *	\fn void SecurityTokenPreferencesDialog (HWND hwndDlg)
 *	\brief Set the focus of the dialog box to the security dialog box.
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK BootLoaderPreferencesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Window opened with permanently decrypt selected partition/drive. Permits to decrypt drives before boot. 
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return static BOOL 1 switch (msg) case WM_INITDIALOG if (!BootEncObj->GetStatus().DriveMounted)
 *			1 switch (msg) case WM_INITDIALOG catch (Exception &e)
 *			0 switch (msg) case WM_INITDIALOG else
 *			1 switch (lw) case IDCANCEL
 *			1 switch (lw) case IDOK
 *			0 switch (lw) case WM_COMMAND if switch (lw) didn't return
 *			0 else
 *			
 *
 */
...
/**
 *
 *	\fn void MountSelectedVolume (HWND hwndDlg, BOOL mountWithOptions)
 *	\brief Take the selected object by the user and mount it.
 *	\param[in] BOOL mountWithOptions
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL GetExecutableImageInformation (const string &path, string &version, string &description, string &companyName, string &productName)
 *	\brief Get information about the program like the version and update the parameter by pointer
 *	\param[in] string & productName
 *	\param[in] string & companyName
 *	\param[in] string & description
 *	\param[in] string & version
 *	\param[in] const string
 *	\return BOOL FALSE if (size == 0)
 *			FALSE if (!GetFileVersionInfo (path.c_str(), handle, size, buf))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn void AnalyzeKernelMiniDump (HWND hwndDlg)
 *	\brief Analyse the memory dump, send error to user when a problem is encountered
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL HandleDriveListMouseWheelEvent (UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL bListMustBePointed)
 *	\brief Manage the mousewheel event
 *	\param[in] BOOL bListMustBePointed
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\return static BOOL 0 if (eventHandlerActive)
 *			0 if (bListMustBePointed && bListPointed || !bListMustBePointed)
 *			1 else
 *
 */
...
/**
 *
 *	\fn static LRESULT CALLBACK MouseWheelProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
 *	\brief When a mousewheel happens scroll ONLY one window function
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT message
 *	\param[in] HWND hwnd
 *	\return static LRESULT 0 switch (message) case WM_MOUSEWHEEL
...
/**
 *
 *	\fn void HookMouseWheel (HWND hwndDlg, UINT ctrlId)
 *	\brief Set windows with the mousewheel object.
