...
*
*	\file InPlace.c
*	\brief Actions on cluster, header & filesystem
*	\version 1.3.1
*	\date 14/12/2016
*
*/
...
/**
 *
 *	\fn static __int64 NewFileSysSizeAfterShrink (HANDLE dev, const char *devicePath, int64 *totalClusterCount, DWORD *bytesPerCluster, BOOL silent)
 *	\brief If the returned value is greater than 0, it is the desired volume size in NTFS sectors (not in bytes) 
 *			after shrinking has been performed. If there's any error, returns -1.
 *	\param[in] BOOL silent
 *	\param[in] DWORD * bytesPerCluster
 *	\param[in] int64 * totalClusterCount
 *	\param[in] const char * devicePath
 *	\param[in] HANDLE dev
 *	\return static __int64 desiredNbrSectors
 *			-1 if (desiredNbrSectors <= 0)
 *			-1 	if (!DeviceIoControl (dev, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0, (LPVOID) &ntfsVolData, sizeof (ntfsVolData), &nBytesReturned, NULL))
 *
 */
...
/**
 *
 *	\fn BOOL CheckRequirementsForNonSysInPlaceEnc (const char *devicePath, BOOL silent)
 *	\brief Access the partition
 *			Checks if the filesystem is formated in NTFS
 *			Checks if the filesystem can be safely shrunk
 *			Determines the filesystem size and checks if its long enough
 *			Checks free space on filesystem
 *			Checks if sector size is supported
 *	\param[in] BOOL silent
 *	\param[in] const char * devicePath
 *	\return BOOL FALSE if (CurrentOSMajor < 6)
 *			FALSE if (partitionNumber == 0)
 *			FALSE if (dev == INVALID_HANDLE_VALUE)
 *			FALSE if (FakeDosNameForDevice (devicePath, dosDev, devName, FALSE) != 0)
 *			FALSE if (!DeviceIoControl (dev, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0, (LPVOID) &ntfsVolData, sizeof (ntfsVolData), &nBytesReturned, NULL))
 *			FALSE if (NewFileSysSizeAfterShrink (dev, devicePath, NULL, NULL, silent) == -1)
 *			FALSE if (ntfsVolData.BytesPerSector > GST_MAX_VOLUME_SECTOR_SIZE || ntfsVolData.BytesPerSector % ENCRYPTION_DATA_UNIT_SIZE != 0)
 *			FALSE if (ntfsVolData.FreeClusters.QuadPart * ntfsVolData.BytesPerCluster < GST_TOTAL_VOLUME_HEADERS_SIZE)
 *			FALSE if (!DeviceIoControl(dev, FSCTL_GET_NTFS_VOLUME_DATA, NULL, 0, (LPVOID)&ntfsVolData, sizeof(ntfsVolData), &nBytesReturned, NULL))
 *			FALSE if (deviceSize < GST_NONSYS_INPLACE_ENC_MIN_VOL_SIZE)
 *			FALSE if (deviceSize < 0)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn int EncryptPartitionInPlaceBegin (volatile FORMAT_VOL_PARAMETERS *volParams, volatile HANDLE *outHandle, WipeAlgorithmId wipeAlgorithm)
 *	\brief Gain raw access, shrink the filesystem
 *			Create header backup on the partition.
 *			Decrypt the backup header to verify it has been correctly written
 *			Conceal the NTFS filesystem
 *			If a drive letter is assigned to the device, remove it
 *			Add the wizard to the system startup sequence if appropriate
 *	\param[in] volatile WipeAlgorithmId wipeAlgorithm
 *	\param[in] volatile HANDLE * outHandle
 *	\param[in] volatile FORMAT_VOL_PARAMETERS wipeAlgorithm
 *	\return int ERR_DONT_REPORT
 *			ERR_OUTOFMEMORY
...
/**
 *
 *	\fn int EncryptPartitionInPlaceResume (HANDLE dev, volatile FORMAT_VOL_PARAMETERS *volParams, WipeAlgorithmId wipeAlgorithm, volatile BOOL *bTryToCorrectReadErrors)
 *	\brief Resume the encryption of the partition.
 *	\param[in] volatile BOOL * bTryToCorrectReadErrors
 *	\param[in] WipeAlgorithmId wipeAlgorithm
 *	\param[in] volatile FORMAT_VOL_PARAMETERS * volParams
 *	\param[in] HANDLE dev
 *	\return int nStatus
 *
 */
...
/**
 *
 *	\fn int FastVolumeHeaderUpdate (HANDLE dev, CRYPTO_INFO *headerCryptoInfo, CRYPTO_INFO *masterCryptoInfo, __int64 deviceSize)
 *	\brief Update the volume header
 *	\param[in] __int64 deviceSize
 *	\param[in] CRYPTO_INFO * masterCryptoInfo
 *	\param[in] CRYPTO_INFO * headerCryptoInfo
 *	\param[in] HANDLE dev
 *	\return int nStatus
 *
 */
...
/**
 *
 *	\fn static HANDLE OpenPartitionVolume (const char *devName, BOOL bExclusiveRequired, BOOL bSharedRequired, BOOL bSharedRequiresConfirmation, BOOL bShowAlternativeSteps, BOOL bSilent)
 *	\brief Get exclusive access
 *			Open the volume partition
 *	\param[in] BOOL bSilent
 *	\param[in] BOOL bShowAlternativeSteps
 *	\param[in] BOOL bSharedRequiresConfirmation
 *	\param[in] BOOL bSharedRequired
 *	\param[in] BOOL bExclusiveRequired
 *	\param[in] const char * devName
 *	\return static HANDLE INVALID_HANDLE_VALUE
 *			INVALID_HANDLE_VALUE if (bExclusiveRequired)
 *			INVALID_HANDLE_VALUE if (bSharedRequiresConfirmation && !bSilent && AskWarnNoYes ("DEVICE_IN_USE_INPLACE_ENC") == IDNO)
 *			INVALID_HANDLE_VALUE else (dev != INVALID_HANDLE_VALUE)
 *			dev otherwise
 *
 */
...
/**
 *
 *	\fn static int DismountFileSystem (HANDLE dev, int driveLetter, BOOL bForcedAllowed, BOOL bForcedRequiresConfirmation, BOOL bSilent)
 *	\brief Dismout the volume
 *	\param[in] BOOL bSilent
 *	\param[in] BOOL bForcedRequiresConfirmation
 *	\param[in] BOOL bForcedAllowed
 *	\param[in] int driveLetter
 *	\return static int ERR_DONT_REPORT if (!bForcedAllowed)
 *			ERR_DONT_REPORT if (bForcedRequiresConfirmation && !bSilent && AskWarnYesNo ("VOL_LOCK_FAILED_OFFER_FORCED_DISMOUNT") == IDNO)
 *			ERR_DONT_REPORT if (!bResult)
...
/**
 *
 *	\fn static int ConcealNTFS (HANDLE dev)
 *	\brief Easy-to-undo modification applied to conceal the NTFS filesystem (to prevent Windows and apps from 
 ...
 *	\param[in] HANDLE dev
 *	\return static int ERR_OS_ERROR if (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0)
 *			ERR_OS_ERROR if (ReadFile (dev, buf, GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE, &nbrBytesProcessed, NULL) == 0)
 *			ERR_OS_ERROR if (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0)
 *			ERR_OS_ERROR if (WriteFile (dev, buf, GST_INITIAL_NTFS_CONCEAL_PORTION_SIZE, &nbrBytesProcessed, NULL) == 0)
 *			ERR_SUCCESS otherwise
 *
 */
...
/**
 *
 *	\fn void ShowInPlaceEncErrMsgWAltSteps (char *iniStrId, BOOL bErr)
 *	\brief Show error or warining
 *	\param[in] BOOL bErr
 *	\param[in] char * iniStrId
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void ExportProgressStats (__int64 bytesDone, __int64 totalSize)
 *	\brief Change progress stats values
 *	\param[in] __int64 totalSize
 *	\param[in] __int64 bytesDone
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void SetNonSysInplaceEncUIStatus (int nonSysInplaceEncStatus)
 *	\brief Change NonSysInplaceEncStatus value
 *	\param[in] int nonSysInplaceEncStatus
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL SaveNonSysInPlaceEncSettings (int delta, WipeAlgorithmId newWipeAlgorithm)
 *	\brief Save in settings
 *	\param[in] WipeAlgorithmId newWipeAlgorithm
 *	\param[in] int delta
 *	\return BOOL TRUE if (delta == 0)
 *			TRUE if (count < 1)
 *			SaveBufferToFile (str, GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC), strlen(str), FALSE)
 *
 */
...
/**
 *
 *	\fn int ZeroUnreadableSectors (HANDLE dev, LARGE_INTEGER startOffset, int64 size, int sectorSize, uint64 *zeroedSectorCount)
 *	\brief Repairs damaged sectors (i.e. those with read errors) by zeroing them. 
 *			Note that this operating fails if there are any write errors.
 *	\param[in] uint64 * zeroedSectorCount
 *	\param[in] int sectorSize
 *	\param[in] int64 size
 *	\param[in] LARGE_INTEGER startOffset
 *	\param[in] HANDLE dev
 *	\return int ERR_OS_ERROR if (SetFilePointerEx (dev, startOffset, NULL, FILE_BEGIN) == 0)
 *			ERR_OS_ERROR if (SetFilePointerEx (dev, workOffset, NULL, FILE_BEGIN) == 0)
 *			ERR_OS_ERROR if (WriteFile (dev, sectorBuffer, sectorSize, &n, NULL) == 0)
 *			ERR_OUTOFMEMORY if (!sectorBuffer)
 *			ERR_SUCCESS otherwise
 *
 */
...
/**
 *
 *	\fn static int OpenBackupHeader (HANDLE dev, const char *devicePath, Password *password, PCRYPTO_INFO *retMasterCryptoInfo, CRYPTO_INFO *headerCryptoInfo, __int64 deviceSize)
 *	\brief Open the header backup
 *	\param[in] __int64 deviceSize
 *	\param[in] CRYPTO_INFO * headerCryptoInfo
 *	\param[in] PCRYPTO_INFO * retMasterCryptoInfo
 *	\param[in] Password * password
 *	\param[in] const char
 *	\param[in] HANDLE dev
 *	\return static int ERR_SUCCESS
 *			ERR_OUTOFMEMORY if (!header)
 *			ERR_OS_ERROR if (SetFilePointerEx (dev, offset, NULL, FILE_BEGIN) == 0 || !ReadEffectiveVolumeHeader (TRUE, dev, (byte *) header, &n) || n < GST_VOLUME_HEADER_EFFECTIVE_SIZE)
 *
 */
...
/**
 *
 *	\fn static BOOL GetFreeClusterBeforeThreshold (HANDLE volumeHandle, int64 *freeCluster, int64 clusterThreshold)
 *	\brief Get the free cluster before threshold
 *	\param[in] int64 clusterThreshold
 *	\param[in] int64 * freeCluster
 *	\param[in] HANDLE volumeHandle
 *	\return static BOOL TRUE if ((bitmap->Buffer[bitmapIndex] & (1 << bit)) == 0)
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL MoveClustersBeforeThresholdInDir (HANDLE volumeHandle, const wstring &directory, int64 clusterThreshold)
 *	\brief Move cluster to directory before threshold
 *	\param[in] int64 clusterThreshold
 *	\param[in] const wstring
 *	\param[in] HANDLE volumeHandle
 *	\return static BOOL FALSE if (retry > 600)
 *			FALSE if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn BOOL MoveClustersBeforeThreshold (HANDLE volumeHandle, PWSTR volumeDevicePath, int64 clusterThreshold)
 *	\brief Move the cluster before threshold
 *	\param[in] int64 clusterThreshold
 *	\param[in] PWSTR volumeDevicePath
 *	\param[in] HANDLE volumeHandle
 *	\return BOOL FALSE if (drive == -1)
 *			MoveClustersBeforeThresholdInDir (volumeHandle, volumeRoot, clusterThreshold) otherwise
 *
 */
