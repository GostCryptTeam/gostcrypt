 ...
 *
 *	\file Password.c
 *	\brief Manage the basic checks and actions on the password such as creating / modifying checking length and encoding
 *	\version 1.3.1
 *	\date 20/01/2017
 *
 */
...
/**
 *
 *	\fn void VerifyPasswordAndUpdate(HWND hwndDlg, HWND hButton, HWND hPassword, HWND hVerify, unsigned char *szPassword, char *szVerify, BOOL keyFilesEnabled)
 *	\brief Verify the password value (minimum size and repeated is equal) when the user enter it for the first time to protect its folder.
 *	\param[in] HWND hwndDlg
 *	\param[in] HWND hButton
 *	\param[in] HWND hPassword
 *	\param[in] HWND hVerify
 *	\param[in] unsigned char * szPassword
 *	\param[in] char * szVerify
			 ...
			 BOOL keyFilesEnabled)
{
	...
	if (hwndDlg);		/* Remove warning */ // useless
...
/**
 *
 *	\fn BOOL CheckPasswordCharEncoding (HWND hPassword, Password *ptrPw)
 *	\brief Check if the password contains ASCII or printable character.
 *	\param[in] Password * ptrPw
 *	\param[in] HWND hPassword
 *	\return BOOL FALSE if (i < len)
 *			FALSE if (len > MAX_PASSWORD)
 *			FALSE if (pw[i] >= 0x7f || pw[i] < 0x20)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL CheckPasswordLength (HWND hwndDlg, HWND hwndItem)
 *	\brief Check the password length
 *	\param[in] HWND hwndItem
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (MessageBoxW (hwndDlg, GetString ("PASSWORD_LENGTH_WARNING"), lpszTitle, MB_YESNO|MB_ICONWARNING|MB_DEFBUTTON2) != IDYES)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn int ChangePwd (char *lpszVolume, Password *oldPassword, Password *newPassword, int pkcs5, HWND hwndDlg)
 *	\brief Check the old password. Check all the requirements if the new password fits the rules and the program doesn't have problem.
 *			make a copy of teh ciphered area, encrypt the old area with the new password and if the encryption is successful change the password otherwise cancel.
 *	\param[in] HWND hwndDlg
 *	\param[in] int pkcs5
 *	\param[in] Password * newPassword
 *	\param[in] Password * oldPassword
 *	\param[in] char * lpszVolume
 *	\return int ERR_OS_ERROR if (nDosLinkCreated != 0)
 *			ERR_OS_ERROR if (dev == INVALID_HANDLE_VALUE)
 *			ERR_OS_ERROR if (bDevice && (dev == INVALID_HANDLE_VALUE))
 *			ERR_OS_ERROR if (bDevice && !(dev == INVALID_HANDLE_VALUE) && (!bResult))
 *			ERR_VOL_SIZE_WRONG if (bDevice && !(dev == INVALID_HANDLE_VALUE) && (hostSize == 0))
 *			ERR_OS_ERROR if (!bDevice && (!GetFileSizeEx (dev, &fileSize)))
 *	
 *			ERR_OS_ERROR if (Randinit ())
 *			ERR_OS_ERROR if (!SetFilePointerEx ((HANDLE) dev, headerOffset, NULL, FILE_BEGIN))
 *			ERR_OS_ERROR if (!ReadEffectiveVolumeHeader (bDevice, dev, buffer, &bytesRead))
 *			ERR_SYS_HIDVOL_HEAD_REENC_MODE_WRONG if (cryptoInfo->HeaderFlags & GST_HEADER_FLAG_ENCRYPTED_SYSTEM)
 *			ERR_OS_ERROR if (!SetFilePointerEx ((HANDLE) dev, headerOffset, NULL, FILE_BEGIN))
 *			ERR_OS_ERROR if (!WriteEffectiveVolumeHeader (bDevice, dev, buffer))
 *			WriteRandomDataToReservedHeaderAreas (dev, cryptoInfo, cryptoInfo->VolumeSize.Value, !backupHeader, backupHeader) if (nStatus != ERR_SUCCESS)
 *			0 else
 *
 */
