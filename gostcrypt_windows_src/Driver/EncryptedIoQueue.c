/**
 *
 *	\file EncryptedIoQueue.c
 *	\brief Manage the fragment for encryption on the drive, the sending for irp packets and device filter.
 *	The encryption queue will order the axtions to do.
 *	\version 1.3.1
 *	\date 13/12/2016
 *
 */
...
/**
 *
 *	\fn static void AcquireBufferPoolMutex (EncryptedIoQueue *queue)
 *	\brief Acquire buffer pool mutex
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ReleaseBufferPoolMutex (EncryptedIoQueue *queue)
 *	\brief Release buffer pool mutex
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void *GetPoolBuffer (EncryptedIoQueue *queue, ULONG requestedSize)
 *	\brief Get the pool buffer
 *	\param[in] ULONG requestedSize
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void ReleasePoolBuffer (EncryptedIoQueue *queue, void *address)
 *	\brief Release pool buffer
 *	\param[in] void * address
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void FreePoolBuffers (EncryptedIoQueue *queue)
 *	\brief Free the pool buffer
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void DecrementOutstandingIoCount (EncryptedIoQueue *queue)
 *	\brief Decrement IO count
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void OnItemCompleted (EncryptedIoQueueItem *item, BOOL freeItem)
 *	\brief When item is completed, release in the Queue, unlock, add the number of bytes written to the count, release the pool buffer.
 *	\param[in] BOOL freeItem
 *	\param[in] EncryptedIoQueueItem * item
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static NTSTATUS CompleteOriginalIrp (EncryptedIoQueueItem *item, NTSTATUS status, ULONG_PTR information)
 *	\brief Complete the disk irp packet.
...
/**
 *
 *	\fn static void AcquireFragmentBuffer (EncryptedIoQueue *queue, byte *buffer)
 *	\brief Get the update the status by the fragmentbuffer that's located in the queue
...
/**
 *
 *	\fn static void ReleaseFragmentBuffer (EncryptedIoQueue *queue, byte *buffer)
 *	\brief Release the fragment buffer
 *	\param[in] byte * buffer
 *	\param[in] EncryptedIoQueue * queue
 *	\return static void
 *
...
/**
 *
 *	\fn static VOID CompletionThreadProc (PVOID threadArg)
 *	\brief Derypt data, send the irp packet
...
/**
 *
 *	\fn static NTSTATUS GSTCachedRead (EncryptedIoQueue *queue, IO_STATUS_BLOCK *ioStatus, PVOID buffer, LARGE_INTEGER offset, ULONG length)
 *	\brief Verify that the block that is supposed to be read is valid, if it is valid read it.
 *	\param[in] ULONG length
 *	\param[in] LARGE_INTEGER offset
 *	\param[in] PVOID buffer
 *	\param[in] IO_STATUS_BLOCK * ioStatus
 *	\param[in] EncryptedIoQueue * queue
 *	\return static NTSTATUS ZwReadFile (queue->HostFileHandle, NULL, NULL, NULL, ioStatus, buffer, length, &offset, NULL)
 *
 */
...
/**
 *
 *	\fn static VOID IoThreadProc (PVOID threadArg)
 *	\brief Start the thread and set its priority to low. Perform IO requests if no precedent requests failed. 
 *			Fragment the data in the request in maximum three fragment. Remap the fragment.
 *			If the request is not write, read the data buffer. Return success when the program terminates.
 *	\param[in] PVOID threadArg
 *	\return static VOID
 *
 */
...
/**
 *
 *	\fn static VOID MainThreadProc (PVOID threadArg)
 *	\brief Start the main thread, the priority is low also. Enter into a loop that wait to exit if the queue reach the exit request
 *			Create an IRP Check the IRP major function to execute their request (read or write).
 *			Manage the filter device and hidden volume.
 *			Handle misaligned read operations to work around a bug in Windows System Assessment
 *			Tool which does not follow FILE_FLAG_NO_BUFFERING requirements when benchmarking disk devices
 *			Fragment the data and encrypt them.
 *	\param[in] PVOID threadArg
 *	\return static VOID
 *
 */
...
/**
 *
 *	\fn NTSTATUS EncryptedIoQueueAddIrp (EncryptedIoQueue *queue, PIRP irp)
 *	\brief Remove the lock on the queue and add the irp to the main queue then lock it
 *	\param[in] PIRP irp
 *	\param[in] EncryptedIoQueue * queue
 *	\return NTSTATUS status if error
 *			STATUS_PENDING else
 *
 */
...
/**
 *
 *	\fn NTSTATUS EncryptedIoQueueHoldWhenIdle (EncryptedIoQueue *queue, int64 timeout)
 *	\brief Duspend the queue, encrypt a single object and if success resume the queue
 *	\param[in] int64 timeout
 *	\param[in] EncryptedIoQueue * queue
...
/**
 *
 *	\fn BOOL EncryptedIoQueueIsSuspended (EncryptedIoQueue *queue)
 *	\brief Getter for returning the suspended status
 *	\param[in] EncryptedIoQueue * queue
 *	\return BOOL queue->Suspended
 *
 */
...
/**
 *
 *	\fn BOOL EncryptedIoQueueIsRunning (EncryptedIoQueue *queue)
 *	\brief Getter of StopPending, return if the queue is running or not
 *	\param[in] EncryptedIoQueue * queue
 *	\return BOOL !queue->StopPending
 *
 */
...
/**
 *
 *	\fn NTSTATUS EncryptedIoQueueResumeFromHold (EncryptedIoQueue *queue)
...
/**
 *
 *	\fn NTSTATUS EncryptedIoQueueStart (EncryptedIoQueue *queue)
 *	\brief Start the encrypting queue and launch the encryption in the main queue thread, the IO thread, and completion thread
 *	\param[in] EncryptedIoQueue * queue
 *	\return NTSTATUS STATUS_SUCCESS if success
 *			status if error
 *
...
/**
 *
 *	\fn NTSTATUS EncryptedIoQueueStop (EncryptedIoQueue *queue)
 *	\brief Stop the encrypting queue
 *	\param[in] EncryptedIoQueue * queue
 *	\return NTSTATUS STATUS_SUCCESS
 *
 */
