 ...
 *
 *	\file Gstformat.c
 *	\brief Manage calls for volume formatting functions and Handle the interface for Volume creation.
 *	\version 1.3.1
 *	\date 14/12/2016
 *
 */
...
/**
 *
 *	\fn static BOOL ElevateWholeWizardProcess (string arguments)
 *	\brief To install the wizard need the admin privileged to install, this function assure that this process have these privileged.
 *	\param[in] string arguments
 *	\return static BOOL TRUE if the process is admin
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn static void WipePasswordsAndKeyfiles (void)
 *	\brief Delete the passwords and key files that are in the field buffers
 *	\return static void
 *
 */
 ...
 *	\brief Clean localy the variables
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK BroadcastSysEncCfgUpdateCallb (HWND hwnd, LPARAM lParam)
 *	\brief Post a message in broadcast about the system encryption status when it is updated
...
/**
 *
 *	\fn static BOOL BroadcastSysEncCfgUpdate (void)
 *	\brief start BroadcastSysEncCfgUpdateCallb function
 *	\return bSuccess can be TRUE or FALSE
 *
 */
...
/**
 *
 *	\fn static BOOL SaveSysEncSettings (HWND hwndDlg)
 *	\brief IMPORTANT: This function may be called only by Format (other modules can only _read_ the system encryption config).
 *		   Returns TRUE if successful (otherwise FALSE)
 *		   save the encrypted settings in the config files
 *	\param[in] HWND hwndDlg
 *	\return static BOOL TRUE if (!bSystemEncryptionStatusChanged)
 *			FALSE if (!CreateSysEncMutex ())
 *			FALSE if (remove (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION)) != 0)
 *			TRUE if not
 *			FALSE if (f == NULL)
 *			FALSE if (XmlWriteHeader (f) < 0 || fputs ("\n\t<sysencryption>", f) < 0 || fprintf (f, "\n\t\t<config key=\"SystemEncryptionStatus\">%d</config>", SystemEncryptionStatus) < 0 || fprintf (f, "\n\t\t<config key=\"WipeMode\">%d</config>", (int) nWipeMode) < 0 || fputs ("\n\t</sysencryption>", f) < 0 || XmlWriteFooter (f) < 0)
 *			TRUE otherwise
 */
...
/**
 *
 *	\fn static unsigned int DetermineHiddenOSCreationPhase (void)
 *	\brief WARNING: This function may take a long time to finish
 *		   try to get the hidden OS phase, abort if cannot otherwise return the phase.
 *	\return static unsigned int phase
 *
 */
...
/**
 *
 *	\fn static BOOL ChangeHiddenOSCreationPhase (int newPhase) 
 *	\brief IMPORTANT: This function may be called only by Format (other modules can only _read_ the system encryption status).
 *			Returns TRUE if successful (otherwise FALSE)
 *			change the phase of the hidden OS to the new phase in parameter
...
/**
 *
 *	\fn static BOOL ChangeSystemEncryptionStatus (int newStatus)
 *	\brief IMPORTANT: This function may be called only by Format (other modules can only _read_ the system encryption status).
 *			Returns TRUE if successful (otherwise FALSE)
 *			set the status of the encryption to newStatus
 *	\param[in] int newStatus
 *	\return static BOOL FALSE if (!CreateSysEncMutex ())
 *			FALSE if (newStatus == SYSENC_STATUS_ENCRYPTING)
 *			FALSE if (newStatus == SYSENC_STATUS_NONE && !IsHiddenOSRunning())
 *			FALSE if (!SaveSysEncSettings (MainDlg))
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL ChangeWizardMode (int newWizardMode)
 *	\brief If the return code of this function is ignored and newWizardMode == WIZARD_MODE_SYS_DEVICE, then this function
 *			may be called only after CreateSysEncMutex() returns TRUE. It returns TRUE if successful (otherwise FALSE).
 *	\param[in] int newWizardMode
 *	\return static BOOL FALSE if (!ElevateWholeWizardProcess ("/e"))
 *			FALSE if (!CreateSysEncMutex ())
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL SysEncInEffect (void)
 *	\brief Determines whether the wizard directly affects system encryption in any way.
 ...
 *	\return static BOOL (WizardMode == WIZARD_MODE_SYS_DEVICE|| CreatingHiddenSysVol())
 *
 */
...
/**
 *
 *	\fn static BOOL CreatingHiddenSysVol (void)
 *	\brief Return TRUE if there is hiddenOS and Volume but not the hidden host volume
 *	\return static BOOL (bHiddenOS && bHiddenVol && !bHiddenVolHost)
 *
 */
...
/**
 *
 *	\fn static void LoadSettings (HWND hwndDlg)
 *	\brief Load settings from config files and set the global variable to these values
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void SaveSettings (HWND hwndDlg)
 *	\brief Save settings into the configuration files.
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void EndMainDlg (HWND hwndDlg)
 *	\brief WARNING: This function does NOT cause immediate application exit (use e.g. return 1 after calling it
...
/**
 *
 *	\fn BOOL SwitchWizardToSysEncMode (void)
 *	\brief This functions is to be used when the wizard mode needs to be changed to WIZARD_MODE_SYS_DEVICE.
 *			If the function fails to switch the mode, it returns FALSE (otherwise TRUE).
 *	\return BOOL FALSE catch (Exception &e)
 *			FALSE if (!CreateSysEncMutex ())
 *			FALSE if (!ResolveUnknownSysEncDirection ())
 *			FALSE else (!ResolveUnknownSysEncDirection ())
 *			FALSE if (!ChangeWizardMode (WIZARD_MODE_SYS_DEVICE))
 *			FALSE if (AskYesNo ("ASK_ENCRYPT_PARTITION_INSTEAD_OF_DRIVE") == IDNO)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn void SwitchWizardToFileContainerMode (void)
 *	\brief Switch the wizard to file container mode
 *	\return void
 *
 */
...
/**
 *
 *	\fn void SwitchWizardToNonSysDeviceMode (void)
 *	\brief Switch wizard to non sys device mode.
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL SwitchWizardToHiddenOSMode (void)
 *	\brief Switch wizard to Hidden OS Mode
 *	\return BOOL FALSE catch (Exception &e)
 *			FALSE else (SwitchWizardToSysEncMode())
 *			TRUE else (nCurPageNo != SYSENC_ENCRYPTION_PAGE)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn void SwitchWizardToNonSysInplaceEncResumeMode (void)
 *	\brief Switch Wizard To Non Sys Inplace Enc Resume Mode.
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL ResolveUnknownSysEncDirection (void)
 *	\brief Use this function e.g. if the config file with the system encryption settings was lost or not written
 ...
 *	\return static BOOL TRUE if (CreateSysEncMutex ()) and no error
 *			FALSE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL ForceRemoveSysEnc (void)
 *	\brief This function should be used to resolve inconsistencies that might lead to a deadlock (inability to encrypt or
...
/**
 *
 *	\fn __int64 GetSystemPartitionSize (void)
 *	\brief Returns 0 if there's an error. otherwise return the size of the partition.
 *	\return __int64 BootEncObj->GetSystemDriveConfiguration().SystemPartition.Info.PartitionLength.QuadPart
 *			0 catch (Exception &e)
 *
 */
...
/**
 *
 *	\fn void ComboSelChangeEA (HWND hwndDlg)
 *	\brief EA : encryption algorithm. In the volume creation wizard this function manage the algorithm choice window.
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void VerifySizeAndUpdate (HWND hwndDlg, BOOL bUpdate)
 *	\brief Verify the size of a volume and update the variables.
 *	\param[in] BOOL bUpdate
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void UpdateWizardModeControls (HWND hwndDlg, int setWizardMode)
 *	\brief Update the wizard into the mode asked.
 *	\param[in] int setWizardMode
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static int GetSelectedWizardMode (HWND hwndDlg)
 *	\brief Get the selected wizard by the user.
 *	\param[in] HWND hwndDlg
 *	\return static int The mode of the wizard that can be :
 *			WIZARD_MODE_FILE_CONTAINER
 *			WIZARD_MODE_NONSYS_DEVICE
 *			WIZARD_MODE_SYS_DEVICE
 *			DEFAULT_VOL_CREATION_WIZARD_MODE
 *
 */
...
/**
 *
 *	\fn static void RefreshMultiBootControls (HWND hwndDlg)
 *	\brief If in DEBUG mode start in multi boot otherwise refresh the multi boot option to let the user chose
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static int Get2RadButtonPageAnswer (void)
 *	\brief Return if a page answer with a button checked -1 = Undecided or error, 0 = No, 1 = Yes
 *	\return static int 1 if button1 checked
 *			0 if button2 checked
 *			-1 otherwise
 *
 */
...
/**
 *
 *	\fn static void Update2RadButtonPage (int answer)
 *	\brief Set a button on a page checked or unchecked 0 = No, 1 = Yes
 *	\param[in] int answer
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void Init2RadButtonPageYesNo (int answer)
 *	\brief Initialise a button in a page -1 = Undecided, 0 = No, 1 = Yes
 *	\param[in] int answer
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void UpdateSysEncProgressBar (void)
...
/**
 *
 *	\fn static void InitSysEncProgressBar (void)
 *	\brief Initialise the progress bar
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void UpdateSysEncControls (void)
 *	\brief Update the encryption control system
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void SysEncPause (void)
 *	\brief Pause de encryption system
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void SysEncResume (void)
 *	\brief Resume the encryption system
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static BOOL GetDevicePathForHiddenOS (void)
 *	\brief Get the path for the device in case of hiddenOS mode. Return false in case of error, otherwise return the device,file name ,and directory
 *	\return static BOOL FALSE catch (Exception &e)
 *			(szFileName[0] != 0 && szDiskFile[0] != 0 && tmpbDevice) otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL CheckGapBetweenSysAndHiddenOS (void)
 *	\brief Returns TRUE if there is unallocated space greater than 64 MB (max possible slack space size) between the 
		   boot partition and the first partition behind it. If there's none or if an error occurs, returns FALSE.
 *	\return static BOOL FALSE catch (Exception &e)
 *			(sysDriveCfg.SystemPartition.Info.StartingOffset.QuadPart + sysDriveCfg.SystemPartition.Info.PartitionLength.QuadPart+ 64 * BYTES_PER_MB+ 128 * BYTES_PER_KB <= BootEncObj->GetPartitionForHiddenOS().Info.StartingOffset.QuadPart) otherwise
 *
 */
...
/**
 *
 *	\fn static void NonSysInplaceEncPause (void)
 *	\brief Set in pause a thread the GUI events (10 seconds max.)
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void NonSysInplaceEncResume (void)
 *	\brief Resume the GUI events
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void ShowNonSysInPlaceEncUIStatus (void)
 *	\brief Show the status of the encryption, pause encrypting ....
 *	\return void
 *
 */
...
/**
 *
 *	\fn void UpdateNonSysInPlaceEncControls (void)
 *	\brief Manage controls for the encryption windows.
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void UpdateNonSysInplaceEncProgressBar (void)
 *	\brief Update the progress bar of encryption
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void InitNonSysInplaceEncProgressBar (void)
 *	\brief Initialise the progress bar.
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void DisplayRandPool (HWND hPoolDisplay, BOOL bShow)
 *	\brief Display the random nuber for the encryption
 *	\param[in] BOOL bShow
 *	\param[in] HWND hPoolDisplay
 *	\return void
 *
 */
...
/**
 *
 *	\fn void DisplayPortionsOfKeys (HWND headerKeyHandle, HWND masterKeyHandle, char *headerKeyStr, char *masterKeyStr, BOOL hideKeys)
 *	\brief Dislay stars to hide the key but it shows that you are writing in the text box.
 *	\param[in] BOOL hideKeys
 *	\param[in] char * masterKeyStr
 *	\param[in] char * headerKeyStr
 *	\param[in] HWND masterKeyHandle
 *	\param[in] HWND headerKeyHandle
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void WipeAbort (void)
 *	\brief Abort the wipe
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void WipeStart (void)
 *	\brief Start the wipe (progress bar)
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void UpdateWipeProgressBar (void)
 *	\brief Update the wipe progress bar
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void InitWipeProgressBar (void)
 *	\brief Initialize the wipe progress bar
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void UpdateWipeControls (void)
 *	\brief Update the wipe controls on decoys system partition and regular devices
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void __cdecl sysEncDriveAnalysisThread (void *hwndDlgArg)
 *	\brief Mark "in progress" or "completed" and delay to prevent confusion
 *	\param[in] void * hwndDlgArg
 *	\return static void __cdecl
 *
 */
...
/**
 *
 *	\fn static void __cdecl volTransformThreadFunction (void *hwndDlgArg)
 *	\brief Create a volume, assign a drive letter and clear the outer volume password
 *	\param[in] void * hwndDlgArg
 *	\return static void __cdecl
 *
 */
...
/**
 *
 *	\fn static void LoadPage (HWND hwndDlg, int nPageNo)
 *	\brief Load a page with its nPageNo
 *	\param[in] int nPageNo
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn int PrintFreeSpace (HWND hwndTextBox, char *lpszDrive, PLARGE_INTEGER lDiskFree)
 *	\brief Print the free space
 *	\param[in] PLARGE_INTEGER lDiskFree
 *	\param[in] char * lpszDrive
 *	\param[in] HWND hwndTextBox
 *	\return int nMultiplier
 *
 */
...
/**
 *
 *	\fn void DisplaySizingErrorText (HWND hwndTextBox)
 *	\brief Display error
 *	\param[in] HWND hwndTextBox
 *	\return void
 *
...
/**
 *
 *	\fn void EnableDisableFileNext (HWND hComboBox, HWND hMainButton)
 *	\brief Enable or disable hMainButton & set focus to correct window
 *	\param[in] HWND hMainButton
...
/**
 *
 *	\fn BOOL IsSparseFile (HWND hwndDlg)
 *	\brief  Returns TRUE if the file is a sparse file. If it's not a sparse file or in case of any error, returns FALSE.
 *	\param[in] HWND hwndDlg
 *	\return BOOL retCode
 *			FALSE if (hFile == INVALID_HANDLE_VALUE)
 *
 */
...
/**
 *
 *	\fn BOOL GetFileVolSize (HWND hwndDlg, unsigned __int64 *size)
 *	\brief Get the volume size of a file
 *			Note: GetFileVolSize is not to be used for devices (only for file-hosted volumes)
 *	\param[in] unsigned __int64
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (hFile == INVALID_HANDLE_VALUE)
 *			FALSE if (GetFileSizeEx(hFile, &fileSize) == 0)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn BOOL QueryFreeSpace (HWND hwndDlg, HWND hwndTextBox, BOOL display)
 *	\brief Query to get free space
 *	\param[in] BOOL display
 *	\param[in] HWND hwndTextBox
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if (bHiddenVol && !bHiddenVolHost)
 *			FALSE if (!GetVolumePathName (szFileName, root, sizeof (root)))
 *			FALSE if (!GetDiskFreeSpaceEx (root, &free, 0, 0))
 *			TRUE else (!GetDiskFreeSpaceEx (root, &free, 0, 0))
 *			FALSE if (!piValid && !gValid)
 *			FALSE if (sectorSize < GST_MIN_VOLUME_SECTOR_SIZE || sectorSize > GST_MAX_VOLUME_SECTOR_SIZE || sectorSize % ENCRYPTION_DATA_UNIT_SIZE != 0)
 *			FALSE if (nVolumeSize == 0)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn static BOOL FinalPreTransformPrompts (void)
 *	\brief If we find that the partition contains more than 1GB of data or more than 12%
 *			of its space is occupied, we will display an extra warning.
 *	\return static BOOL FALSE if (x != IDYES)
 *			FALSE switch (AskMultiChoice ((void **) tmpStr, TRUE) case default
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn void HandleOldAssignedDriveLetter (void)
 *	\brief Handle old assigned drive letter
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL FileSize4GBLimitQuestionNeeded (void)
 *	\brief Returns TRUE if it makes sense to ask the user whether he wants to store files larger than 4GB in the volume.
 *	\return static BOOL (dataAreaSize > 4 * BYTES_PER_GB + GST_MIN_FAT_FS_SIZE && dataAreaSize <= GST_MAX_FAT_SECTOR_COUNT * GetFormatSectorSize())
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK PageDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Draw the page in GostCrypt Volume Creation Wizard
 ...
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (AskMultiChoice ((void **) tmpStr, FALSE)) case 1 
 *			0 switch (AskMultiChoice ((void **) tmpStr, FALSE)) case default
 *			0 catch (Exception &e)
 *			0 switch (uMsg) case SYSENC_ENCRYPTION_PAGE
 *			1 on many events caught in switch (uMsg)
 *			0 if no event returned 1 before
 *
 */
 ...
 *	\brief Main dialog proc is the main function that manage Volume Creation page window display and connections with GUI and execution 
 ...
 *			0 else (lw == IDC_ABOUT)
 *			1 case TIMER_ID_WIPE_PROGRESS if event is catch 0 otherwise
 *			1 in switch (uMsg) when event is caught
 *			0 switch (uMsg) WM_ENDSESSION
 *			0 if not catch in switch (uMsg)
 *
 */
 ...
 *	\brief Extract and execute command line
...
/**
 *
 *	\fn int DetermineMaxHiddenVolSize (HWND hwndDlg)
 *	\brief Determine the maximum hidden volume size
 *	\param[in] HWND hwndDlg
 *	\return int 1
 *
 */
...
/**
 *
 *	\fn int AnalyzeHiddenVolumeHost (HWND hwndDlg, int *driveNo, __int64 hiddenVolHostSize, int *realClusterSize, __int64 *pnbrFreeClusters)
 *	\brief Tests whether the file system of the given volume is suitable to host a hidden volume,
 ...
 *	\param[in] __int64 * pnbrFreeClusters
 *	\param[in] int * realClusterSize
 *	\param[in] __int64 hiddenVolHostSize
 *	\param[in] int * driveNo
 *	\param[in] HWND hwndDlg
 *	\return int ScanVolClusterBitmap (hwndDlg,driveNo,hiddenVolHostSize / *realClusterSize,pnbrFreeClusters)
 *			0 or -1 if error
 *
 */
...
/**
 *
 *	\fn int MountHiddenVolHost (HWND hwndDlg, char *volumePath, int *driveNo, Password *password, BOOL bReadOnly)
 *	\brief Mounts a volume within which the user intends to create a hidden volume
 *	\param[in] BOOL bReadOnly
 *	\param[in] Password * password
 *	\param[in] int * driveNo
 *	\param[in] char * volumePath
 *	\param[in] HWND hwndDlg
 *	\return int ERR_VOL_MOUNT_FAILED if mount failed
 *			ERR_NO_FREE_DRIVES if no free drives
 *			0 otherwise
 *
 */
...
/**
 *
 *	\fn int ScanVolClusterBitmap (HWND hwndDlg, int *driveNo, __int64 nbrClusters, __int64 *nbrFreeClusters)
 *	\brief Gets the map of the clusters that are free and in use on a volume that is to host
...
/**
 *
 *	\fn static BOOL WipeHiddenOSCreationConfig (void)
 *	\brief Wipe the hidden OS config flag bits in the MBR
 *	\return static BOOL FALSE catch (Exception &e)
 *			TRue otherwise
 *
 */
...
/**
 *
 *	\fn static void AfterSysEncProgressWMInitTasks (HWND hwndDlg)
 *	\brief Tasks that need to be performed after the WM_INITDIALOG message for the SYSENC_ENCRYPTION_PAGE dialog is
...
/**
 *
 *	\fn static void AfterWMInitTasks (HWND hwndDlg)
 *	\brief Tasks that need to be performed after the WM_INITDIALOG message is handled must be done here. 
 ...
 *	\brief Create the drivers for the coming volume to mount.
 *			GUI commands for volume creation
...
/**
 *
 *	\fn static int GetFormatSectorSize ()
 *	\brief Get the Bytes per sector
 *	\return static int geometry.BytesPerSector
 *
 */
