 ...
 *
 *	\file Setup.c
 *	\brief Manage the graphical interface for the installer.
 *			the files generated from other projects will be compressed and saved into the installer,
 *			when launching the extractor it will uncompress all the files that will make the program (executable archive).
 *	\version 1.3.1
 *	\date 14/12/2016
 *
 */
...
/**
 *
 *	\fn void localcleanup (void)
 *	\brief Call the cleanup function, that will free and close objects
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL StatDeleteFile (char *lpszFile)
 *	\brief Delete the lpszFile (maybe only if empty)
 *	\param[in] char * lpszFile
 *	\return BOOl DeleteFile (lpszFile) if (_stat64 (lpszFile, &st) == 0)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL StatRemoveDirectory (char *lpszDir)
 *	\brief Delete the lpszDir.
 *	\param[in] char * lpszDir
 *	\return BOOL RemoveDirectory (lpszDir) if (_stat64 (lpszDir, &st) == 0)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn HRESULT CreateLink (char *lpszPathObj, char *lpszArguments, char *lpszPathLink)
 *	\brief Create a shortcut linked to the program.
 *	\param[in] char * lpszPathObj
 *	\param[in] char * lpszArguments
 *	\param[in] char * lpszPathLink
 *	\return HRESULT hres
 *
 */
...
/**
 *
 *	\fn void GetProgramPath (HWND hwndDlg, char *path)
 *	\brief Set the path to the program. 
 *	\param[in] char * path
...
/**
 *
 *	\fn void StatusMessage (HWND hwndDlg, char *stringId)
 *	\brief Send a message giving information about the status of the window status
...
/**
 *
 *	\fn void StatusMessageParam (HWND hwndDlg, char *stringId, char *param)
 *	\brief Send a message to a item of the window
 *	\param[in] char * param
 *	\param[in] char * stringId
 *	\param[in] HWND hwndDlg
 *	\return void
...
/**
 *
 *	\fn void ClearLogWindow (HWND hwndDlg)
 *	\brief Clear the log of the window
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn void RegMessage (HWND hwndDlg, char *txt)
 *	\brief Set the registry message to an item on the display
 *	\param[in] char * txt
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn void CopyMessage (HWND hwndDlg, char *txt)
 *	\brief Copy txt to an item on the display
 *	\param[in] char * txt
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn void RemoveMessage (HWND hwndDlg, char *txt)
 *	\brief Remove txt from the display
 *	\param[in] char * txt
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn void IconMessage (HWND hwndDlg, char *txt)
 *	\brief Add an icon to the display
 *	\param[in] char * txt
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn void DetermineUpgradeDowngradeStatus (BOOL bCloseDriverHandle, LONG *driverVersionPtr)
 *	\brief Find the driver version and determines the upgrade/downgrade status
 *	\param[in] LONG * driverVersionPtr
 *	\param[in] BOOL bCloseDriverHandle
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL IsFileInUse (const string &filePath)
 *	\brief Check if the files is already used
 *	\param[in] const string & filePath
 *	\return static BOOL TRUE if (GetLastError() == ERROR_SHARING_VIOLATION)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn BOOL DoFilesInstall (HWND hwndDlg, char *szDestDir)
 *	\brief Install all files
 *	\param[in] char * szDestDir
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (!SelfExtractInMemory (szTmp))
 *			FALSE if (x < 2)
 *			FALSE if (!Silent && MessageBoxW (hwndDlg, szTmp2, lpszTitle, MB_YESNO | MB_ICONHAND) != IDYES)
 *			bOK else
 *
 */
...
/**
 *
 *	\fn BOOL DoRegInstall (HWND hwndDlg, char *szDestDir, BOOL bInstallType)
 *	\brief Install the registery
 *	\param[in] BOOL bInstallType
 *	\param[in] char * szDestDir
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if (SystemEncryptionUpdate)
 *			FALSE if (IsOSAtLeast (WIN_VISTA) && !RegisterComServers (szDir))
 *			bOK else
 *
 */
...
/**
 *
 *	\fn BOOL DoApplicationDataUninstall (HWND hwndDlg)
 *	\brief Uninstall the data application
 *	\param[in] HWND hwndDlg
 *	\return BOOL bOK
 *
 */
...
/**
 *
 *	\fn BOOL DoRegUninstall (HWND hwndDlg, BOOL bRemoveDeprecated)
 *	\brief Uninstall the register
 *	\param[in] BOOL bRemoveDeprecated
 *	\param[in] HWND hwndDlg
 *	\return BOOL bOK
 *
 */
...
/**
 *
 *	\fn BOOL DoServiceUninstall (HWND hwndDlg, char *lpszService)
 *	\brief Uninstall the service
 *	\param[in] char * lpszService
 *	\param[in] HWND hwndDlg
 *	\return BOOL bOK
 *
 */
...
/**
 *
 *	\fn BOOL DoDriverUnload (HWND hwndDlg)
 *	\brief Unload the driver
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (bUninstallInProgress || bDowngrade)
 *			bOk else
 *
 */
...
/**
 *
 *	\fn BOOL UpgradeBootLoader (HWND hwndDlg)
 *	\brief Upgrade the bootloader
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if (!SystemEncryptionUpdate)
...
/**
 *
 *	\fn BOOL DoShortcutsUninstall (HWND hwndDlg, char *szDestDir)
 *	\brief Make the shortcut of uninstalled .exe
 *	\param[in] char * szDestDir
 *	\param[in] HWND hwndDlg
 *	\return BOOL bOK
 *
...
/**
 *
 *	\fn BOOL DoShortcutsInstall (HWND hwndDlg, char *szDestDir, BOOL bProgGroup, BOOL bDesktopIcon)
 *	\brief Make the shortcut of installed .exe
 *	\param[in] BOOL bDesktopIcon
 *	\param[in] BOOL bProgGroup
 *	\param[in] char * szDestDir
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if (bProgGroup == FALSE && bDesktopIcon == FALSE)
 *			bOk else
 *
 */
...
/**
 *
 *	\fn void OutcomePrompt (HWND hwndDlg, BOOL bOK)
 *	\brief Print the outcome of installation and uninstallation
 *	\param[in] BOOL bOK
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn static void SetSystemRestorePoint (HWND hwndDlg, BOOL finalize)
 *	\brief Restore the system
 *	\param[in] BOOL finalize
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void DoUninstall (void *arg)
 *	\brief Make the uninstall Of GostCrypt
 *	\param[in] void * arg
 *	\return void
 *
 */
...
/**
 *
 *	\fn void DoInstall (void *arg)
 *	\brief Make the installation of GostCrypt
 *	\param[in] void * arg
 *	\return void
 *
 */
...
/**
 *
 *	\fn void SetInstallationPath (HWND hwndDlg)
 *	\brief Set the path to the installation
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK UninstallDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Handler for uninstall only (install is handled by the wizard)
 *			Uninstall the dialogue window
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 case WM_INITDIALOG
 *			1 case WM_SYSCOMMAND if (lw == IDC_ABOUT)
 *			0 case WM_SYSCOMMAND else
 *			1 case WM_COMMAND if (bDone && (lw == IDC_UNINSTALL))
 *			1 case WM_COMMAND if (lw == IDC_UNINSTALL)
 *			1 case WM_COMMAND if (lw == IDC_SYSTEM_RESTORE)
 *			1 case WM_COMMAND if (lw == IDCANCEL)
 *			0 case WM_COMMAND else
 *			1 case GST_APPMSG_UNINSTALL_SUCCESS
 *			1 case WM_CLOSE if (bUninstallInProgres && AskNoYes("CONFIRM_EXIT_UNIVERSAL") == IDNO)
 *			1 case WM_CLOSE else
 *			0 else
 *
 */
 ...
 *	\brief Create Read arguments and launch the action requested (install, uninstall, repair), launch the window
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, char *lpszCommandLine, int nCmdShow)
{
	...
	InitCommonControls (); //obsolete, initialize windows basic controls
