 ...
 *
 *	\file SelfExtract.c
 *	\brief The extractor for the executable files.
 *	\version 1.3.1
 *	\date 14/13/2016
 *
 */
...
/**
 *
 *	\fn void SelfExtractStartupInit (void)
 *	\brief Call a deobfuscation function
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void DeobfuscateMagEndMarker (void)
 *	\brief The end marker must be included in the self-extracting exe only once, not twice (used e.g.
...
/**
 *
 *	\fn static void PkgError (char *msg)
...
/**
 *
 *	\fn static void PkgWarning (char *msg)
 *	\brief Displays a error dialog box
 *	\param[in] char * msg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void PkgInfo (char *msg)
 *	\brief Display an information dialog box
 *	\param[in] char * msg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static int DecompressBuffer (char *out, char *in, int len)
 *	\brief Returns 0 if decompression fails or, if successful, returns the size of the decompressed data
 *	\param[in] int len
 *	\param[in] char * in
 *	\param[in] char * out
 *	\return static int (DecompressDeflatedData (out, in, len))
 *
 */
...
/**
 *
 *	\fn static void __cdecl PipeWriteThread (void *len)
 *	\brief Send data to the pipe (STDIN)
 *	\param[in] void * len
...
/**
 *
 *	\fn static int CompressBuffer (char *out, char *in, int len)
 *	\brief Compress buffer and return its size
 *			Return 0 if compression failed, else return its length
 *	\param[in] int len
 *	\param[in] char * in
 *	\param[in] char * out
 *	\return static int 0 if c(!CreatePipe (&hChildStdoutRead, &hChildStdoutWrite, &securityAttrib, 0))
 *			0 if (!CreatePipe (&hChildStdinRead, &((HANDLE) hChildStdinWrite), &securityAttrib, 0))
 *			0 if (!CreateProcess (NULL, "gzip --best", NULL, NULL, TRUE, 0, NULL, NULL, &startupInfo, &procInfo))
 *			0 if (!CloseHandle (hChildStdoutWrite))
 *			res_len - 8 else
 *
 */
...
/**
 *
 *	\fn static void WipeSignatureAreas (char *buffer)
 *	\brief Clears all bytes that change when an exe file is digitally signed, except the data that are appended.
...
/**
 *
 *	\fn BOOL MakeSelfExtractingPackage (HWND hwndDlg, char *szDestDir)
 *	\brief Make a compressed extracted package to launch the program, in the folder szDestDir and named thanks to GetModuleFileName
 *			szDestDir is the directories that contains setup files then load them
 *	\param[in] char * szDestDir
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if the package has been extracted successfully
...
BOOL MakeSelfExtractingPackage (HWND hwndDlg, char *szDestDir)
{
	...
	x = strlen (szDestDir); 
	if (x < 2) //can't be installed in C: (or other path too short)
		goto err;

	if (szDestDir[x - 1] != '\\') // adding the \ at the end if missing
		strcat (szDestDir, "\\");

	GetModuleFileName (NULL, inputFile, sizeof (inputFile));

	strcpy (outputFile, szDestDir);
	strncat (outputFile, OutputPackageFile, sizeof (outputFile) - strlen (outputFile) - 1); // add the package name at the end of outputfile (destination)
	...
	// Compute and write CRC-32 hash of the entire package
	{
		...
		if (tmpBuffer == NULL)
		{
			handleWin32Error (hwndDlg);
			remove (outputFile);
			bePkgError ("Cannot load the package to compute CRC");
			goto err;
		}
...
/**
 *
 *	\fn BOOL VerifyPackageIntegrity (void)
 *	\brief Verifies the CRC-32 of the whole self-extracting package (except the digital signature areas, if present)
 *	\return BOOL FALSE if (fileDataEndPos < 0)
 *			FALSE if (fileDataStartPos < 0)
 *			FALSE if (!LoadInt32 (path, &crc, fileDataEndPos + strlen (MagEndMarker) + 1))
 *			FALSE if (tmpBuffer == NULL)
 *			FALSE if (crc != GetCrc32 (tmpBuffer, fileDataEndPos + 1 + strlen (MagEndMarker)))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL IsSelfExtractingPackage (void)
 *	\brief Determines whether we are a self-extracting package
 *	\return BOOL (FindStringInFile (path, MagEndMarker, strlen (MagEndMarker)) != -1)
 *
 */
...
/**
 *
 *	\fn static void FreeAllFileBuffers (void)
 *	\brief Free all the file buffers
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL SelfExtractInMemory (char *path)
 *	\brief Assumes that VerifyPackageIntegrity() has been used. Returns TRUE, if successful (otherwise FALSE).
 ...
 *	\return BOOL FALSE if (fileDataEndPos < 0)
 *			FALSE if (fileDataStartPos < 0)
 *			FALSE if (!LoadInt32 (path, &uncompressedLen, filePos))
 *			FALSE if (!LoadInt32 (path, &compressedLen, filePos))
 *			FALSE if (compressedLen != fileDataEndPos - fileDataStartPos - 8 + 1)
 *			FALSE if (DecompressedData == NULL)
 *			FALSE if (compressedData == NULL
 *			FALSE if (DecompressBuffer (DecompressedData, compressedData, compressedLen) != uncompressedLen)
 *			FALSE if (Decompressed_Files[fileNo].crc != GetCrc32 (Decompressed_Files[fileNo].fileContent, Decompressed_Files[fileNo].fileLength))
 *			FALSE if (fileNo < NBR_COMPRESSED_FILES)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn void __cdecl ExtractAllFilesThread (void *hwndDlg)
 *	\brief This function (that seems called as a thread) to extract the executable files, send a message if successed or failed.
 *	\param[in] void * hwndDlg
 *	\return void __cdecl
 *
 */
