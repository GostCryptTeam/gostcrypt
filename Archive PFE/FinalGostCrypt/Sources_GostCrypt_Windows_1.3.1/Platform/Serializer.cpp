/*
 Copyright (c) 2008 TrueCrypt Developers Association. All rights reserved.

 Governed by the TrueCrypt License 3.0 the full text of which is contained in
 the file License.txt included in TrueCrypt binary and source code distribution
 packages.
*/

/**
*
*	\file Serializer.cpp
*	\brief
*	\version
*	\date
*
*/

#include "Exception.h"
#include "ForEach.h"
#include "Memory.h"
#include "Serializer.h"

namespace GostCrypt
{
	template <typename T>
/**
 *
 *	\fn T Serializer::Deserialize ()
 *	\brief
 *	\return
 *
 */
	T Serializer::Deserialize ()
	{
		uint64 size;
		DataStream->ReadCompleteBuffer (BufferPtr ((byte *) &size, sizeof (size)));
		
		if (Endian::Big (size) != sizeof (T))
			throw ParameterIncorrect (SRC_POS);

		T data;
		DataStream->ReadCompleteBuffer (BufferPtr ((byte *) &data, sizeof (data)));

		return Endian::Big (data);
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, bool &data)
 *	\brief
 *	\param[in] bool &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, bool &data)
	{
		ValidateName (name);
		data = Deserialize <byte> () == 1;
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, byte &data)
 *	\brief
 *	\param[in] byte &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, byte &data)
	{
		ValidateName (name);
		data = Deserialize <byte> ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, int32 &data)
 *	\brief
 *	\param[in] int32 &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, int32 &data)
	{
		ValidateName (name);
		data = (int32) Deserialize <uint32> ();
	}
	
/**
 *
 *	\fn void Serializer::Deserialize (const string &name, int64 &data)
 *	\brief
 *	\param[in] int64 &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, int64 &data)
	{
		ValidateName (name);
		data = (int64) Deserialize <uint64> ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, uint32 &data)
 *	\brief
 *	\param[in] uint32 &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, uint32 &data)
	{
		ValidateName (name);
		data = Deserialize <uint32> ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, uint64 &data)
 *	\brief
 *	\param[in] uint64 &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, uint64 &data)
	{
		ValidateName (name);
		data = Deserialize <uint64> ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, string &data)
 *	\brief
 *	\param[in] string &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, string &data)
	{
		ValidateName (name);
		data = DeserializeString ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, wstring &data)
 *	\brief
 *	\param[in] wstring &data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, wstring &data)
	{
		ValidateName (name);
		data = DeserializeWString ();
	}

/**
 *
 *	\fn void Serializer::Deserialize (const string &name, const BufferPtr &data)
 *	\brief
 *	\param[in] const BufferPtr
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Deserialize (const string &name, const BufferPtr &data)
	{
		ValidateName (name);

		uint64 size = Deserialize <uint64> ();
		if (data.Size() != size)
			throw ParameterIncorrect (SRC_POS);

		DataStream->ReadCompleteBuffer (data);
	}

/**
 *
 *	\fn bool Serializer::DeserializeBool (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	bool Serializer::DeserializeBool (const string &name)
	{
		bool data;
		Deserialize (name, data);
		return data;
	}

/**
 *
 *	\fn int32 Serializer::DeserializeInt32 (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	int32 Serializer::DeserializeInt32 (const string &name)
	{
		ValidateName (name);
		return Deserialize <uint32> ();
	}

/**
 *
 *	\fn int64 Serializer::DeserializeInt64 (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	int64 Serializer::DeserializeInt64 (const string &name)
	{
		ValidateName (name);
		return Deserialize <uint64> ();
	}

/**
 *
 *	\fn uint32 Serializer::DeserializeUInt32 (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	uint32 Serializer::DeserializeUInt32 (const string &name)
	{
		ValidateName (name);
		return Deserialize <uint32> ();
	}

/**
 *
 *	\fn uint64 Serializer::DeserializeUInt64 (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	uint64 Serializer::DeserializeUInt64 (const string &name)
	{
		ValidateName (name);
		return Deserialize <uint64> ();
	}

/**
 *
 *	\fn string Serializer::DeserializeString ()
 *	\brief
 *	\return
 *
 */
	string Serializer::DeserializeString ()
	{
		uint64 size = Deserialize <uint64> ();

		vector <char> data ((size_t) size);
		DataStream->ReadCompleteBuffer (BufferPtr ((byte *) &data[0], (size_t) size));

		return string (&data[0]);
	}

/**
 *
 *	\fn string Serializer::DeserializeString (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	string Serializer::DeserializeString (const string &name)
	{
		ValidateName (name);
		return DeserializeString ();
	}

/**
 *
 *	\fn list <string> Serializer::DeserializeStringList (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	list <string> Serializer::DeserializeStringList (const string &name)
	{
		ValidateName (name);
		list <string> deserializedList;
		uint64 listSize = Deserialize <uint64> ();

		for (size_t i = 0; i < listSize; i++)
			deserializedList.push_back (DeserializeString ());

		return deserializedList;
	}

/**
 *
 *	\fn wstring Serializer::DeserializeWString ()
 *	\brief
 *	\return
 *
 */
	wstring Serializer::DeserializeWString ()
	{
		uint64 size = Deserialize <uint64> ();

		vector <wchar_t> data ((size_t) size / sizeof (wchar_t));
		DataStream->ReadCompleteBuffer (BufferPtr ((byte *) &data[0], (size_t) size));

		return wstring (&data[0]);
	}

/**
 *
 *	\fn list <wstring> Serializer::DeserializeWStringList (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	list <wstring> Serializer::DeserializeWStringList (const string &name)
	{
		ValidateName (name);
		list <wstring> deserializedList;
		uint64 listSize = Deserialize <uint64> ();

		for (size_t i = 0; i < listSize; i++)
			deserializedList.push_back (DeserializeWString ());

		return deserializedList;
	}

/**
 *
 *	\fn wstring Serializer::DeserializeWString (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	wstring Serializer::DeserializeWString (const string &name)
	{
		ValidateName (name);
		return DeserializeWString ();
	}

	template <typename T>
/**
 *
 *	\fn void Serializer::Serialize (T data)
 *	\brief
 *	\param[in] T data)
 *	\return
 *
 */
	void Serializer::Serialize (T data)
	{
		uint64 size = Endian::Big (uint64 (sizeof (data)));
		DataStream->Write (ConstBufferPtr ((byte *) &size, sizeof (size)));

		data = Endian::Big (data);
		DataStream->Write (ConstBufferPtr ((byte *) &data, sizeof (data)));
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, bool data)
 *	\brief
 *	\param[in] bool data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, bool data)
	{
		SerializeString (name);
		byte d = data ? 1 : 0;
		Serialize (d);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, byte data)
 *	\brief
 *	\param[in] byte data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, byte data)
	{
		SerializeString (name);
		Serialize (data);
	}
	
/**
 *
 *	\fn void Serializer::Serialize (const string &name, const char *data)
 *	\brief
 *	\param[in] const char
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const char *data)
	{
		Serialize (name, string (data));
	}
	
/**
 *
 *	\fn void Serializer::Serialize (const string &name, int32 data)
 *	\brief
 *	\param[in] int32 data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, int32 data)
	{
		SerializeString (name);
		Serialize ((uint32) data);
	}
		
/**
 *
 *	\fn void Serializer::Serialize (const string &name, int64 data)
 *	\brief
 *	\param[in] int64 data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, int64 data)
	{
		SerializeString (name);
		Serialize ((uint64) data);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, uint32 data)
 *	\brief
 *	\param[in] uint32 data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, uint32 data)
	{
		SerializeString (name);
		Serialize (data);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, uint64 data)
 *	\brief
 *	\param[in] uint64 data)
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, uint64 data)
	{
		SerializeString (name);
		Serialize (data);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, const string &data)
 *	\brief
 *	\param[in] const string
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const string &data)
	{
		SerializeString (name);
		SerializeString (data);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, const wchar_t *data)
 *	\brief
 *	\param[in] const wchar_t
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const wchar_t *data)
	{
		Serialize (name, wstring (data));
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, const wstring &data)
 *	\brief
 *	\param[in] const wstring
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const wstring &data)
	{
		SerializeString (name);
		SerializeWString (data);
	}
	
/**
 *
 *	\fn void Serializer::Serialize (const string &name, const list <string> &stringList)
 *	\brief
 *	\param[in] const list
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const list <string> &stringList)
	{
		SerializeString (name);
		
		uint64 listSize = stringList.size();
		Serialize (listSize);

		foreach (const string &item, stringList)
			SerializeString (item);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, const list <wstring> &stringList)
 *	\brief
 *	\param[in] const list
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const list <wstring> &stringList)
	{
		SerializeString (name);
		
		uint64 listSize = stringList.size();
		Serialize (listSize);

		foreach (const wstring &item, stringList)
			SerializeWString (item);
	}

/**
 *
 *	\fn void Serializer::Serialize (const string &name, const ConstBufferPtr &data)
 *	\brief
 *	\param[in] const ConstBufferPtr
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::Serialize (const string &name, const ConstBufferPtr &data)
	{
		SerializeString (name);

		uint64 size = data.Size();
		Serialize (size);

		DataStream->Write (data);
	}

/**
 *
 *	\fn void Serializer::SerializeString (const string &data)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::SerializeString (const string &data)
	{
		Serialize ((uint64) data.size() + 1);
		DataStream->Write (ConstBufferPtr ((byte *) (data.data() ? data.data() : data.c_str()), data.size() + 1));
	}

/**
 *
 *	\fn void Serializer::SerializeWString (const wstring &data)
 *	\brief
 *	\param[in] const wstring
 *	\return
 *
 */
	void Serializer::SerializeWString (const wstring &data)
	{
		uint64 size = (data.size() + 1) * sizeof (wchar_t);
		Serialize (size);
		DataStream->Write (ConstBufferPtr ((byte *) (data.data() ? data.data() : data.c_str()), (size_t) size));
	}

/**
 *
 *	\fn void Serializer::ValidateName (const string &name)
 *	\brief
 *	\param[in] const string
 *	\return
 *
 */
	void Serializer::ValidateName (const string &name)
	{
		string dName = DeserializeString();
		if (dName != name)
		{
			throw ParameterIncorrect (SRC_POS);
		}
	}
}
