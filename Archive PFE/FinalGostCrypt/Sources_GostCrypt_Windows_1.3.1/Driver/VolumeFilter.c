 ...
 *
 *	\file VolumeFilter.c
 *	\brief Order the irp, device filters received and manage the power for each device in order to send informations to the right device.
 *			Manage the queue lock for its own functions.
 *	\version 1.3.1
 *	\date 13/12/2016
 *
 */
...
/**
 *
 *	\fn NTSTATUS VolumeFilterAddDevice (PDRIVER_OBJECT driverObject, PDEVICE_OBJECT pdo)
 *	\brief Create VolumeFilterextension Extension and put into the devices in argument and the filter device.
 ...
 *	\brief Go to the next irp call and return it.
 *	\param[in] PIRP irp
 ...
 *	\brief Copies the IRP stack parameters from the current I/O stack location to the stack location of the next-lower driver. Complete the routine
...
/**
 *
 *	\fn static NTSTATUS OnDeviceUsageNotificationCompleted (PDEVICE_OBJECT filterDeviceObject, PIRP Irp, VolumeFilterExtension *Extension)
 *	\brief Remove the lock and return that a device completed its tasks.
 *	\param[in] VolumeFilterExtension * Extension
 *	\param[in] PIRP Irp
...
/**
 *
 *	\fn static NTSTATUS OnStartDeviceCompleted (PDEVICE_OBJECT filterDeviceObject, PIRP Irp, VolumeFilterExtension *Extension)
 *	\brief Return when the device completed
 *	\param[in] VolumeFilterExtension * Extension
 *	\param[in] PIRP Irp
 *	\param[in] PDEVICE_OBJECT filterDeviceObject
 *	\return static NTSTATUS STATUS_CONTINUE_COMPLETION
...
/**
 *
 *	\fn static NTSTATUS DispatchControl (PDEVICE_OBJECT DeviceObject, PIRP Irp, VolumeFilterExtension *Extension, PIO_STACK_LOCATION irpSp)
 *	\brief Associate the control commands to the right actions.
 *	\param[in] PIO_STACK_LOCATION irpSp
 *	\param[in] VolumeFilterExtension * Extension
 *	\param[in] PIRP Irp
 *	\param[in] PDEVICE_OBJECT DeviceObject
 *	\return static NTSTATUS GSTCompleteDiskIrp (Irp, STATUS_SUCCESS, 0)
 *			status else
 *
 */
...
/**
 *
 *	\fn static NTSTATUS DispatchPnp (PDEVICE_OBJECT DeviceObject, PIRP Irp, VolumeFilterExtension *Extension, PIO_STACK_LOCATION irpSp)
 *	\brief Dispatch irp minor function to the associated actions
 *	\param[in] PIO_STACK_LOCATION irpSp
 *	\param[in] VolumeFilterExtension * Extension
 *	\param[in] PIRP Irp
 *	\param[in] PDEVICE_OBJECT DeviceObject
...
/**
 *
 *	\fn static NTSTATUS DispatchPower (PDEVICE_OBJECT DeviceObject, PIRP Irp, VolumeFilterExtension *Extension, PIO_STACK_LOCATION irpSp)
 *	\brief Dispatch power between the devices.
 *	\param[in] PIO_STACK_LOCATION irpSp
 *	\param[in] VolumeFilterExtension * Extension
 *	\param[in] PIRP Irp
 *	\param[in] PDEVICE_OBJECT DeviceObject
...
/**
 *
 *	\fn NTSTATUS VolumeFilterDispatchIrp (PDEVICE_OBJECT DeviceObject, PIRP Irp)
 *	\brief Send irp to the right dispatcher depending on its value
 *	\param[in] PIRP Irp
 *	\param[in] PDEVICE_OBJECT DeviceObject
 *	\return NTSTATUS GSTCompleteIrp (Irp, status, 0)
 *			status else
 *
 */
