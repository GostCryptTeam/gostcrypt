 ...
 *
 *	\file DumpFilter.c
 *	\brief Define dump filter functions
 *	\version 1.3.1
 *	\date 13/12/2016
 *
 */
...
/**
 *
 *	\fn NTSTATUS DumpFilterEntry (PFILTER_EXTENSION filterExtension, PFILTER_INITIALIZATION_DATA filterInitData)
 *	\brief Check the driver version of the main device get his dump configuration.
 *		Verify that it is located on the system drive and system encryption.
 *	\param[in] PFILTER_INITIALIZATION_DATA filterInitData
 *	\param[in] PFILTER_EXTENSION filterExtension
 *	\return NTSTATUS status if error
 *			STATUS_SUCCESS if not
 *
 */
...
/**
 *
 *	\fn static NTSTATUS DumpFilterStart (PFILTER_EXTENSION filterExtension)
 *	\brief Start the dump filter
 *	\param[in] PFILTER_EXTENSION filterExtension
 *	\return static NTSTATUS BootDriveFilterExtension->DriveMounted ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL
 *
 */
...
/**
 *
 *	\fn static NTSTATUS DumpFilterWrite (PFILTER_EXTENSION filterExtension, PLARGE_INTEGER diskWriteOffset, PMDL writeMdl)
 *	\brief Write in the dump filter
 *	\param[in] PMDL writeMdl
 *	\param[in] PLARGE_INTEGER diskWriteOffset
 *	\param[in] PFILTER_EXTENSION filterExtension
 *	\return static NTSTATUS STATUS_SUCCESS
 *
 */
...
/**
 *
 *	\fn static NTSTATUS DumpFilterFinish (PFILTER_EXTENSION filterExtension)
 *	\brief Return a success when the dump filter finished
 *	\param[in] PFILTER_EXTENSION filterExtension
 *	\return static NTSTATUS STATUS_SUCCESS
 *
 */
...
/**
 *
 *	\fn static NTSTATUS DumpFilterUnload (PFILTER_EXTENSION filterExtension)
 *	\brief Unload the dump filter
 *	\param[in] PFILTER_EXTENSION filterExtension
 *	\return static NTSTATUS STATUS_SUCCESS
 *
 */
