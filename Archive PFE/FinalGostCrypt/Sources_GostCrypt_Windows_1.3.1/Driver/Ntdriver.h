...
#ifndef GST_HEADER_NTDRIVER
#define GST_HEADER_NTDRIVER
...
typedef struct _THREAD_BLOCK_
{
	...
	WCHAR wszMountVolume[GST_MAX_PATH + 8];
	MOUNT_STRUCT *mount;
...
typedef struct EXTENSION
{
	...
	WCHAR wszVolume[GST_MAX_PATH];	/*  DONT change this size without also changing MOUNT_LIST_STRUCT! */
...
extern PDRIVER_OBJECT GSTDriverObject;
...
NTSTATUS GSTCreateDeviceObject (PDRIVER_OBJECT DriverObject, PDEVICE_OBJECT * ppDeviceObject, MOUNT_STRUCT * mount);
...
NTSTATUS GSTFsctlCall (PFILE_OBJECT fileObject, LONG IoControlCode, void *InputBuffer, int InputBufferSize, void *OutputBuffer, int OutputBufferSize);
...
#define GST_BUG_CHECK(status) KeBugCheckEx (SECURITY_SYSTEM, __LINE__, (ULONG_PTR) status, 0, 'GST')

#endif // GST_HEADER_NTDRIVER
