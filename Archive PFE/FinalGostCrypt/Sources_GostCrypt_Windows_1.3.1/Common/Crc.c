 ...
 *
 *	\file Crc.c
 *	\brief Manage functions that returns the CRC
 *	\version 1.3.1
 *	\date 2008	
 *
 */
...
/**
 *
 *	\fn unsigned __int32 GetCrc32 (unsigned char *data, int length)
 *	\brief Get the Crc, there are bit shift in a while loop
 *	\param[in] int length
...
/**
 *
 *	\fn unsigned __int32 crc32int (unsigned __int32 *data)
 *	\brief Apply some calculs on the CRC
 *	\param[in] unsigned __int32 * data
 *	\return unsigned __int32 (CRC >> 8) ^ crc_32_tab[ (CRC ^ *d) & 0xFF ] ^ 0xffffffff
 *
 */
...
/**
 *
 *	\fn BOOL crc32_selftests (void)
 *	\brief Test if the crc32 is good or not
 *	\return BOOL bSuccess if == CRC_SELFTEST
 *			FALSE otherwise
 *
 */
 ...
 *
 *	\fn unsigned __int32 GetCrc32 (unsigned char *data, int length)
 *	\brief redefinition of the GetCrc32 in iterative mode
 *	\param[in] int length
 *	\param[in] unsigned char * data
...
/**
 *
 *	\fn BOOL crc32_selftests ()
 *	\brief redefinition of the crc32_selftests in iterative mode
 *	\return BOOL
 *
 */
