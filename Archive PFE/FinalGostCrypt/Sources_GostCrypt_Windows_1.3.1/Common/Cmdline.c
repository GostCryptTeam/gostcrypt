 ...
 *
 *	\file Cmdline.c
 *	\brief Manage command line functions
 *	\version 1.3.1
 *	\date 19/01/2017
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK CommandHelpDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 ...
 * 			1 switch msg case WM_INITDIALOG
 * 			1 switch msg case WM_INITDIALOG
 *			0 else
 *
 */
...
/**
 *
 *	\fn int Win32CommandLine (char *lpszCommandLine, char ***lpszArgs)
 *	\brief Manage win32 command line
 *	\param[in] char *** lpszArgs
 *	\param[in] char * lpszCommandLine
 *	\return int 0 if !arguments
 *			0 if argumentCount < 1
 *			argumentCount else
 *
 */
...
/**
 *
 *	\fn int GetArgSepPosOffset (char *lpszArgument)
 *	\brief Return 1 if an there is an separator on first char
 *	\param[in] char * lpszArgument
 *	\return int 1 if lpszArgument[0] == '/'
 *			0 else
 *
 */
...
/**
 *
 *	\fn int GetArgumentID (argumentspec *as, char *lpszArgument, int *nArgPos)
 *	\brief Get the argument ID
 *	\param[in] int * nArgPos
 *	\param[in] char * lpszArgument
 *	\param[in] argumentspec * as
 *	\return int as->args[i].Id if x == as->arg_cnt
 *			-1 else
 *
 */
...
/**
 *
 *	\fn int GetArgumentValue (char **lpszCommandLineArgs, int nArgPos, int *nArgIdx, int nNoCommandLineArgs, char *lpszValue, int nValueSize)
 *	\brief Get the argument value
 *	\param[in] int nValueSize
 *	\param[in] char * lpszValue
 *	\param[in] int nNoCommandLineArgs
 *	\param[in] int * nArgIdx
 *	\param[in] int nArgPos
 *	\param[in] char ** lpszCommandLineArgs
 *	\return int HAS_ARGUMENT if nArgPos
 *			HAS_ARGUMENT if (*nArgIdx + 1 < nNoCommandLineArgs)
 *			HAS_NO_ARGUMENT else
 *
 */
