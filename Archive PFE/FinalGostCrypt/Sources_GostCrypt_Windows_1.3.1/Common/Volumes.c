 ...
 *
 *	\file Volumes.c
 *	\brief Manage the generation and use of header of volumes.
 *	\version 1.3.1
 *	\date 20/01/2017
 *
 */
...
/**
 *
 *	\fn uint16 GetHeaderField16 (byte *header, int offset)
 *	\brief Getter for 16 bits value in a header, starting at the offset.
 *	\param[in] int offset
 *	\param[in] byte * header
 *	\return uint16 BE16 (*(uint16 *) (header + offset))
 *
 */
...
/**
 *
 *	\fn uint32 GetHeaderField32 (byte *header, int offset)
 *	\brief Getter for 32 bits value in a header, starting at the offset.
 *	\param[in] int offset
 *	\param[in] byte * header
...
/**
 *
 *	\fn UINT64_STRUCT GetHeaderField64 (byte *header, int offset)
 *	\brief Getter for 64 bits value in a header, starting at the offset.
 *	\param[in] int offset
 *	\param[in] byte * header
 *	\return UINT64_STRUCT uint64Struct
 *
 */
...
/**
 *
 *	\fn int ReadVolumeHeader (BOOL bBoot, char *encryptedHeader, Password *password, PCRYPTO_INFO *retInfo, CRYPTO_INFO *retHeaderCryptoInfo)
 *	\brief Read the volume header using getter from 16 to 64 bits. It permits to load the volume with its parameters.
 *	\param[in] CRYPTO_INFO * retHeaderCryptoInfo
 *	\param[in] PCRYPTO_INFO * retInfo
 *	\param[in] Password * password
 *	\param[in] char * encryptedHeader
 *	\param[in] BOOL bBoot
 *	\return int ERR_OUTOFMEMORY if (encryptionThreadCount > 1)
 *			ERR_OUTOFMEMORY else (retHeaderCryptoInfo != NULL)
 *			ERR_OUTOFMEMORY if (!keyDerivationCompletedEvent)
 *			ERR_OUROFMEMORY if (!noOutstandingWorkItemEvent)
 *			ERR_PARAMETER_INCORRECT if (cryptoInfo->SectorSize < GST_MIN_VOLUME_SECTOR_SIZE || cryptoInfo->SectorSize > GST_MAX_VOLUME_SECTOR_SIZE || cryptoInfo->SectorSize % ENCRYPTION_DATA_UNIT_SIZE != 0)
 *			ERR_MODE_INIT_FAILED if (!EAInitMode (cryptoInfo))
 *			ERR_NEW_VERSION_REQUIRED if (headerVersion > VOLUME_HEADER_VERSION)
 *			ERR_NEW_VERSION_REQUIRED if (cryptoInfo->RequiredProgramVersion > VERSION_NUM)
 *			ERR_CIPHER_INIT_FAILURE if (status == ERR_CIPHER_INIT_FAILURE)
 *			ERR_MODE_INIT_FAILED if (!EAInitMode (cryptoInfo))
 *			status else
 *
 */
...
/**
 *
 *	\fn static char itoc_(unsigned char input)
 *	\brief Transform an unsigned char to char
 *	\param[in] unsigned char input
 *	\return (char)input + 0x30 if (input >= 0x0 && input <= 0x9)
 *			(char)input + 0x61 - 0xA else if (input >= 0xA && input <= 0xF)
 *			'?' else
 */
...
/**
 *
 *	\fn int ReadVolumeHeader (BOOL bBoot, char *header, Password *password, PCRYPTO_INFO *retInfo, CRYPTO_INFO *retHeaderCryptoInfo)
 *	\brief Decrypt the header thanks to the password entered saved using all the algorithm available.
 *	\param[in] CRYPTO_INFO * retHeaderCryptoInfo
 *	\param[in] PCRYPTO_INFO * retInfo
 *	\param[in] Password * password
 *	\param[in] char * header
 *	\param[in] BOOL bBoot
 *	\return int ERR_CIPHER_INIT_FAILURE if (status == ERR_CIPHER_INIT_FAILURE)
 *			ERR_CIPHER_INIT_FAILUREif (status == ERR_CIPHER_INIT_FAILURE)
 *			status else
 *
 */
...
/**
 *
 *	\fn int CreateVolumeHeaderInMemory (BOOL bBoot, char *header, int ea, int mode, Password *password, int pkcs5_prf, char *masterKeydata, PCRYPTO_INFO *retInfo,  unsigned __int64 volumeSize, unsigned __int64 hiddenVolumeSize,  unsigned __int64 encryptedAreaStart, unsigned __int64 encryptedAreaLength, uint16 requiredProgramVersion, uint32 headerFlags, uint32 sectorSize, BOOL bWipeMode)
 *	\brief Creates a volume header in memory
 *	\param[in] BOOL bBoot 
 *	\param[in] char * header
 *	\param[in] int ea
 *	\param[in] int mode
 *	\param[in] Password * password
 *	\param[in] int pkcs5_prf
 *	\param[in] char * masterKeydata
 *	\param[in] PCRYPTO_INFO * retInfo
 *	\param[in] unsigned __int64 volumeSize
 *	\param[in] unsigned __int64 hiddenVolumeSize
 *	\param[in] unsigned __int64 encryptedAreaStart
 *	\param[in] unsigned __int64 encryptedAreaLength
 *	\param[in] uint16 requiredProgramVersion
 *	\param[in] uint32 headerFlags
 *	\param[in] uint32 sectorSize
 *	\param[in] BOOL bWipeMode
 *	\return int ERR_OUTOFMEMORY if (cryptoInfo == NULL)
 *			retval if (retVal != ERR_SUCCESS)
 *			ERR_CIPHER_INIT_WEAK_KEY if (!RandgetBytes (keyInfo.salt, PKCS5_SALT_SIZE, !bWipeMode))
 *			ERR_CIPHER_INIT_WEAK_KEY if (!RandgetBytes (keyInfo.master_keydata, bytesNeeded, TRUE))
 *			ERR_OUTOFMEMORY if (!EAInitMode (cryptoInfo))
 *			0 else
 *
 *
 */
...
/**
 *
 *	\fn BOOL ReadEffectiveVolumeHeader (BOOL device, HANDLE fileHandle, byte *header, DWORD *bytesRead)
 *	\brief Read the effective volume header
 *	\param[in] DWORD * bytesRead
 *	\param[in] byte * header
 *	\param[in] HANDLE fileHandle
 *	\param[in] BOOL device
 *	\return BOOL FALSE if (!DeviceIoControl (fileHandle, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &geometry, sizeof (geometry), bytesRead, NULL))
 *			FALSE if (geometry.BytesPerSector > sizeof (sectorBuffer) || geometry.BytesPerSector < GST_MIN_VOLUME_SECTOR_SIZE)
 *			FALSE if (!ReadFile (fileHandle, sectorBuffer, max (GST_VOLUME_HEADER_EFFECTIVE_SIZE, geometry.BytesPerSector), bytesRead, NULL))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL WriteEffectiveVolumeHeader (BOOL device, HANDLE fileHandle, byte *header)
 *	\brief Write the header on the device passed in parameter
 *	\param[in] byte * header
 *	\param[in] HANDLE fileHandle
 *	\param[in] BOOL device
 *	\return BOOL TRUE if (!device)
 *			FALSE if (!WriteFile (fileHandle, header, GST_VOLUME_HEADER_EFFECTIVE_SIZE, &bytesDone, NULL))
 *			FALSE if (bytesDone != GST_VOLUME_HEADER_EFFECTIVE_SIZE)
 *			FALSE if (!DeviceIoControl (fileHandle, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &geometry, sizeof (geometry), &bytesDone, NULL))
 *			FALSE if (geometry.BytesPerSector > sizeof (sectorBuffer) || geometry.BytesPerSector < GST_MIN_VOLUME_SECTOR_SIZE)
 *			FALSE if (!ReadFile (fileHandle, sectorBuffer, geometry.BytesPerSector, &bytesDone, NULL) && (geometry.BytesPerSector != GST_VOLUME_HEADER_EFFECTIVE_SIZE))
 *			FALSE if (bytesDone != geometry.BytesPerSector && (geometry.BytesPerSector != GST_VOLUME_HEADER_EFFECTIVE_SIZE))
 *			FALSE if (!SetFilePointerEx (fileHandle, seekOffset, NULL, FILE_CURRENT) && (geometry.BytesPerSector != GST_VOLUME_HEADER_EFFECTIVE_SIZE))
 *			FALSE if (!WriteFile (fileHandle, sectorBuffer, geometry.BytesPerSector, &bytesDone, NULL) && (geometry.BytesPerSector != GST_VOLUME_HEADER_EFFECTIVE_SIZE))
 *			FALSE if (bytesDone != geometry.BytesPerSector)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn int WriteRandomDataToReservedHeaderAreas (HANDLE dev, CRYPTO_INFO *cryptoInfo, uint64 dataAreaSize, BOOL bPrimaryOnly, BOOL bBackupOnly)
 *	\brief Writes randomly generated data to unused/reserved header areas.
 ...
 *	\param[in] BOOL bBackupOnly
 *	\param[in] BOOL bPrimaryOnly
 *	\param[in] uint64 dataAreaSize
 *	\param[in] CRYPTO_INFO * cryptoInfo
 *	\param[in] HANDLE dev
 *	\return int ERR_PARAMETER_INCORRECT if (!RandgetBytes (temporaryKey, EAGetKeySize (cryptoInfo->ea), FALSE) || !RandgetBytes (cryptoInfo->k2, sizeof (cryptoInfo->k2), FALSE))
 *			ERR_MODE_INIT_FAILED if (!EAInitMode (cryptoInfo))
 *			ERR_OS_ERROR if (!ReadFile (dev, buf, sizeof (buf), &bytesDone, NULL))
 *			ERR_OS_ERROR if (!SetFilePointerEx (dev, offset, NULL, FILE_BEGIN))
 *			ERR_OS_ERROR if (bytesDone < GST_VOLUME_HEADER_EFFECTIVE_SIZE)
 *			ERR_OS_ERROR if (!SetFilePointerEx (dev, offset, NULL, FILE_BEGIN))
 *			ERR_OS_ERROR if (bytesDone < GST_VOLUME_HEADER_EFFECTIVE_SIZE)
 *			ERR_PARAMETER_INCORRECT if (bytesDone != sizeof (buf))
 *			ERR_MODE_INIT_FAILED if (!EAInitMode (cryptoInfo))
 *			nStatus else
 *
 */
