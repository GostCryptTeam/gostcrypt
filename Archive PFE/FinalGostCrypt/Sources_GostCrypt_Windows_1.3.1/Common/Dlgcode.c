 ...
 *
 *	\file Dlgcode.c
 *	\brief Function that manage warning and error dialog proc, threads, help balloon interactions, drivers, file system... Some functions are used to work on volumes and partitions. There is also some functions to launch a benchmark.
 *	\version 1.3.1
 *	\date 20/01/2017
 *
 */
 ...
 *
 *	\fn void cleanup ()
 *	\brief Make a clean up of GDI/dialog and close device driver handle
 *	\return void
 *
 */
void cleanup ()
...
/**
 *
 *	\fn void LowerCaseCopy (char *lpszDest, const char *lpszSource)
 *	\brief Copy string to lower case
 *	\param[in] const char * lpszSource
 *	\param[in] char * lpszDest
 *	\return void
 *
 */
...
/**
 *
 *	\fn void UpperCaseCopy (char *lpszDest, const char *lpszSource)
 *	\brief Copy string to upper case
 *	\param[in] const char * lpszSource
 *	\param[in] char * lpszDest
 *	\return void
 *
...
/**
 *
 *	\fn BOOL IsVolumeDeviceHosted (const char *lpszDiskFile)
 *	\brief Is the volume device hosted ?
 *	\param[in] const char * lpszDiskFile
 *	\return BOOL strstr (lpszDiskFile, "\\Device\\") == lpszDiskFile || strstr (lpszDiskFile, "\\DEVICE\\") == lpszDiskFile
 *
 */
...
/**
 *
 *	\fn void CreateFullVolumePath (char *lpszDiskFile, const char *lpszFileName, BOOL * bDevice)
 *	\brief Create the path to the created volume
 *	\param[in] BOOL * bDevice
 *	\param[in] const char
 *	\param[in] char * lpszDiskFile
 *	\return void
 *
 */
...
/**
 *
 *	\fn int FakeDosNameForDevice (const char *lpszDiskFile, char *lpszDosDevice, char *lpszCFDevice, BOOL bNameOnly)
 *	\brief Create a fake dos name for the device
 *	\param[in] BOOL bNameOnly
 *	\param[in] char* lpszCFDevice
 *	\param[in] char* lpszDosDevice
 *	\param[in] const char * lpszDiskFile
 *	\return int ERR_OS_ERROR if (bDosLinkCreated == FALSE)
 *			0 else
 *
 */
...
/**
 *
 *	\fn int RemoveFakeDosName (char *lpszDiskFile, char *lpszDosDevice)
 *	\brief Remove the fake dos name
 *	\param[in] char * lpszDosDevice
 *	\param[in] char * lpszDiskFile
 *	\return int ERR_OS_ERROR if (bDosLinkRemoved == FALSE)
 *			0 else
 *
 */
...
/**
 *
 *	\fn void AbortProcess (char *stringId)
 *	\brief Abort the process with a local clean up at exit
 *	\param[in] char* stringId
 *	\return void
 *
 */
...
/**
 *
 *	\fn void AbortProcessSilent (void)
 *	\brief Abort the process without message with a local clean up at exit
 *	\return void
 *
 */
...
/**
 *
 *	\fn void *err_malloc (size_t size)
 *	\brief Apply malloc managing errors
...
/**
 *
 *	\fn char *err_strdup (char *lpszText)
 *	\brief Apply a memmove on a string managing errors
 *	\param[in] char * lpszText
 *	\return char * z
 *
 */
...
/**
 *
 *	\fn BOOL IsDiskReadError (DWORD error)
 *	\brief Is there an error reading the disk
 *	\param[in] DWORD error
 *	\return BOOL (error == ERROR_CRC || error == ERROR_IO_DEVICE || error == ERROR_BAD_CLUSTERS || error == ERROR_SECTOR_NOT_FOUND || error == ERROR_READ_FAULT || error == ERROR_INVALID_FUNCTION // I/O error may be reported as ERROR_INVALID_FUNCTION by buggy chipset drivers || error == ERROR_SEM_TIMEOUT)
 *
 */
...
/**
 *
 *	\fn BOOL IsDiskWriteError (DWORD error)
 *	\brief Is there an error writing on the disk
 *	\param[in] DWORD error
 *	\return BOOL (error == ERROR_IO_DEVICE || error == ERROR_BAD_CLUSTERS || error == ERROR_SECTOR_NOT_FOUND || error == ERROR_WRITE_FAULT || error == ERROR_INVALID_FUNCTION // I/O error may be reported as ERROR_INVALID_FUNCTION by buggy chipset drivers || error == ERROR_SEM_TIMEOUT)
 *
 */
...
/**
 *
 *	\fn BOOL IsDiskError (DWORD error)
 *	\brief Is there an reading or writing error on the disk
 *	\param[in] DWORD error
...
/**
 *
 *	\fn DWORD handleWin32Error (HWND hwndDlg)
 *	\brief Handle windows32 errors
 *	\param[in] HWND hwndDlg
 *	\return DWORD dwError if (Silent || dwError == 0 || dwError == ERROR_INVALID_WINDOW_HANDLE)
 *			dwError if (dwError == ERROR_ACCESS_DENIED && !IsAdmin ())
 *			dwError else
 *
 */
...
/**
 *
 *	\fn BOOL translateWin32Error (wchar_t *lpszMsgBuf, int nWSizeOfBuf)
 *	\brief Translate the windows32 error
 *	\param[in] int nWSizeOfBuf
 *	\param[in] wchar_t * lpszMsgBuf
 *	\return BOOL TRUE if (FormatMessageW (FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError,MAKELANGID (LANG_NEUTRAL, SUBLANG_DEFAULT), lpszMsgBuf, nWSizeOfBuf, NULL))
 *			FALSE  else
 *
 */
...
/**
 *
 *	\fn int CompensateDPIFont (int val)
 *	\brief If the user has a non-default screen DPI, all absolute font sizes must be
 *			converted using this function.
 *	\param[in] int val
 *	\return int val if (ScreenDPI == USER_DEFAULT_SCREEN_DPI)
 *			floor(tmpVal) if (tmpVal > 0)
 *			ceil(tmpVal) else
 *
 */
...
/**
 *
 *	\fn int CompensateXDPI (int val)
 *	\brief If the user has a non-default screen DPI, some screen coordinates and sizes must
...
/**
 *
 *	\fn int GetTextGfxWidth (HWND hwndDlgItem, const wchar_t *text, HFONT hFont)
 *	\brief Get the Gfx text width
 *	\param[in] HFONT hFont
 *	\param[in] const wchar_t * text
 *	\param[in] HWND hwndDlgItem
 *	\return int ((int) sizes.cx - (int) textMetrics.tmOverhang)
 *
 */
...
/**
 *
 *	\fn int GetTextGfxHeight (HWND hwndDlgItem, const wchar_t *text, HFONT hFont)
 *	\brief Get the Gfx text height
 *	\param[in] HFONT hFont
 *	\param[in] const wchar_t * text
 *	\param[in] HWND hwndDlgItem
 *	\return int ((int) sizes.cy)
 *
 */
...
/**
 *
 *	\fn std::string FitPathInGfxWidth (HWND hwnd, HFONT hFont, LONG width, const std::string &path)
 *	\brief Fit the path in the Gfx width
...
/**
 *
 *	\fn static LRESULT CALLBACK HyperlinkProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
 *	\brief Passes message information to specified window procedure
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT message
 *	\param[in] HWND hwnd
 *	\return static LRESULT 0 switch (message) case WM_SETCURSOR
 *			0 switch (message) case WM_MOUSELEAVE
 *			CallWindowProc (wp, hwnd, message, wParam, lParam) else
 *
 */
...
/**
 *
 *	\fn BOOL ToHyperlink (HWND hwndDlg, UINT ctrlId)
 *	\brief Make a Hyper link
 *	\param[in] UINT ctrlId
 *	\param[in] HWND hwndDlg
 *	\return BOOL ToCustHyperlink (hwndDlg, ctrlId, hUserUnderlineFont)
 *
 */
...
/**
 *
 *	\fn BOOL ToCustHyperlink (HWND hwndDlg, UINT ctrlId, HFONT hFont)
 *	\brief Make custom hyper link
 *	\param[in] HFONT hFont
 *	\param[in] UINT ctrlId
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE
...
/**
 *
 *	\fn void AccommodateTextField (HWND hwndDlg, UINT ctrlId, BOOL bFirstUpdate, HFONT hFont)
 *	\brief Resizes a text field according to its actual width and height in pixels (font size is taken into account) and moves
...
/**
 *
 *	\fn static LRESULT CALLBACK BootPwdFieldProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
 *	\brief Protects an input field from having its content updated by a Paste action (call ToBootPwdField() to use this).
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT message
 *	\param[in] HWND hwnd
 *	\return static LRESULT 1 switch (message) case WM_PASTE
 *			CallWindowProc (wp, hwnd, message, wParam, lParam) else
 *
 */
...
/**
 *
 *	\fn void ToBootPwdField (HWND hwndDlg, UINT ctrlId)
 *	\brief Protects an input field from having its content updated by a Paste action. Used for pre-boot password
...
/**
 *
 *	\fn BOOL CALLBACK AuxiliaryDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief This function currently serves the following purposes:
 ...
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK AboutDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Except in response to the WM_INITDIALOG message, the dialog box procedure
 ...
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_APP
 *			1 switch (msg) case WM_COMMAND if (lw == IDOK || lw == IDCANCEL)
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_HOMEPAGE)
 *			1 switch (msg) case WM_COMMAND if (HIWORD (wParam) == EN_UPDATE)
 *			0 switch (msg) case WM_COMMAND else 
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK StaticModelessWaitDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Call DisplayStaticModelessWaitDlg() to open this dialog and CloseStaticModelessWaitDlg() to close it.
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (msg) case WM_INITDIALOG
 *	\return 1 switch (msg) case WM_COMMAND if (lw == IDOK || lw == IDCANCEL)
 *	\return 0 switch (msg) case WM_COMMAND else
 *	\return 1 switch (msg) case WM_CLOSE
 *	\return 0 else
 *
 */
...
/**
 *
 *	\fn void DisplayStaticModelessWaitDlg (HWND parent)
 *	\brief Opens a dialog window saying "Please wait..." which is not modal and does not need any GUI refresh after initialization.
...
/**
 *
 *	\fn void CloseStaticModelessWaitDlg (void)
 *	\brief Close the dialog window
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL IsButtonChecked (HWND hButton)
 *	\brief Is the button checked ?
 *	\param[in] HWND hButton
 *	\return BOOL TRUE if (SendMessage (hButton, BM_GETCHECK, 0, 0) == BST_CHECKED)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn void CheckButton (HWND hButton)
...
/**
 *
 *	\fn void LeftPadString (char *szTmp, int len, int targetLen, char filler)
 *	\brief Manage left padding on the string
 *	\param[in] char filler
 *	\param[in] int targetLen
 *	\param[in] int len
 *	\param[in] char * szTmp
 *	\return void
 *
...
/**
 *
 *	\fn void ToSBCS (LPWSTR lpszText)
 *	\brief converts a unicode string to Single Byte Character String (SBCS).
 *	\param[in] LPWSTR lpszText
 *	\return void
 *
 */
...
/**
 *
 *	\fn void ToUNICODE (char *lpszText)
 *	\brief converts a SBCS string to a UNICODE string.
 *	\param[in] char * lpszText
 *	\return void
 *
 */
...
/**
 *
 *	\fn void InitDialog (HWND hwndDlg)
 *	\brief InitDialog - initialize the applications main dialog, this function should
...
/**
 *
 *	\fn void ProcessPaintMessages (HWND hwnd, int maxMessagesToProcess)
 *	\brief The parameter maxMessagesToProcess prevents endless processing of paint messages
 *	\param[in] int maxMessagesToProcess
 *	\param[in] HWND hwnd
 *	\return void
 *
 */
...
/**
 *
 *	\fn HDC CreateMemBitmap (HINSTANCE hInstance, HWND hwnd, char *resource)
 *	\brief Create memory bitmap
 *	\param[in] char * resource
 *	\param[in] HWND hwnd
...
/**
 *
 *	\fn HBITMAP RenderBitmap (char *resource, HWND hwndDest, int x, int y, int nWidth, int nHeight, BOOL bDirectRender, BOOL bKeepAspectRatio)
 *	\brief Renders the specified bitmap at the specified location and stretches it to fit (anti-aliasing is applied). 
 ...
 *	\param[in] BOOL bKeepAspectRatio
 *	\param[in] BOOL bDirectRender
 *	\param[in] int nHeight
 *	\param[in] int nWidth
 *	\param[in] int y
 *	\param[in] int x
 *	\param[in] HWND hwndDest
 *	\param[in] char * resource
 *	\return HBITMAP hbmpRescaled
 *
 */
...
/**
 *
 *	\fn LRESULT CALLBACK RedTick (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Paint with red tick
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwnd
 *	\return LRESULT DefWindowProc (hwnd, uMsg, wParam, lParam) if (uMsg == WM_PAINT && GetUpdateRect (hwnd, NULL, FALSE))
 *			TRUE if (uMsg == WM_PAINT) otherwise
 *			DefWindowProc (hwnd, uMsg, wParam, lParam) else
 *
 */
LRESULT CALLBACK
...
/**
 *
 *	\fn BOOL RegisterRedTick (HINSTANCE hInstance)
 *	\brief Register the red tick
 *	\param[in] HINSTANCE hInstance
 *	\return BOOL rc == 0 ? FALSE : TRUE
 *
 */
BOOL
...
/**
 *
 *	\fn BOOL UnregisterRedTick (HINSTANCE hInstance)
 *	\brief Unregister the red tick
 *	\param[in] HINSTANCE hInstance
 *	\return BOOL UnregisterClass ("REDTICK", hInstance)
 *
 */
BOOL
...
/**
 *
 *	\fn LRESULT CALLBACK SplashDlgProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Splash the dialog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwnd
 *	\return LRESULT DefDlgProc (hwnd, uMsg, wParam, lParam)
 *
 */
LRESULT CALLBACK
...
/**
 *
 *	\fn void ArrowWaitCursor ()
 *	\brief Arrow Wait cursor mode
 *	\return void
 *
 */
void
...
/**
 *
 *	\fn void AddComboPair (HWND hComboBox, const char *lpszItem, int value)
 *	\brief Add a combo pair and set data
 *	\param[in] int value
 *	\param[in] const char * lpszItem
...
/**
 *
 *	\fn void AddComboPairW (HWND hComboBox, const wchar_t *lpszItem, int value)
 *	\brief Add a combo pair (as multibyte) and set data
 *	\param[in] int value
 *	\param[in] const char * lpszItem
 *	\param[in] HWND hComboBox
 *	\return void
...
/**
 *
 *	\fn void SelectAlgo (HWND hComboBox, int *algo_id)
 *	\brief Select the algo, select default algo if something went wrong
 *	\param[in] int * algo_id
 *	\param[in] HWND hComboBox
 *	\return void
 *
 */
...
/**
 *
 *	\fn void PopulateWipeModeCombo (HWND hComboBox, BOOL bNA, BOOL bInPlaceEncryption)
 *	\brief Add Wipe mode combo
 *	\param[in] BOOL bInPlaceEncryption
 *	\param[in] BOOL bNA
 *	\param[in] HWND hComboBox
 *	\return void
 *
...
/**
 *
 *	\fn wchar_t *GetWipeModeName (WipeAlgorithmId modeId)
 *	\brief Get the name of the wipe mode
 *	\param[in] WipeAlgorithmId modeId
 *	\return wchar_t * GetString ("WIPE_MODE_NONE") switch (modeId) case GST_WIPE_NONE
 *			GetString ("WIPE_MODE_1_RAND") switch (modeId) case GST_WIPE_1_RAND
 *			GetString ("WIPE_MODE_3_DOD_5220") switch (modeId) case GST_WIPE_3_DOD_5220
 *			GetString ("WIPE_MODE_7_DOD_5220") switch (modeId) case GST_WIPE_7_DOD_5220
 *			GetString ("WIPE_MODE_35_GUTMANN") switch (modeId) case GST_WIPE_35_GUTMANN
 *			GetString ("NOT_APPLICABLE_OR_NOT_AVAILABLE") switch (modeId) case default
 *
 */
...
/**
 *
 *	\fn wchar_t *GetPathType (const char *path, BOOL bUpperCase, BOOL *bIsPartition)
 *	\brief Get the path type (Partition / Volume / Device)(upper / lower)
 *	\param[in] BOOL * bIsPartition
 *	\param[in] BOOL bUpperCase
 *	\param[in] const char * path
 *	\return wchar_t * GetString (bUpperCase ? "PARTITION_UPPER_CASE" : "PARTITION_LOWER_CASE") if (strstr (path, "Partition") && strstr (path, "Partition0") == NULL)
 *			GetString (bUpperCase ? "VOLUME_UPPER_CASE" : "VOLUME_LOWER_CASE") if (strstr (path, "HarddiskVolume"))
 *			GetString (bUpperCase ? "DEVICE_UPPER_CASE" : "DEVICE_LOWER_CASE") else
 *
 */
...
/**
 *
 *	\fn LRESULT CALLBACK CustomDlgProc (HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Make custom dialog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwnd
 *	\return LRESULT TRUE if (uMsg == WM_SETCURSOR && hCursor != NULL)
 *			DefDlgProc (hwnd, uMsg, wParam, lParam) else
 *
 */
...
/**
 *
 *	\fn static BOOL IsReturnAddress (DWORD64 address)
 *	\brief Is there a return adress ?
 *	\param[in] DWORD64 address
 *	\return static BOOL FALSE if (address < 0x401000 + 8 || address > codeEnd)
 *			sp[-5] == 0xe8 || (sp[-6] == 0xff && sp[-5] == 0x15) || (sp[-2] == 0xff && (sp[-1] & 0xf0) == 0xd0) else
 *
 */
...
/**
 *
 *	\fn void ExceptionHandlerThread (void *threadArg)
 *	\brief Thread to handle exceptions
...
/**
 *
 *	\fn LONG __stdcall ExceptionHandler (EXCEPTION_POINTERS *ep)
 *	\brief  Wait for the objet to be in _beginthread state
 *	\param[in] EXCEPTION_POINTERS * ep
 *	\return LONG __stdcall EXCEPTION_EXECUTE_HANDLER if (SystemFileSelectorCallPending && SystemFileSelectorCallerThreadId == GetCurrentThreadId())
 *			EXCEPTION_EXECUTE_HANDLER else
 *
 */
...
/**
 *
 *	\fn void InvalidParameterHandler (const wchar_t *expression, const wchar_t *function, const wchar_t *file, unsigned int line, uintptr_t reserved)
 *	\brief Handle invalid parameters
 *	\param[in] uintptr_t reserved
...
/**
 *
 *	\fn static LRESULT CALLBACK NonInstallUacWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
 *	\brief Calls the default window procedure to provide default processing for any window messages that an application does not process
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT message
 *	\param[in] HWND hWnd
 *	\return static LRESULT DefWindowProc (hWnd, message, wParam, lParam)
 *
 */
...
/**
 *
 *	\fn BOOL CreateSysEncMutex (void)
 *	\brief Mutex handling to prevent multiple instances of the wizard or main app from dealing with system encryption.
 *			Returns TRUE if the mutex is (or had been) successfully acquired (otherwise FALSE). 
 *	\return BOOL GSTCreateMutex (&hSysEncMutex, GST_MUTEX_NAME_SYSENC)
 *
 */
...
/**
 *
 *	\fn BOOL InstanceHasSysEncMutex (void)
 *	\brief Is there a hSysEncMutex ?
 *	\return BOOL (hSysEncMutex != NULL)
 *
 */
...
/**
 *
 *	\fn void CloseSysEncMutex (void)
 *	\brief Mutex handling to prevent multiple instances of the wizard from dealing with system encryption
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CreateNonSysInplaceEncMutex (void)
 *	\brief Returns TRUE if the mutex is (or had been) successfully acquired (otherwise FALSE).
 *	\return BOOL GSTCreateMutex (&hNonSysInplaceEncMutex, GST_MUTEX_NAME_NONSYS_INPLACE_ENC)
 *
 */
...
/**
 *
 *	\fn BOOL InstanceHasNonSysInplaceEncMutex (void)
 *	\brief Is there a hNonSysInplaceEncMutex ?
 *	\return BOOL (hNonSysInplaceEncMutex != NULL)
 *
 */
...
/**
 *
 *	\fn void CloseNonSysInplaceEncMutex (void)
 *	\brief Close the hNonSysInplaceEncMutex mutex
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL NonSysInplaceEncInProgressElsewhere (void)
 *	\brief Returns TRUE if another instance of the wizard is preparing, resuming or performing non-system in-place encryption
 *	\return BOOL (!InstanceHasNonSysInplaceEncMutex () && MutexExistsOnSystem (GST_MUTEX_NAME_NONSYS_INPLACE_ENC))
 *
 */
...
/**
 *
 *	\fn BOOL CreateDriverSetupMutex (void)
 *	\brief Mutex handling to prevent multiple instances of the wizard or main app from trying to install or register the driver or from trying to launch it in portable mode at the same time. Returns TRUE if the mutex is (or had been) successfully acquired (otherwise FALSE). 
 *	\return BOOL GSTCreateMutex (&hDriverSetupMutex, GST_MUTEX_NAME_DRIVER_SETUP)
 *
 */
...
/**
 *
 *	\fn void CloseDriverSetupMutex (void)
 *	\brief Close the hDriverSetupMutex mutex
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CreateAppSetupMutex (void)
 *	\brief Create the hAppSetupMutex mutex
 *	\return BOOL GSTCreateMutex (&hAppSetupMutex, GST_MUTEX_NAME_APP_SETUP)
 *
 */
...
/**
 *
 *	\fn void CloseAppSetupMutex (void)
 *	\brief Close the hAppSetupMutex mutex
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL IsGostCryptInstallerRunning (void)
 *	\brief Is the GostCrypt intaller running ?
 *	\return BOOL (MutexExistsOnSystem (GST_MUTEX_NAME_APP_SETUP))
 *
 */
...
/**
 *
 *	\fn BOOL GSTCreateMutex (volatile HANDLE *hMutex, char *name)
 *	\brief Returns TRUE if the mutex is (or had been) successfully acquired (otherwise FALSE). 
 *	\param[in] char * name
 *	\param[in] volatile HANDLE
 *	\return BOOL TRUE if (*hMutex != NULL)
 *			FALSE if (*hMutex == NULL)
 *			FALSE if (GetLastError () == ERROR_ALREADY_EXISTS)
 *			True else
 *
 */
...
/**
 *
 *	\fn void GSTCloseMutex (volatile HANDLE *hMutex)
 *	\brief Close the specified mutex
 *	\param[in] volatile HANDLE * hMutex
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL MutexExistsOnSystem (char *name)
 *	\brief Returns TRUE if a process running on the system has the specified mutex (otherwise FALSE). 
 *	\param[in] char * name
 *	\return BOOL FALSE if (name[0] == 0)
 *			FALSE if (hMutex == NULL && GetLastError () == ERROR_FILE_NOT_FOUND)
 *			TRUE if (hMutex == NULL && GetLastError () == ERROR_ACCESS_DENIED)
 *			FALSE if (hMutex == NULL)
 ù			TRUE else
 *
 */
...
/**
 *
 *	\fn uint32 ReadDriverConfigurationFlags ()
 *	\brief return 0 or return the value of the driver configuration flags
...
/**
 *
 *	\fn uint32 ReadEncryptionThreadPoolFreeCpuCountLimit ()
 *	\brief return 0 or return the value of the Cpu count limit
 *	\return uint32
 *
 */
...
/**
 *
 *	\fn BOOL LoadSysEncSettings (HWND hwndDlg)
 *	\brief Read the xml file for configuration, return TRUE if the status is initialized correctly (initialized in the function), FALSE otherwise.
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (xml == NULL)
 *			status else
 *
 */
BOOL LoadSysEncSettings (HWND hwndDlg)
{
	...
	if (!FileExists (GetConfigPath (GST_APPD_FILENAME_SYSTEM_ENCRYPTION))) //if there is no encrypted file
	{
		SystemEncryptionStatus = newSystemEncryptionStatus; //set to new
		nWipeMode = newnWipeMode; // set to new
	}

	if (xml == NULL) //if there is no xml, return false
	{
		return FALSE;
	}

	while (xml = XmlFindElement (xml, "config")) //read the xml and set the configuration
	{
...
/**
 *
 *	\fn int LoadNonSysInPlaceEncSettings (WipeAlgorithmId *wipeAlgorithm)
 *	\brief Returns the number of partitions where non-system in-place encryption is progress or had been in progress
 ...
 *	\return int 0 if (!FileExists (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC)))
 *			0 if ((fileBuf = LoadFile (GetConfigPath (GST_APPD_FILENAME_NONSYS_INPLACE_ENC), &size)) == NULL)
 *			count else
 *
 */
...
/**
 *
 *	\fn void RemoveNonSysInPlaceEncNotifications (void)
 *	\brief Remove encryption notifications
 *	\return void
 *
 */
...
/**
 *
 *	\fn void SavePostInstallTasksSettings (int command)
 *	\brief Save post install tasks settings
 *	\param[in] int command
 *	\return void
 *
 */
...
/**
 *
 *	\fn void DoPostInstallTasks (void)
 *	\brief Do post install tasks
 *	\return void
 *
 */
...
/**
 *
 *	\fn void InitOSVersionInfo ()
 *	\brief Modify the global nCurrent windows OS to the actual OS used, if the OS is unknown, return WIN_UNKOWN
 *	\return void
 *
 */
void InitOSVersionInfo ()
{
	...
	CurrentOSMajor = os.dwMajorVersion; //get the major value of OS example : Windows 10 : 10
	CurrentOSMinor = os.dwMinorVersion; // get minor value of OS example : Windows 10.3 : 3

	/* détermination de la version de  l'OS utilisé  (windows) */
...
/**
 *
 *	\fn void InitApp (HINSTANCE hInstance, char *lpszCommandLine)
 *	\brief InitApp - initialize the application, this function is called once in the
 *			applications WinMain function, but before the main dialog has been created
 *	\param[in] char * lpszCommandLine
 *	\param[in] HINSTANCE hInstance
 *	\return void
 *
 */
...
/**
 *
 *	\fn void InitHelpFileName (void)
 *	\brief Initialize the help file name
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL OpenDevice (const char *lpszPath, OPEN_TEST_STRUCT *driver, BOOL detectFilesystem)
 *	\brief Open a device
 *	\param[in] BOOL detectFilesystem
 *	\param[in] OPEN_TEST_STRUCT * driver
 *	\param[in] const char lpszPath
 *	\return BOOL TRUE if (bResult == FALSE && (dwResult == ERROR_SHARING_VIOLATION || dwResult == ERROR_NOT_READY) )
 *			FALSE if (bResult == FALSE && !(dwResult == ERROR_SHARING_VIOLATION || dwResult == ERROR_NOT_READY) )
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn void NotifyDriverOfPortableMode (void)
 *	\brief Tells the driver that it's running in portable mode
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL GetDriveLabel (int driveNo, wchar_t *label, int labelSize)
 *	\brief Get the drive label
 *	\param[in] int labelSize
 *	\param[in] wchar_t * label
 *	\param[in] int driveNo
 *	\return BOOL GetVolumeInformationW (root, label, labelSize / 2, NULL, NULL, &fileSystemFlags, NULL, 0)
 *
 */
...
/**
 *
 *	\fn BOOL GetSysDevicePaths (HWND hwndDlg)
 *	\brief Stores the device path of the system partition in SysPartitionDevicePath and the device path of the system drive
 ...
 *	\param[in] HWND hwndDlg
 *	\return BOOL (bCachedSysDevicePathsValid && strlen (SysPartitionDevicePath) > 1 && strlen (SysDriveDevicePath) > 1)
 *
 */
...
/**
 *
 *	\fn int IsSystemDevicePath (char *path, HWND hwndDlg, BOOL bReliableRequired)
 *\brief Determines whether the device path is the path of the system partition or of the system drive (or neither). 
 ...
 *	\param[in] BOOL bReliableRequired
 *	\param[in] HWND hwndDlg
 *	\param[in] char * path
 *	\return int -1 if (!bCachedSysDevicePathsValid && bReliableRequired && !GetSysDevicePaths (hwndDlg))
 *			-1 if (strlen (SysPartitionDevicePath) <= 1 || strlen (SysDriveDevicePath) <= 1)
 *			1 if (strncmp (path, SysPartitionDevicePath, max (strlen(path), strlen(SysPartitionDevicePath))) == 0)
 *			2 else if (strncmp (path, SysDriveDevicePath, max (strlen(path), strlen(SysDriveDevicePath))) == 0)
 *			3 else if (ExtraBootPartitionDevicePath == path)
 *			0 else
 *
 */
...
/**
 *
 *	\fn wstring GetSysEncryptionPretestInfo2String (void)
 *	\brief Get system encryption info string
 *	\return wstring (wstring (L"\n") + GetString ("SYS_ENCRYPTION_PRETEST_INFO2_PORTION_1") + GetString ("SYS_ENCRYPTION_PRETEST_INFO2_PORTION_2") + GetString ("SYS_ENCRYPTION_PRETEST_INFO2_PORTION_3") + GetString ("SYS_ENCRYPTION_PRETEST_INFO2_PORTION_4"))
 *
 */
...
/**
 *
 *	\fn wstring GetRescueDiskHelpString (void)
 *	\brief Get rescue disk help string
 *	\return wstring (wstring ( GetString ("RESCUE_DISK_HELP_PORTION_1")) + GetString ("RESCUE_DISK_HELP_PORTION_2") + GetString ("RESCUE_DISK_HELP_PORTION_3") + GetString ("RESCUE_DISK_HELP_PORTION_4") + GetString ("RESCUE_DISK_HELP_PORTION_5") + GetString ("RESCUE_DISK_HELP_PORTION_6") + GetString ("RESCUE_DISK_HELP_PORTION_7") + GetString ("RESCUE_DISK_HELP_PORTION_8") + GetString ("RESCUE_DISK_HELP_PORTION_9"))
 *
 */
...
/**
 *
 *	\fn BOOL TextInfoDialogBox (int nID)
 *	\brief Manage the dialog box of text info
 *	\param[in] int nID
 *	\return BOOl DialogBoxParamW (hInst, MAKEINTRESOURCEW (IDD_TEXT_INFO_DIALOG_BOX_DLG), MainDlg, (DLGPROC) TextInfoDialogBoxDlgProc, (LPARAM) nID)
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK TextInfoDialogBoxDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage every case of text info diallog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_COMMAND if (lw == IDOK || lw == IDCANCEL)
 *			1 switch (msg) case WM_COMMAND if if (lw == IDC_PRINT)
 *			0 switch (msg) case WM_COMMAND else
 *			1 switch (msg) case GST_APPMSG_LOAD_TEXT_BOX_CONTENT
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
 */
...
/**
 *
 *	\fn char * GetLegalNotices ()
 *	\briefGet legal notices
 *	\return char * buf
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK RawDevicesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage raw devices dialog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG if (devices.empty())
 *			1 switch (msg) case WM_INITDIALOG otherwise
 *			1 switch (msg) case WM_COMMAND if (msg == WM_NOTIFY && ((LPNMHDR) lParam)->code == LVN_ITEMCHANGED && (((LPNMLISTVIEW) lParam)->uNewState & LVIS_FOCUSED ))
 *			1 switch (msg) case WM_COMMAND if (lw == IDCANCEL)
 *			0 switch (msg) case WM_COMMAND else
 *			1 switch (msg) case WM_NOTIFY if (msg == WM_NOTIFY && ((LPNMHDR) lParam)->code == LVN_ITEMCHANGED && (((LPNMLISTVIEW) lParam)->uNewState & LVIS_FOCUSED ))
 *			1 switch (msg) case WM_NOTIFY if (lw == IDCANCEL)
 *			0 switch (msg) case WM_NOTIFY else
 *			0 else
 *
 */
...
/**
 *
 *	\fn BOOL DoDriverInstall (HWND hwndDlg)
 *	\brief Do the driver install
 *	\param[in] HWND hwndDlg
 *	\return BOOL TRUE if (SystemEncryptionUpdate)
 *			bOK else 
 *
 */
...
/**
 *
 *	\fn static int DriverLoad ()
 *	\brief Install and start driver service and mark it for removal (non-install mode)
 *	\return static int ERR_PARAMETER_INCORRECT if (ReadLocalMachineRegistryDword ("SYSTEM\\CurrentControlSet\\Services\\gostcrypt", "Start", &startType) && startType == SERVICE_BOOT_START)
 *			ERR_DONT_REPORT if (file == INVALID_HANDLE_VALUE)
 *			ERR_DONT_REPORT if (hManager == NULL && GetLastError () == ERROR_ACCESS_DENIED)
 *			ERR_OS_ERROR if (hManager == NULL && !GetLastError () == ERROR_ACCESS_DENIED)
 *			ERR_OS_ERROR if (hService == NULL)
 *			!res ? ERR_OS_ERROR : ERROR_SUCCESS else
 *
 */
...
/**
 *
 *	\fn BOOL DriverUnload ()
 *	\brief Unload the driver
 *	\return BOOL TRUE if (hDriver == INVALID_HANDLE_VALUE)
 *			FALSE if (BootEncryption (NULL).GetStatus().DeviceFilterActive)
 *			FALSE if (bResult && volumesMounted != 0)
 *			TRUE if (!bResult)
 *			FALSE if (refCount > 1)
 *			TRUE if (driverUnloaded)
 *			FALSE else
 *			
 *
 */
...
/**
 *
 *	\fn int DriverAttach (void)
 *	\brief Attach a driver
 *	\return int res if (res != ERROR_SUCCESS)
 *			ERR_OS_ERROR if (hDriver == INVALID_HANDLE_VALUE)
 *			ERR_OS_ERROR if (bResult == FALSE)
 *			ERR_DRIVER_VERSION else if (DriverVersion != VERSION_NUM)
 *			0 else
 *
 */
...
/**
 *
 *	\fn void ResetCurrentDirectory ()
 *	\brief Reset the current directory
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL BrowseFiles (HWND hwndDlg, char *stringId, char *lpszFileName, BOOL keepHistory, BOOL saveMode, wchar_t *browseFilter)
 *	\brief Browse files
 *	\param[in] wchar_t * browseFilter
 *	\param[in] BOOL saveMode
 *	\param[in] BOOL keepHistory
 *	\param[in] char * lpszFileName
 *	\param[in] char * stringId
 *	\param[in] HWND hwndDlg
 *	\return BOOL BrowseFilesInDir (hwndDlg, stringId, NULL, lpszFileName, keepHistory, saveMode, browseFilter)
 *
 */
...
/**
 *
 *	\fn BOOL BrowseFilesInDir (HWND hwndDlg, char *stringId, char *initialDir, char *lpszFileName, BOOL keepHistory, BOOL saveMode, wchar_t *browseFilter, const wchar_t *initialFileName, const wchar_t *defaultExtension)
 *	\brief Browse files in a directory
 *	\param[in] const wchar_t * defaultExtension
 *	\param[in] const wchar_t * initialFileName
 *	\param[in] wchar_t * browseFilter
 *	\param[in] BOOL saveMode
 *	\param[in] BOOL keepHistory
 *	\param[in] char * lpszFileName
 *	\param[in] char * initialDir
 *	\param[in] char * stringId
 *	\param[in] HWND hwndDlg
 *	\return BOOL status
 *
...
/**
 *
 *	\fn BOOL SelectMultipleFiles (HWND hwndDlg, char *stringId, char *lpszFileName, BOOL keepHistory)
 *	\brief Select multiples files
 *	\param[in] BOOL keepHistory
 *	\param[in] char * lpszFileName
 *	\param[in] char * stringId
 *	\param[in] HWND hwndDlg
...
/**
 *
 *	\fn BOOL SelectMultipleFilesNext (char *lpszFileName)
 *	\brief Select next file
 *	\param[in] char * lpszFileName
 *	\return BOOL FALSE if (SelectMultipleFilesOffset == 0)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static int CALLBACK BrowseCallbackProc(HWND hwnd,UINT uMsg,LPARAM lp, LPARAM pData) 
 *	\brief Select next file
 *	\param[in] LPARAM pData
 *	\param[in] LPARAM lp
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwnd
 *	\return static int 0
...
/**
 *
 *	\fn BOOL BrowseDirectories (HWND hwndDlg, char *lpszTitle, char *dirName)
 *	\brief Browse directory
 *	\param[in] char * dirName
 *	\param[in] char * lpszTitle
 *	\param[in] HWND hwndDlg
 *	\return BOOL bOK
 *
 */
...
/**
 *
 *	\fn std::wstring GetWrongPasswordErrorMessage (HWND hwndDlg)
 *	\briefGet the wrong password error message
 *	\param[in] HWND hwndDlg
...
/**
 *
 *	\fn void handleError (HWND hwndDlg, int code)
 *	\brief Check the code sent to the function and print the error accorded to it.
 *	\param[in] int code
 *	\param[in] HWND hwndDlg
 *	\return void
 *
...
/**
 *
 *	\fn BOOL CheckFileStreamWriteErrors (FILE *file, const char *fileName)
 *	\brief Check write errors in file stream
 *	\param[in] const char
 *	\param[in] FILE * file
 *	\return BOOL FALSE if (ferror (file))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK LocalizeDialogEnum( HWND hwnd, LPARAM font)
 *	\brief Localize the dialog enumeration
 *	\param[in] LPARAM font
 *	\param[in] HWND hwnd
...
/**
 *
 *	\fn void LocalizeDialog (HWND hwnd, char *stringId)
 *	\brief Localize a Dialog
 *	\param[in] char * stringId
 *	\param[in] HWND hwnd
 *	\return void
 *
...
/**
 *
 *	\fn void OpenVolumeExplorerWindow (int driveNo)
 *	\brief Open a volume in windows explorer
 *	\param[in] int driveNo
 *	\return void
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK CloseVolumeExplorerWindowsChildEnum (HWND hwnd, LPARAM driveStr)
 *	\brief Close the volume in windows explorer
 *	\param[in] LPARAM driveStr
 *	\param[in] HWND hwnd
 *	\return static BOOL FALSE if (strstr (s, (char *) driveStr) != NULL)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static BOOL CALLBACK CloseVolumeExplorerWindowsEnum (HWND hwnd, LPARAM driveNo)
 *	\brief Close the volume in windows explorer
 *	\param[in] LPARAM driveNo
 *	\param[in] HWND hwnd
 *	\return static BOOL TRUE
 *
 */
...
/**
 *
 *	\fn BOOL CloseVolumeExplorerWindows (HWND hwnd, int driveNo)
 *	\brief Close volume explorer windows
 *	\param[in] int driveNo
 *	\param[in] HWND hwnd
 *	\return BOOL explorerCloseSent
 *
 */
...
/**
 *
 *	\fn string GetUserFriendlyVersionString (int version)
 *	\brief Get a user-friendly version string
...
/**
 *
 *	\fn void GetSizeString (unsigned __int64 size, wchar_t *str)
 *	\brief Get size in string
 *	\param[in] wchar_t * str
 *	\param[in] unsigned __int64 size
 *	\return void
 *
 */
...
/**
 *
 *	\fn void GetSpeedString (unsigned __int64 speed, wchar_t *str)
 *	\brief Get speed in string
 *	\param[in] wchar_t * str
 *	\param[in] unsigned __int64 speed
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void DisplayBenchmarkResults (HWND hwndDlg)
...
/**
 *
 *	\fn static BOOL PerformBenchmark(HWND hwndDlg)
 *	\briefPerform a benchmark
 *	\param[in] HWND hwndDlg
 *	\return static BOOL FALSE if (!ci)
 *			FALSE if (QueryPerformanceFrequency (&benchmarkPerformanceFrequency) == 0)
 *			FALSE if (lpTestBuffer == NULL)
 *			FALSE goto counter_error
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK BenchmarkDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Benchmark dialog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_BENCHMARK_SORT_METHOD
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_PERFORM_BENCHMARK
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_PARALLELIZATION_LABEL_LINK
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDCLOSE
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDCANCEL
 *			0 switch (msg) case WM_COMMAND) else
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_BENCHMARK_SORT_METHOD
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_PERFORM_BENCHMARK
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_PARALLELIZATION_LABEL_LINK
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDCLOSE
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDCANCEL
 *			0 switch (msg) case WM_NOTIFY else
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
...
/**
 *
 *	\fn static BOOL CALLBACK RandomPoolEnrichementDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Random Pool enrichement dialog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return static BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_TIMER
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_PRF_ID && hw == CBN_SELCHANGE)
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_DISPLAY_POOL_CONTENTS)
 *			0 switch (msg) case WM_COMMAND else
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
...
/**
 *
 *	\fn void UserEnrichRandomPool (HWND hwndDlg)
 *	\brief User enrichement a random pool
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK KeyfileGeneratorDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Except in response to the WM_INITDIALOG message, the dialog box procedure
 *			should return nonzero if it processes the message, and zero if it does
 *			not. - see DialogProc
 *	\param[in] LPARAM lParam
 ...
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_TIMER
 *			1 switch (msg) case WM_COMMAND
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_PRF_ID && hw == CBN_SELCHANGE)
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_DISPLAY_POOL_CONTENTS)
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_GENERATE_AND_SAVE_KEYFILE && (!BrowseFiles (hwndDlg, "OPEN_TITLE", szFileName, bHistory, TRUE, NULL)))
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_DISPLAY_POOL_CONTENTS && ((fhKeyfile = _open(szFileName, _O_CREAT|_O_TRUNC|_O_WRONLY|_O_BINARY, _S_IREAD|_S_IWRITE)) == -1))
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_DISPLAY_POOL_CONTENTS && (!RandgetBytes (keyfile, sizeof(keyfile), TRUE)))
 *			1 switch (msg) case WM_INITDIALOG if (lw == IDC_GENERATE_AND_SAVE_KEYFILE)
 *			0 switch (msg) case WM_INITDIALOG else
 *			1 switch (msg) case WM_CLOSE
 *			0 else
 *
 */
...
/**
 *
 *	\fn CipherTestDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Except in response to the WM_INITDIALOG message, the dialog box procedure
 *			should return nonzero if it processes the message, and zero if it does
 *			not. - see DialogProc
 *	\param[in] LPARAM lParam
 ...
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (uMsg) case WM_INITDIALOG
 *			1 switch (uMsg) case WM_INITDIALOG
 *			1 switch (uMsg) case WM_COMMAND if (hw == CBN_SELCHANGE && lw == IDC_CIPHER)
 *			1 switch (uMsg) case WM_COMMAND if (hw == CBN_SELCHANGE && lw == IDC_KEY_SIZE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_RESET)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_AUTO)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (n != ks * 2))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (n != pt * 2))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (bXTSTestEnabled) && (GetWindowText(GetDlgItem(hwndDlg, IDC_SECONDARY_KEY), szTmp, sizeof(szTmp)) != 64 && GetWindowText(GetDlgItem(hwndDlg, IDC_SECONDARY_KEY), szTmp, sizeof(szTmp)) != 256))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (bXTSTestEnabled) && (tlen > 16 || tlen < 1))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (ks != CB_ERR && pt != CB_ERR) && (!ci))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (ks != CB_ERR && pt != CB_ERR) && ((tmpRetVal = EAInit (ci->ea, (unsigned char *) key, ci->ks)) != ERR_SUCCESS))
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT && (ks != CB_ERR && pt != CB_ERR) && (!EAInitMode (ci)) )
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDOK || lw == IDC_ENCRYPT || lw == IDC_DECRYPT)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDCLOSE || lw == IDCANCEL)
 *			1 switch (uMsg) case WM_CLOSE
 *			0 else
 *
 */
BOOL CALLBACK
...
/**
 *
 *	\fn void ResetCipherTest(HWND hwndDlg, int idTestCipher)
 *	\brief Recipher for testing
 *	\param[in] int idTestCipher
 *	\param[in] HWND hwndDlg
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK MultiChoiceDialogProc (HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
 *	\brief Dialog proc to manage multi choice
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (uMsg) case WM_INITDIALOG
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDCLOSE || lw == IDCANCEL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == nChoiceIDs[nStr])
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDCLOSE || lw == IDCANCEL)
 *			1 switch (uMsg) case WM_CLOSE
 *			0 else
 *
...
/**
 *
 *	\fn BOOL CheckCapsLock (HWND hwnd, BOOL quiet)
 *	\brief Check capslock
 *	\param[in] BOOL quiet
 *	\param[in] HWND hwnd
 *	\return BOOL TRUE if ((GetKeyState(VK_CAPITAL) & 1) != 0 && (!quiet))
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn BOOL CheckFileExtension (char *fileName)
 *	\brief Checks whether the file extension is not used for executable files or similarly problematic, which often
 *			causes Windows and antivirus software to interfere with the container.
 *	\param[in] char * fileName
 *	\return BOOL FALSE if (!ext)
 *			TRUE if (!_stricmp (ext, problemFileExt[i++]))
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn void IncreaseWrongPwdRetryCount (int count)
 *	\brief Increase count of wrong password retry
 *	\param[in] int count
 *	\return void
 *
 */
...
/**
 *
 *	\fn void ResetWrongPwdRetryCount (void)
 *	\brief Reset the count of wrong password retry
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL WrongPwdRetryCountOverLimit (void)
 *	\brief Is the wrong password count over limit ?
 *	\return BOOL (WrongPwdRetryCounter > GST_TRY_HEADER_BAK_AFTER_NBR_WRONG_PWD_TRIES)
 *
 */
...
/**
 *
 *	\fn int GetFirstAvailableDrive ()
 *	\brief Get the first available drive
 *	\return int i if (!(dwUsedDrives & 1 << i))
 *			-1 else
 *
 */
...
/**
 *
 *	\fn int GetLastAvailableDrive ()
 *	\brief Get the last available drive
 *	\return int i if (!(dwUsedDrives & 1 << i))
 *			-1 else
 *
 */
...
/**
 *
 *	\fn BOOL IsDriveAvailable (int driveNo)
 *	\brief Is the drive available ?
 *	\param[in] int driveNo
 *	\return BOOL (GetLogicalDrives() & (1 << driveNo)) == 0
 *
 */
...
/**
 *
 *	\fn BOOL IsDeviceMounted (char *deviceName)
...
/**
 *
 *	\fn int DriverUnmountVolume (HWND hwndDlg, int nDosDriveNo, BOOL forced)
 *	\brief  Driver to unmount volume
 *	\param[in] BOOL forced
 *	\param[in] int nDosDriveNo
 *	\param[in] HWND hwndDlg
 *	\return int 1 if (bResult == FALSE)
 *			unmount.nReturnCode else
 *
 */
...
/**
 *
 *	\fn void BroadcastDeviceChange (WPARAM message, int nDosDriveNo, DWORD driveMap)
 *	\brief Broadcast the device change
 *	\param[in] DWORD driveMap
 *	\param[in] int nDosDriveNo
 *	\param[in] WPARAM message
 *	\return void
 *
 */
...
/**
*
*	\fn int MountVolume (HWND hwndDlg,int driveNo,char *volumePath,Password *password,BOOL cachePassword,BOOL sharedAccess,const MountOptions* const mountOptions,BOOL quiet,BOOL bReportWrongPassword)
*	\brief Mount the volume safely, check if it is running well and return the value
...
*	\param[in] HWND hwndDlg,
*	\param[in] int driveNo,
*	\param[in] char *volumePath,
*	\param[in] Password *password,
*	\param[in] BOOL cachePassword,
*	\param[in] BOOL sharedAccess,
*	\param[in] const MountOptions* const mountOptions
*	\param[in] BOOL quiet,
*	\param[in] BOOL bReportWrongPassword
*	\return int -1 if (mountOptions->PartitionInInactiveSysEncScope && (!CheckSysEncMountWithoutPBA (volumePath, quiet)))
*			-1 if (IsMountedVolume (volumePath) && !quiet)
*			-1 if (!IsDriveAvailable (driveNo) && ! quiet)
*			0 if (password == NULL && IsPasswordCacheEmpty ())
*			-1 if (mountOptions->PartitionInInactiveSysEncScope && (mount.wszVolume == NULL || swscanf_s ((const wchar_t *) mount.wszVolume, WIDE("\\Device\\Harddisk%d\\Partition"), &mount.nPartitionInInactiveSysEncScopeDriveNo, sizeof(mount.nPartitionInInactiveSysEncScopeDriveNo)) != 1))
*			-1 if (GetLastError () == ERROR_SHARING_VIOLATION)
*			-1 if (bResult == FALSE)
*			0 if ((mount.nReturnCode != 0) && (mount.nReturnCode == ERR_PASSWORD_WRONG) && bReportWrongPassword)
*			0 if (mount.nReturnCode != 0)
*			2 if (mount.bExclusiveAccess == FALSE)
*			1 else
*
*/
...
retry:
	...
	//if there is a return code that raise up an error
...
/**
 *
 *	\fn BOOL UnmountVolume (HWND hwndDlg, int nDosDriveNo, BOOL forceUnmount)
 *	\brief Unmount the volume and return False if there is an error and True if it works
 *	\param[in] BOOL forceUnmount
 *	\param[in] int nDosDriveNo
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (result != 0)
 *			TRUE else
 *
 */
...
retry:
	...
	if (result != 0)
	{
		if (result == ERR_FILES_OPEN && !Silent)
		{
			...
			{
				forced = TRUE;
				goto retry; //retry if there is an error
			}
...
/**
 *
 *	\fn BOOL IsPasswordCacheEmpty (void)
 *	\brief Check if password cache is empty
 *	\return BOOL !DeviceIoControl (hDriver, GST_IOCTL_GET_PASSWORD_CACHE_STATUS, 0, 0, 0, 0, &dw, 0)
 *
 */
...
/**
 *
 *	\fn BOOL IsMountedVolume (const char *volname)
 *	\brief Check if the volume is mouted
 *	\param[in] const char * volname
 *	\return BOOl TRUE if (0 == _wcsicmp ((wchar_t *) mlist.wszVolume[i], (WCHAR *)volume))
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn int GetMountedVolumeDriveNo (char *volname)
 *	\brief Get the number of the mounted volume
 *	\param[in] char* volname
 *	\return int -1 if (volname == NULL)
 *			i if (0 == _wcsicmp ((wchar_t *) mlist.wszVolume[i], (WCHAR *)volume))
 *			-1 else
 *
 */
...
/**
 *
 *	\fn BOOL IsAdmin (void)
 *	\brief Does the user have admin rights
...
/**
 *
 *	\fn BOOL IsBuiltInAdmin ()
 *	\brief Check if its build as admin
 *	\return BOOL FALSE if (!IsAdmin() || !OpenProcessToken (GetCurrentProcess(), TOKEN_QUERY, &procToken))
 *			FALSE if (GetTokenInformation (procToken, TokenUser, NULL, 0, &size) || GetLastError() != ERROR_INSUFFICIENT_BUFFER)
 *			FALSE if (!tokenUser)
 *			FALSE if (!GetTokenInformation (procToken, TokenUser, tokenUser, size, &size))
 *			IsWellKnownSid (tokenUser->User.Sid, WinAccountAdministratorSid) else
 *
 */
BOOL IsBuiltInAdmin ()
{
...
/**
 *
 *	\fn BOOL IsUacSupported ()
 *	\brief Check if Uac is supported
 *	\return BOOl FALSE if (!IsOSAtLeast (WIN_VISTA))
 *			(value != 0) else
 *
 */
BOOL IsUacSupported ()
...
/**
 *
 *	\fn BOOL ResolveSymbolicLink (const wchar_t *symLinkName, PWSTR targetName)
 *	\brief Resolv all symbolic link
 *	\param[in] PWSTR targetName
 *	\param[in] const wchar_t * symLinkName
 *	\return BOOL bResult
 *
 */
...
/**
 *
 *	\fn BOOL GetPartitionInfo (const char *deviceName, PPARTITION_INFORMATION rpartInfo)
 *	\brief Get all partition informations
 *	\param[in] PPARTITION_INFORMATION rpartInfo
 *	\param[in] const char * deviceName
 *	\return BOOL bResult
 *
 */
...
/**
 *
 *	\fn BOOL GetDeviceInfo (const char *deviceName, DISK_PARTITION_INFO_STRUCT *info)
 *	\brief Get all device informations
 *	\param[in] DISK_PARTITION_INFO_STRUCT * info
 *	\param[in] const char * devicename
...
/**
 *
 *	\fn BOOL GetDriveGeometry (const char *deviceName, PDISK_GEOMETRY diskGeometry)
 *	\brief Get all drive informations
 *	\param[in] PDISK_GEOMETRY diskGeometry
 *	\param[in] const char * devicename
 *	\return BOOL bResult
 *
 */
...
/**
 *
 *	\fn int GetDiskDeviceDriveLetter (PWSTR deviceName)
 *	\brief Returns drive letter number assigned to device (-1 if none)
 *	\param[in] PWSTR deviceName
 *	\return int i if (wcscmp (device, target) == 0)
 ù			-1 else
 *
 */
...
/**
 *
 *	\fn int FileSystemAppearsEmpty (const char *devicePath)
 *	\brief WARNING: This function does NOT provide 100% reliable results -- do NOT use it for critical/dangerous operations!
 *			Return values: 0 - filesystem does not appear empty, 1 - filesystem appears empty, -1 - an error occurred
 *	\param[in] const char * devicePath
 *	\return int 0 if (GetStatsFreeSpaceOnPartition (devicePath, &percentFreeSpace, &occupiedBytes, TRUE) != -1 && (occupiedBytes > BYTES_PER_GB && percentFreeSpace < 99.99	|| percentFreeSpace < 88))
 *			1 if (GetStatsFreeSpaceOnPartition (devicePath, &percentFreeSpace, &occupiedBytes, TRUE) != -1 && !(occupiedBytes > BYTES_PER_GB && percentFreeSpace < 99.99	|| percentFreeSpace < 88))
 *			-1 else
 *
 */
...
/**
 *
 *	\fn __int64 GetStatsFreeSpaceOnPartition (const char *devicePath, float *percentFree, __int64 *occupiedBytes, BOOL silent)
 *	\brief Returns the free space on the specified partition (volume) in bytes. If the 'occupiedBytes' pointer is not NULL, size of occupied space (in bytes) is written to the pointed location. In addition, if the 'percent' pointer is not NULL, % of free space is stored in the pointed location. If there's an error,  returns -1.
 *	\param[in] BOOL silent
 *	\param[in] __int64 * occupiedBytes
 *	\param[in] float * percentFree
 *	\param[in] const char * devicePath
 *	\return __int64 -1 if (!GetDiskFreeSpaceEx (szRootPath, &freeSpaceSize, &totalNumberOfBytes, &totalNumberOfFreeBytes))
 *			-1 if (percentFree != NULL || occupiedBytes != NULL && (!GetPartitionInfo (devicePath, &partitionInfo)))
 *			freeSpaceSize.QuadPart else
 *
 */
...
/**
 *
 *	\fn __int64 GetDeviceSize (const char *devicePath)
 *	\brief Returns -1 if there's an error.
 *	\param[in] const char * devicePath
 *	\return __int64 -1 if (!GetPartitionInfo (devicePath, &partitionInfo))
 *			partitionInfo.PartitionLength.QuadPart else
 *
 */
...
/**
 *
 *	\fn HANDLE DismountDrive (char *devName, char *devicePath)
 *	\brief Dismount a drive
 *	\param[in] char * devicePath
 *	\param[in] char * devName
 *	\return HANDLE INVALID_HANDLE_VALUE if (hVolume == INVALID_HANDLE_VALUE)
 *			(bResult ? hVolume : INVALID_HANDLE_VALUE) else
 *
 */
...
/**
 *
 *	\fn int64 FindString (const char *buf, const char *str, int64 bufLen, size_t strLen, int64 startOffset)
 *	\brief Returns -1 if the specified string is not found in the buffer. Otherwise, returns the
 *			offset of the first occurrence of the string. The string and the buffer may contain zeroes,
 *			which do NOT terminate them.
 *	\param[in] int64 startOffset
 *	\param[in] size_t strLen
 *	\param[in] int64 bufLen
 *	\param[in] const char * str
 *	\param[in] const char * buf
 *	\return int64 -1 if (buf == NULL || str == NULL || strLen > bufLen|| bufLen < 1|| strLen < 1|| startOffset > bufLen - strLen)
 *			i if (memcmp (buf + i, str, strLen) == 0)
 *			-1 else
 *
 */
...
/**
 *
 *	\fn BOOL FileExists (const char *filePathPtr)
 *	\brief Returns TRUE if the file or directory exists (both may be enclosed in quotation marks).
 *	\param[in] const char * filePathPtr
 *	\return BOOL (_access (filePath, 0) != -1)
 *
 */
...
/**
 *
 *	\fn __int64 FindStringInFile (const char *filePath, const char* str, int strLen)
 *	\brief Searches the file from its end for the LAST occurrence of the string str. The string may contain zeroes, which do NOT terminate the string. If the string is found, its offset from the start of the file is returned.  If the string isn't found or if any error occurs, -1 is returned.
 *	\param[in] int strLen
 *	\param[in] const char * str
 *	\param[in] const char * filePath
 *	\return __int64 -1 if (filePos <= 0 || buffer == NULL || strLen > bufSize|| strLen < 1)
 *			-1 if (src == INVALID_HANDLE_VALUE)
 *			retVal goto fsif_end
 *
 */
...
/**
 *
 *	\fn BOOL GSTCopyFile (char *sourceFileName, char *destinationFile)
 *	\brief System CopyFile() copies source file attributes (like FILE_ATTRIBUTE_ENCRYPTED)
 *			so we need to use our own copy function
 *	\param[in] char * destinationFile
 *	\param[in] char * sourceFileName
 *	\return BOOL FALSE if (src == INVALID_HANDLE_VALUE)
 *			FALSE if (dst == INVALID_HANDLE_VALUE)
 *			FALSE if (!buffer)
 *			res != 0 else
 *
 */
...
/**
 *
 *	\fn BOOL SaveBufferToFile (const char *inputBuffer, const char *destinationFile, DWORD inputLength, BOOL bAppend)
 *	\brief If bAppend is TRUE, the buffer is appended to an existing file. If bAppend is FALSE, any existing file
 *			is replaced. If an error occurs, the incomplete file is deleted (provided that bAppend is FALSE) and the function returns false.
 *	\param[in] BOOL bAppend
 *	\param[in] DWORD inputLength
 *	\param[in] const char * destinationFile
 *	\param[in] const char * inputBuffer
 *	\return BOOL FALSE if (dst == INVALID_HANDLE_VALUE)
 *			res else
 *
 */
BOOL SaveBufferToFile (const char *inputBuffer, const char *destinationFile, DWORD inputLength, BOOL bAppend)
{
	...
	//if there is a string to append to the file, set the pointer of the file at the end of the file
	if (bAppend)
...
/**
 *
 *	\fn BOOL GSTFlushFile (FILE *f)
 *	\brief Proper flush for Windows systems. Returns TRUE if successful.
 *	\param[in] FILE * f
 *	\return BOOL FALSE if (hf == INVALID_HANDLE_VALUE)
 *			FlushFileBuffers (hf) != 0 else
 *
 */
...
/**
 *
 *	\fn BOOL PrintHardCopyTextUTF16 (wchar_t *text, char *title, int textByteLen)
 *	\brief Prints a UTF-16 text (note that this involves a real printer, not a screen).
 ...
 *	\param[in] int textByteLen
 *	\param[in] char * title
 *	\param[in] wchar_t * text
 *	\return BOOL FALSE if (!FileExists (path) && (strlen(path) < 2))
 *			FALSE if (!SaveBufferToFile ("\xFF\xFE", path, 2, FALSE))
 *			FALSE if (!SaveBufferToFile ((char *) text, path, textByteLen, TRUE))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL IsNonInstallMode ()
 *	\brief Check the install mode status
 *	\return BOOL TRUE if (bPortableModeConfirmed)
 *			TRUE if (hDriver != INVALID_HANDLE_VALUE && (DeviceIoControl (hDriver, GST_IOCTL_GET_PORTABLE_MODE_STATUS, NULL, 0, NULL, 0, &dw, 0)))
 *			FALSE if (hDriver != INVALID_HANDLE_VALUE && !(DeviceIoControl (hDriver, GST_IOCTL_GET_PORTABLE_MODE_STATUS, NULL, 0, NULL, 0, &dw, 0)))
 *			TRUE if (!hDriver != INVALID_HANDLE_VALUE && (hDriverTmp == INVALID_HANDLE_VALUE) && (SUCCEEDED(SHGetFolderPath (NULL, CSIDL_APPDATA, NULL, 0, path))) && (FileExists (path)) && (!IsAdmin () && IsUacSupported ()))
 *			FALSE if (!hDriver != INVALID_HANDLE_VALUE && (hDriverTmp == INVALID_HANDLE_VALUE) && (SUCCEEDED(SHGetFolderPath (NULL, CSIDL_APPDATA, NULL, 0, path))) && (FileExists (path)) && !(!IsAdmin () && IsUacSupported ()))
 *			TRUE if (!hDriver != INVALID_HANDLE_VALUE && (hDriverTmp == INVALID_HANDLE_VALUE))
 *			FALSE (RegOpenKeyEx (HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GostCrypt", 0, KEY_READ, &hkey) == ERROR_SUCCESS)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn LRESULT SetCheckBox (HWND hwndDlg, int dlgItem, BOOL state)
 *	\brief Set parameter from arguments
 *	\param[in] BOOL state
 *	\param[in] int dlgItem
 *	\param[in] HWND hwndDlg
 *	\return LRESULT SendDlgItemMessage (hwndDlg, dlgItem, BM_SETCHECK, state ? BST_CHECKED : BST_UNCHECKED, 0)
 *
 */
...
/**
 *
 *	\fn BOOL GetCheckBox (HWND hwndDlg, int dlgItem)
 *	\brief Get checkbox state (checked or not)
 *	\param[in] int dlgItem
 *	\param[in] HWND hwndDlg
 *	\return BOOL IsButtonChecked (GetDlgItem (hwndDlg, dlgItem))
 *
 */
...
/**
 *
 *	\fn void SetListScrollHPos (HWND hList, int topMostVisibleItem)
 *	\brief Scroll the listview vertically so that the item with index of topMostVisibleItem is the topmost visible item.
 *	\param[in] int topMostVisibleItem
 *	\param[in] HWND hList
 *	\return void
 *
 */
...
/**
 *
 *	\fn void ManageStartupSeq (void)
 *	\brief Adds or removes GostCrypt.exe to/from the system startup sequence (with appropriate command line arguments)
 *	\return void
 *
 */
...
/**
 *
 *	\fn void ManageStartupSeqWiz (BOOL bRemove, const char *arg)
 *	\brief Adds or removes the GostCrypt Volume Creation Wizard to/from the system startup sequence
 *	\param[in] const char * args
 *	\param[in] BOOL bRemove
 *	\return void
 *
 */
...
/**
 *
 *	\fn void CleanLastVisitedMRU (void)
 *	\brief Delete the last used Windows file selector path for GostCrypt from the registry
 *	\return void
 *
 */
...
/**
 *
 *	\fn LRESULT ListItemAdd (HWND list, int index, char *string)
 *	\brief List all item added
 *	\param[in] char * string
 *	\param[in] int index
 *	\param[in] HWND list
 *	\return LRESULT ListView_InsertItem (list, &li)
 *
 */
...
/**
 *
 *	\fn LRESULT ListItemAddW (HWND list, int index, wchar_t *string)
 *	\brief List all item added as Unicode
 *	\param[in] wchar_t * string
 *	\param[in] int index
 *	\param[in] HWND list
 *	\return LRESULT SendMessageW (list, LVM_INSERTITEMW, 0, (LPARAM)(&li))
 *
 */
...
/**
 *
 *	\fn LRESULT ListSubItemSet (HWND list, int index, int subIndex, char *string)
 *	\brief List all sub item set
 *	\param[in] char * string
 *	\param[in] int subIndex
 *	\param[in] int index
 *	\param[in] HWND list
 *	\return LRESULT ListView_SetItem (list, &li)
 *
 */
...
/**
 *
 *	\fn LRESULT ListSubItemSetW (HWND list, int index, int subIndex, wchar_t *string)
 *	\brief List all sub item set as Unicode
 *	\param[in] wchar_t * string
 *	\param[in] int subIndex
 *	\param[in] int index
 *	\param[in] HWND list
 *	\return LRESULT SendMessageW (list, LVM_SETITEMW, 0, (LPARAM)(&li))
 *
 */
...
/**
 *
 *	\fn BOOL GetMountList (MOUNT_LIST_STRUCT *list)
 *	\brief Get the mounted list of devices
 *	\param[in] MOUNT_LIST_STRUCT * list
 *	\return BOOL DeviceIoControl (hDriver, GST_IOCTL_GET_MOUNTED_VOLUMES, list, sizeof (*list), list, sizeof (*list), &dwResult, NULL)
 *
 */
...
/**
 *
 *	\fn int GetDriverRefCount ()
 *	\brief Get the driver reference count
 *	\return int refCount if (bResult)
 *			-1 else 
 *
 */
...
/**
 *
 *	\fn BOOL LoadInt32 (char *filePath, unsigned __int32 *result, __int64 fileOffset)
 *	\brief Loads a 32-bit integer from the file at the specified file offset. The saved value is assumed to have been
 *			processed by mputLong(). The result is stored in *result. Returns TRUE if successful (otherwise FALSE).
 *	\param[in] __int64 fileOffset
 *	\param[in] unsigned __int32 * result
 *	\param[in] char * filePath
 *	\return BOOL -1 if (buffer == NULL)
 *			FALSE if (src == INVALID_HANDLE_VALUE)
 *			retVal goto fsif_end
 *
 */
...
/**
 *
 *	\fn BOOL LoadInt16 (char *filePath, int *result, __int64 fileOffset)
 *	\brief Loads a 16-bit integer from the file at the specified file offset. The saved value is assumed to have been
...
/**
 *
 *	\fn char *LoadFile (const char *fileName, DWORD *size)
 *	\brief Load a file and returns NULL if there's any error. Although the buffer can contain binary data, it is always null-terminated.
 *	\param[in] DWORD * size
 *	\param[in] const char * fileName
 *	\return char * NULL if (h == INVALID_HANDLE_VALUE)
 *			NULL if (buf == NULL)
 *			buf else
 *
 */
...
/**
 *
 *	\fn char *LoadFileBlock (char *fileName, __int64 fileOffset, size_t count)
 *	\brief Load a block file and returns NULL if there's any error.
 *	\param[in] size_t count
 *	\param[in] __int64 fileOffset
 *	\param[in] char * fileName
 *	\return char * NULL if (h == INVALID_HANDLE_VALUE)
 *			NULL if (SetFilePointerEx (h, seekOffset, &seekOffsetNew, FILE_BEGIN) == 0)
 *			NULL if (buf == NULL)
 *			NULL if (bytesRead != count)
 *			buf else
 *
 */
...
/**
 *
 *	\fn __int64 GetFileSize64 (const char *path)
 *	\brief Get the file size and returns -1 if there is an error, or the size of the file.
 *	\param[in] const char * path
 *	\return __int64 -1 if (h == INVALID_HANDLE_VALUE)
 *			-1 if (GetFileSizeEx (h, &size) == 0)
 *			size.QuadPart else
 *
 */
...
/**
 *
 *	\fn char *GetModPath (char *path, int maxSize)
 *	\brief Get the modificated path
...
/**
 *
 *	\fn char *GetConfigPath (char *fileName)
...
/**
 *
 *	\fn char *GetProgramConfigPath (char *fileName)
 *	\brief Get the program configuration path
 *	\param[in] char * fileName
 *	\return char * path
 *
 */
...
/**
 *
 *	\fn std::string GetServiceConfigPath (const char *fileName)
 *	\brief Get the service configuration path
 *	\param[in] const char * fileName
 *	\return std::string string (sysPath) + "\\" + fileName
 *
 */
...
/**
 *
 *	\fn char GetSystemDriveLetter (void)
 *	\brief Get the system drive lettre and returns 0 if an error occurs or the drive letter (as an upper-case char) of the system partition (e.g. 'C');
 *	\return char (char) (toupper (systemDir [0])) if (GetSystemDirectory (systemDir, sizeof (systemDir)))
 *			0 else
 *
 */
...
/**
 *
 *	\fn void TaskBarIconDisplayBalloonTooltip (HWND hwnd, wchar_t *headline, wchar_t *text, BOOL warning)
 *	\briefDisplay the balloon tool icon on task bar
 *	\param[in] BOOL warning
 *	\param[in] wchar_t * text
 *	\param[in] wchar_t * headline
 *	\param[in] HWND hwnd
 *	\return void
 *
 */
...
/**
 *
 *	\fn void InfoBalloon (char *headingStringId, char *textStringId)
 *	\brief Get balloon informations as string id
 *			Either of the pointers may be NULL
 *	\param[in] char * textStringId
 *	\param[in] char * headingStringId
 *	\return void
 *
 */
...
/**
 *
 *	\fn void InfoBalloonDirect (wchar_t *headingString, wchar_t *textString)
 *	\brief Get balloon informations not as string id
 *			Either of the pointers may be NULL
 *	\param[in] wchar_t * textString
 *	\param[in] wchar_t * headingString
 *	\return void
 *
 */
...
/**
 *
 *	\fn void WarningBalloon (char *headingStringId, char *textStringId)
 *	\brief Draw warning message on balloon as string id
 *			Either of the pointers may be NULL
 *	\param[in] char * textStringId
 *	\param[in] char * headingStringId
 *	\return void
...
/**
 *
 *	\fn void WarningBalloonDirect (wchar_t *headingString, wchar_t *textString)
 *	\brief Draw warning message on balloon not as string id
 *			Either of the pointers may be NULL
 *	\param[in] wchar_t * textString
 *	\param[in] wchar_t * headingString
 *	\return
...
/**
 *
 *	\fn int Info (char *stringId)
 *	\brief Draw informations as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONINFORMATION) else
 *
 */
...
/**
 *
 *	\fn int InfoTopMost (char *stringId)
 *	\brief Draw top informations as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONINFORMATION | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int InfoDirect (const wchar_t *msg)
 *	\brief Draw information not as string id in Unicode
 *	\param[in] const wchar_t * msg
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, msg, lpszTitle, MB_ICONINFORMATION) else
 *
 */
...
/**
 *
 *	\fn int Warning (char *stringId)
 *	\brief Draw warning as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING) else
 *
 */
...
/**
 *
 *	\fn int WarningTopMost (char *stringId)
 *	\brief Draw top Warning as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int WarningDirect (const wchar_t *warnMsg)
 *	\brief Draw warning not as string id in Unicode
 *	\param[in] const wchar_t * warnMsg
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, warnMsg, lpszTitle, MB_ICONWARNING) else
 *
 */
...
/**
 *
 *	\fn int Error (char *stringId)
 *	\brief Draw error as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONERROR) else
 *
 */
...
/**
 *
 *	\fn int ErrorTopMost (char *stringId)
 *	\brief Draw top error as string id in Unicode
 *	\param[in] char * stringId
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONERROR | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int ErrorDirect (const wchar_t *errMsg)
 *	\brief Draw error not as string id in Unicode
 *	\param[in] const wchar_t * errMsg
 *	\return int 0 if (Silent)
 *			MessageBoxW (MainDlg, errMsg, lpszTitle, MB_ICONERROR) else
 *
 */
...
/**
 *
 *	\fn int AskYesNo (char *stringId)
 *	\brief Ask yes/no as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskYesNoString (const wchar_t *str)
 *	\brief Ask yes/no not as string id in Unicode
 *	\param[in] const wchar_t * str
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, str, lpszTitle, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskYesNoTopmost (char *stringId)
 *	\brief Ask yes/no on top as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON1 | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int AskNoYes (char *stringId)
 *	\brief Ask no/yes as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON2) else
 *
 */
...
/**
 *
 *	\fn int AskOkCancel (char *stringId)
 *	\brief Ask ok/cancel as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDCANCEL if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONQUESTION | MB_OKCANCEL | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskWarnYesNo (char *stringId)
 *	\brief Ask warning yes/no as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskWarnYesNoString (const wchar_t *string)
 *	\brief Ask warning yes/no not as string id in Unicode
 *	\param[in] const wchar_t * string
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, string, lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskWarnYesNoTopmost (char *stringId)
 *	\brief Ask warning yes/no on top as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON1 | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int AskWarnYesNoStringTopmost (const wchar_t *string)
 *	\brief Ask warning yes/no on top not as string id in Unicode
 *	\param[in] const wchar_t * string
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, string, lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON1 | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int AskWarnNoYes (char *stringId)
 *	\brief Ask warning no/yes as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2) else
 *
 */
...
/**
 *
 *	\fn int AskWarnNoYesString (const wchar_t *string)
 *	\brief Ask warning no/yes not as string id in Unicode
 *	\param[in] const wchar_t * string
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, string, lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2) else
 *
 */
...
/**
 *
 *	\fn int AskWarnNoYesTopmost (char *stringId)
 *	\brief Ask warning no/yes on top as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2 | MB_SETFOREGROUND | MB_TOPMOST) else
 *
 */
...
/**
 *
 *	\fn int AskWarnOkCancel (char *stringId)
 *	\brief Ask warning ok/cancel as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDCANCEL if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_OKCANCEL | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskWarnCancelOk (char *stringId)
 *	\brief Ask warning cancel/ok as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDCANCEL if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONWARNING | MB_OKCANCEL | MB_DEFBUTTON2) else
 *
 */
...
/**
 *
 *	\fn int AskErrYesNo (char *stringId)
 *	\brief Ask error yes/no as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONERROR | MB_YESNO | MB_DEFBUTTON1) else
 *
 */
...
/**
 *
 *	\fn int AskErrNoYes (char *stringId)
 *	\brief Ask error no/yes as string id in Unicode
 *	\param[in] char * stringId
 *	\return int IDNO if (Silent)
 *			MessageBoxW (MainDlg, GetString (stringId), lpszTitle, MB_ICONERROR | MB_YESNO | MB_DEFBUTTON2) else
 *
 */
...
/**
 *
 *	\fn int AskMultiChoice (void *strings[], BOOL bBold)
 *	\brief The function accepts two input formats:
 ...
 *	\param[in] BOOL bBold
 *	\param[in] void * strings[]
 *	\return int DialogBoxParamW (hInst, MAKEINTRESOURCEW (IDD_MULTI_CHOICE_DLG), MainDlg, (DLGPROC) MultiChoiceDialogProc, (LPARAM) &params)
 *
 */
...
/**
 *
 *	\fn BOOL ConfigWriteBegin ()
 *	\briefBegin the configuration writing
 *	\return BOOL FALSE if (ConfigFileHandle != NULL) 
 *			FALSE if (ConfigFileHandle == NULL)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL ConfigWriteEnd ()
 *	\brief End writing in configuration file
 *	\return BOOL FALSE if (ConfigFileHandle == NULL)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL ConfigWriteString (char *configKey, char *configValue)
 *	\brief Write string in configuration file
 *	\param[in] char * configValue
 *	\param[in] char * configKey
 *	\return BOOL FALSE if (ConfigFileHandle == NULL)
 *			0 != fprintf (ConfigFileHandle, "\n\t\t<config key=\"%s\">%s</config>", configKey, configValue) else
 *
 */
...
/**
 *
 *	\fn BOOL ConfigWriteInt (char *configKey, int configValue)
 *	\brief Write int int configuration file
 *	\param[in] int configValue
 *	\param[in] char * configKey
 *	\return BOOL ConfigWriteString (configKey, val)
 *
 */
...
/**
 *
 *	\fn static BOOL ConfigRead (char *configKey, char *configValue, int maxValueSize)
 *	\briefRead configurations
 *	\param[in] int maxValueSize
 *	\param[in] char * configValue
 *	\param[in] char * configKey
 *	\return BOOL TRUE if (xml != NULL)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn int ConfigReadInt (char *configKey, int defaultValue)
 *	\brief Read the configKey, if it is an integer return it, otherwise return the defaultValue
 *	\param[in] int defaultValue
 *	\param[in] char * configKey
 *	\return int atoi (s) if (ConfigRead (configKey, s, sizeof (s)))
 *			defaultValue else
 *
 */
...
/**
 *
 *	\fn char *ConfigReadString (char *configKey, char *defaultValue, char *str, int maxLen)
 *	\brief Read the configKey, if it is a string return it, otherwise return the defaultValue
 *	\param[in] int maxLen
 *	\param[in] char * str
 *	\param[in] char * defaultValue
 *	\param[in] char * configKey
 *	\return char * str if (ConfigRead (configKey, str, maxLen))
 *			defaultValue else
 *
 */
...
/**
 *
 *	\fn void OpenPageHelp (HWND hwndDlg, int nPage)
...
/**
 *
 *	\fn void OpenOnlineHelp ()
 *	\brief Open the online help
 *	\return void
 *
 */
void OpenOnlineHelp ()
...
/**
 *
 *	\fn void RestoreDefaultKeyFilesParam (void)
 *	\brief Restore the default key files parameter
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL LoadDefaultKeyFilesParam (void)
 *	\brief Load the default key files parameter
 *	\return BOOL FALSE if (xml == NULL) 
 *			status else
 *
 */
...
/**
 *
 *	\fn void Debug (char *format, ...)
 *	\brief Generate degug string
...
/**
 *
 *	\fn void DebugMsgBox (char *format, ...)
 *	\brief Generate debug message box
 *	\param[in] char * format
 *	\return
 *
 */
 ...
 *	\brief Check if the os is at least reqMinOS
 *	\param[in] OSVersionEnum reqMinOS
 *	\return BOOL IsOSVersionAtLeast (reqMinOS, 0)!
 *
 */
...
/**
 *
 *	\fn BOOL IsOSVersionAtLeast (OSVersionEnum reqMinOS, int reqMinServicePack)
 *	\brief Check if the os version is a least over the first supported version
 ...
 *	\param[in] int reqMinServicePack
 *	\param[in] OSVersionEnum reqMinOS
 *	\return BOOL ((CurrentOSMajor << 16 | CurrentOSMinor << 8 | CurrentOSServicePack) >= (major << 16 | minor << 8 | reqMinServicePack))
 *
 */
 ...
 *
 *	\fn BOOL Is64BitOs ()
 *	\brief Check if the os is 64 bit
 *	\return BOOL isWow64
 *
 */
BOOL Is64BitOs ()
 ...
 *
 *	\fn BOOL IsServerOS ()
 *	\brief Check if the os is a server os
 *	\return BOOL (osVer.wProductType == VER_NT_SERVER || osVer.wProductType == VER_NT_DOMAIN_CONTROLLER)
 *
 */
BOOL IsServerOS ()
{
...
/**
 *
 *	\fn BOOL IsHiddenOSRunning (void)
 *	\brief Check if an hidden OS is running
...
/**
 *
 *	\fn BOOL EnableWow64FsRedirection (BOOL enable)
 *	\brief Enables file system redirection for the calling thread.
 *	\param[in] BOOL enable
 *	\return BOOL FALSE if (!wow64EnableWow64FsRedirection)
 *			wow64EnableWow64FsRedirection (enable) else
 *
 */
...
/**
 *
 *	\fn BOOL RestartComputer (void)
 *	\brief Check privileges to restart the computer
 *	\return BOOL FALSE if (!OpenProcessToken (GetCurrentProcess (), TOKEN_QUERY|TOKEN_ADJUST_PRIVILEGES, &hTkn))
 *			FALSE if (GetLastError() != ERROR_SUCCESS) 
 *			FALSE if (!ExitWindowsEx (EWX_REBOOT, SHTDN_REASON_MAJOR_OTHER | SHTDN_REASON_MINOR_OTHER | SHTDN_REASON_FLAG_PLANNED))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn std::string GetWindowsEdition ()
 *	\brief Get the windows edition of system
...
/**
 *
 *	\fn void Applink (char *dest, BOOL bSendOS, char *extraOutput)
 *	\brief Open link
 *	\param[in] char * extraOutput
 *	\param[in] BOOL bSendOS
 *	\param[in] char * dest
 *	\return void
 *
 */
...
/**
 *
 *	\fn char *RelativePath2Absolute (char *szFileName)
 *	\brief Convert relative path to absolute path
 *	\param[in] char * szFileName
...
/**
 *
 *	\fn void HandleDriveNotReadyError ()
 *	\brief Handle the not ready error from drive
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK CloseGSTWindowsEnum (HWND hwnd, LPARAM lParam)
 *	\brief Close the GST windows enum
 *	\param[in] LPARAM lParam
 *	\param[in] HWND hwnd
 *	\return BOOL TRUE
 *
...
/**
 *
 *	\fn BOOL CALLBACK FindGSTWindowEnum (HWND hwnd, LPARAM lParam)
 *	\brief Find the GST window
 *	\param[in] LPARAM lParam
 *	\param[in] HWND hwnd
 *	\return BOOL TRUE
 *
...
/**
 *
 *	\fn BYTE *MapResource (char *resourceType, int resourceId, PDWORD size)
 *	\brief Map all ressources
 *	\param[in] PDWORD size
 *	\param[in] int resourceId
 *	\param[in] char * resourceType
 *	\return BYTE (BYTE *) LockResource (hResL)
 *
 */
...
/**
 *
 *	\fn void InconsistencyResolved (char *techInfo)
 *	\brief Resolv inconsistency
 *	\param[in] char * techInfo
 *	\return void
 *
 */
...
/**
 *
 *	\fn void ReportUnexpectedState (char *techInfo)
 *	\brief Report an unexpected state
 *	\param[in] char * techInfo
 *	\return void
 *
 */
...
/**
 *
 *	\fn int OpenVolume (OpenVolumeContext *context, const char *volumePath, Password *password, BOOL write, BOOL preserveTimestamps, BOOL useBackupHeader)
 *	\brief Open a volume
 *	\param[in] BOOL useBackupHeader
 *	\param[in] BOOL preserveTimestamps
 *	\param[in] BOOL write
 *	\param[in] Password * password
 *	\param[in] const char * volumePath
 *	\param[in] OpenVolumeContext * context
 *	\return int status
 *			
 *
 */
...
/**
 *
 *	\fn void CloseVolume (OpenVolumeContext *context)
...
/**
 *
 *	\fn int ReEncryptVolumeHeader (char *buffer, BOOL bBoot, CRYPTO_INFO *cryptoInfo, Password *password, BOOL wipeMode)
 *	\brief Reencrypt the volume header
 *	\param[in] BOOL wipeMode
 *	\param[in] Password * password
 *	\param[in] CRYPTO_INFO * cryptoInfo
 *	\param[in] BOOL bBoot
 *	\param[in] char * buffer
 *	\return int ERR_PARAMETER_INCORRECT if (Randinit() != ERR_SUCCESS)
 *			status else
 *
 */
...
/**
 *
 *	\fn BOOL IsPagingFileActive (BOOL checkNonWindowsPartitionsOnly)
 *	\brief Check if the paging file is active
 *	\param[in] BOOL checkNonWindowsPartitionsOnly
 *	\return BOOL TRUE if (IsPagingFileWildcardActive())
 *			TRUE if (ReadLocalMachineRegistryMultiString ("System\\CurrentControlSet\\Control\\Session Manager\\Memory Management", "PagingFiles", data, &size) && size > 12 && !checkNonWindowsPartitionsOnly)
 *			TRUE if (!(handle != INVALID_HANDLE_VALUE) && GetLastError() == ERROR_SHARING_VIOLATION)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn BOOL IsPagingFileWildcardActive ()
 *	\brief Check if the paging wild card file is active
 *	\return BOOL size > 0 && strstr (pagingFiles, "?:\\") == pagingFiles
 *
 */
...
/**
 *
 *	\fn BOOL DisablePagingFile ()
 *	\brief Desable the paging file
 *	\return BOOL WriteLocalMachineRegistryMultiString ("System\\CurrentControlSet\\Control\\Session Manager\\Memory Management", "PagingFiles", empty, sizeof (empty))
 *
 */
...
/**
 *
 *	\fn std::wstring SingleStringToWide (const std::string &singleString)
 *	\brief Convert a single string to Wide
 *	\param[in] const std::string & singleString
 *	\return std::wstring std::wstring() if (singleString.empty())
 *			wbuf else
 *
 */
...
/**
 *
 *	\fn std::wstring Utf8StringToWide (const std::string &utf8String)
 *	\brief Convert Utf8 string to Wide
 *	\param[in] const std::string & utf8String
 *	\return std::wstring() if (utf8String.empty())
 *			wbuf else
 *
 */
...
/**
 *
 *	\fn std::string WideToUtf8String (const std::wstring &wideString)
 *	\brief Convert Wide to Utf8
 *	\param[in] const std::wstring & wideString
 *	\return std::string std::string() if (wideString.empty())
 *			buf else
 *
 */
...
/**
 *
 *	\fn std::string WideToSingleString (const std::wstring &wideString)
 *	\brief Convert Wide to single string
 *	\param[in] const std::wstring & wideString
 *	\return std::string std::string() if (wideString.empty())
 *			buf else
 *
...
/**
 *
 *	\fn std::string StringToUpperCase (const std::string &str)
 *	\brief Convert a string to a uppercase string
 *	\param[in] const std::string & str
 *	\return std::string upperCase
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK SecurityTokenPasswordDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage the security token paswword dilog proc
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
...
/**
 *
 *	\fn static BOOL CALLBACK NewSecurityTokenKeyfileDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
...
/**
 *
 *	\fn static void SecurityTokenKeyfileDlgFillList (HWND hwndDlg, const vector <SecurityTokenKeyfile> &keyfiles)
 *	\brief Fill the list of the security token file dialog
 *	\param[in] const vector <SecurityTokenKeyfile> & keyfiles
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static list <SecurityTokenKeyfile> SecurityTokenKeyfileDlgGetSelected (HWND hwndDlg, const vector <SecurityTokenKeyfile> &keyfiles)
 *	\brief Manage when the security token file dialog get selected
 *	\param[in] const vector<SecurityTokenKeyfile> & keyfiles
 *	\param[in] HWND hwndDlg
 *	\return static list <SecurityTokenKeyfile> selectedKeyfiles
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK SecurityTokenKeyfileDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 ...
 *	\return BOOL 1 switch (msg) case WM_INITDIALOG
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDCANCEL
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_IMPORT_KEYFILE
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_EXPORT
 *			1 switch (msg) case WM_COMMAND switch (lw) case IDC_DELETE
 *			0 switch (msg) case WM_COMMAND else
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDCANCEL
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_IMPORT_KEYFILE
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_EXPORT
 *			1 switch (msg) case WM_NOTIFY switch (lw) case IDC_DELETE
 *			0 switch (msg) case WM_NOTIFY else
 *			0 else
 *
 */
...
/**
 *
 *	\fn BOOL InitSecurityTokenLibrary ()
 *	\brief Initialize the library of security token
 *	\return BOOL FALSE if (SecurityTokenLibraryPath[0] == 0)
...
/**
 *
 *	\fn std::vector <HostDevice> GetAvailableHostDevices (bool noDeviceProperties, bool singleList, bool noFloppy, bool detectUnencryptedFilesystems)
 *	\brief Get available host devices
 *	\param[in] bool detectUnencryptedFilesystems
 *	\param[in] bool noFloppy
 *	\param[in] bool singleList
 *	\param[in] bool noDeviceProperties
 *	\return std::vector <HostDevice> devices
 *
 */
...
/**
 *
 *	\fn BOOL FileHasReadOnlyAttribute (const char *path)
 *	\brief Check if file has read only attribute
 *	\param[in] const char * path
 *	\return BOOL attributes != INVALID_FILE_ATTRIBUTES && (attributes & FILE_ATTRIBUTE_READONLY) != 0
 *
 */
...
/**
 *
 *	\fn BOOL IsFileOnReadOnlyFilesystem (const char *path)
 *	\brief Check if file is on read only file system
 *	\param[in] const char * path
 *	\return BOOL FALSE if (!GetVolumePathName (path, root, sizeof (root)))
 *			FALSE if (!GetVolumeInformation (root, NULL, 0,  NULL, &d, &flags, NULL, 0))
 *			(flags & FILE_READ_ONLY_VOLUME) ? TRUE : FALSE else
 *
 */
...
/**
 *
 *	\fn void CheckFilesystem (int driveNo, BOOL fixErrors)
 *	\brief Check File system
 *	\param[in] BOOL fixErrors
 *	\param[in] int driveNo
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL BufferContainsString (const byte *buffer, size_t bufferSize, const char *str)
 *	\brief Check if buffer contains string
 *	\param[in] const char * buffer
 *	\param[in] size_t bufferSize
 *	\param[in] const byte * str
 *	\return BOOL FALSE if (bufferSize < strLen)
 *			TRUE if (memcmp (buffer + i, str, strLen) == 0)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn int AskNonSysInPlaceEncryptionResume ()
 *	\briefAsk the non symtem place encryption to resume
 *	\return int IDYES if (AskWarnYesNo ("NONSYS_INPLACE_ENC_RESUME_PROMPT") == IDYES)
 *			IDNO else
 *
 */
...
/**
 *
 *	\fn BOOL RemoveDeviceWriteProtection (HWND hwndDlg, char *devicePath)
 *	\brief
 *	\param[in] char * devicePath
 *	\param[in] HWND hwndDlg
 *	\return BOOL FALSE if (sscanf (devicePath, "\\Device\\Harddisk%d\\Partition%d", &driveNumber, &partitionNumber) != 2)
 *			FALSE if (GetTempPath (sizeof (temp), temp) == 0)
 *			FALSE if (!f)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static LRESULT CALLBACK EnableElevatedCursorChangeWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
 *	\brief Enable change on elevated cursor
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT message
 *	\param[in] HWND hWnd
...
/**
 *
 *	\fn void EnableElevatedCursorChange (HWND parent)
 *	\brief Enable change on elevated cursor
 *	\param[in] HWND parent
 *	\return void
 *
...
/**
 *
 *	\fn BOOL DisableFileCompression (HANDLE file)
 *	\brief Desable the file compression
 *	\param[in] HANDLE file
 *	\return BOOL FALSE if (!DeviceIoControl (file, FSCTL_GET_COMPRESSION, NULL, 0, &format, sizeof (format), &bytesOut, NULL))
 *			TRUE if (format == COMPRESSION_FORMAT_NONE)
 *			DeviceIoControl (file, FSCTL_SET_COMPRESSION, &format, sizeof (format), NULL, 0, &bytesOut, NULL) else
 *
 */
...
/**
 *
 *	\fn BOOL VolumePathExists (char *volumePath)
 *	\brief
 *	\param[in] char * volumePath
 *	\return BOOL OpenDevice (volumePath, &openTest, FALSE) if (strstr (upperCasePath, "\\DEVICE\\") == upperCasePath)
 *			TRUE if (path.find ("\\\\?\\Volume{") == 0 && path.rfind ("}\\") == path.size() - 2 && (QueryDosDevice (path.substr (4, path.size() - 5).c_str(), devicePath, GST_MAX_PATH) != 0))
 *			_access (volumePath, 0) == 0 else
 *
 */
...
/**
 *
 *	\fn BOOL IsWindowsIsoBurnerAvailable ()
 *	\brief Check if windows ISO burner is available
 *	\return BOOL FALSE if (!IsOSAtLeast (WIN_7))
 *			(FileExists (path)) if (SUCCEEDED(SHGetFolderPath (NULL, CSIDL_SYSTEM, NULL, 0, path)))
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn BOOL LaunchWindowsIsoBurner (HWND hwnd, const char *isoPath)
 *	\brief Launch the windows ISO burner
 *	\param[in] const char * isoPath
 *	\param[in] HWND hwnd
 *	\return BOOL FALSE if (r <= 32)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn std::string VolumeGuidPathToDevicePath (std::string volumeGuidPath)
 *	\brief Get the volume path to device
 *	\param[in] std::string volumeGuidPath
 *	\return std::string string() if (volumeGuidPath.find ("Volume{") != 0 || volumeGuidPath.rfind ("}\\") != volumeGuidPath.size() - 2)
 *			string() if (QueryDosDevice (volumeGuidPath.substr (0, volumeGuidPath.size() - 1).c_str(), volDevPath, GST_MAX_PATH) == 0)
 *			partitionPath.empty() ? volDevPath : partitionPath else
 *
 */
...
/**
 *
 *	\fn std::string HarddiskVolumePathToPartitionPath (const std::string &harddiskVolumePath)
 *	\brief Convert hard disk volume path to a partition path
 *	\param[in] const std::string
 *	\return std::string WideToSingleString (partitionPath) if (ResolveSymbolicLink (partitionPath, resolvedPath) && volPath == resolvedPath))
 *			string() else
 *
 */
...
/**
 *
 *	\fn BOOL IsApplicationInstalled (const char *appName)
 *	\brief Check if application is intalled
 *	\param[in] const char * appName
 *	\return BOOL FALSE if (res != ERROR_SUCCESS)
 *			installed else
 *
 */
...
/**
 *
 *	\fn std::string FindLatestFileOrDirectory (const std::string &directory, const char *namePattern, bool findDirectory, bool findFile)
 *	\brief Find the lastest file or directory
 *	\param[in] bool findFile
 *	\param[in] bool findDirectory
 *	\param[in] const char * namePattern
 *	\param[in] const std::string & directory
 *	\return std::string string (directory) + "\\" + name
 *
 */
