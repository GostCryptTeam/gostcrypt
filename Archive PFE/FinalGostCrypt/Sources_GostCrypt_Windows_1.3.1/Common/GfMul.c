 ...
 *
 *	\file GfMul.c
 *	\brief Define function that permits to do basic operation (multiplications ...) in a optimized way, sometimes on big int.
 *	\version 1.3.1
 *	\date 16/12/2016
 *
 */
...
/**
 *
 *	\fn in_line void bsw_32(void * p, unsigned int n)
 *	\brief Rotate to the right the bit values 24 times then 8 times. and swap them after. 
...
/**
 *
 *	\fn in_line void move_block_aligned( void *p, const void *q)
 *	\brief Copy 128 bits block q in block p
 *	\param[in] const void * q
 *	\param[in] void * p
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn in_line void move_block_aligned64( void *p, const void *q)
 *	\brief Copy 128 bits block q in block p
 *	\param[in] const void * q
 *	\param[in] void * p
 *	\return in_line void
...
/**
 *
 *	\fn in_line void xor_block_aligned( void *p, const void *q)
 *	\brief Process a 128 bits xor on block q and block p. stock it in p
 *	\param[in] const void
 *	\param[in] void* p
 *	\return
 *
...
/**
 *
 *	\fn in_line void xor_block_aligned64( void *p, const void *q)
 *	\brief Process a 64 bits xor on block q and block p. stock it in p
 *	\param[in] const void
 *	\param[in] void * p
 *	\return void
 *
...
/**
 *
 *	\fn in_line void mul_x(mode(32t) x[4])
 *	\brief Multiply of a GF128 field element by x. The field element 
...
/**
 *
 *	\fn in_line void mul_x64(mode(32t) x[2])
 *	\brief Process a multiplication on 64 bits
 *	\param[in] mode(32t) x[2]
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn in_line void mul_lex8(mode(32t) x[4])
 *	\brief Mutiply 128 bits with long words in little endian format
 *	\param[in] mode(32t) x[4]
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn in_line void mul_lex8_64(mode(32t) x[2])
 *	\brief Mutiply 64 bits with long words in little endian format
 *	\param[in] mode(32t) x[2]
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn in_line void mul_bex8(mode(32t) x[4])
 *	\brief Mutiply 128 bits with long words in big endian format
 *	\param[in] mode(32t) x[4]
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn in_line void mul_bex8_64(mode(32t) x[2])
 *	\brief Mutiply 64 bits with long words in big endian format
 *	\param[in] mode(32t) x[2]
 *	\return in_line void
 *
 */
...
/**
 *
 *	\fn void GfMul128 (void *a, const void* b)
 *	\brief Different versions of the general gf_mul function are provided 
 *			here. Sadly none are very fast :-(
 *          Apply GfMultiplication on 128 bits
...
/**
 *
 *	\fn void GfMul128Tab (unsigned char a[CBLK_LEN], GfCtx8k *ctx)
 *	\brief Manage the GfMul table for 128 bits
 *	\param[in] GfCtx8k * ctx
 *	\param[in] unsigned char a[CBLK_LEN]
 *	\return void
 *
 */
...
/**
 *
 *	\fn void compile_8k_table(unsigned __int8 *a, GfCtx8k *ctx)
 *	\brief Compile 8k table of 128 bits
 *	\param[in] GfCtx8k * ctx
 *	\param[in] unsigned __int8 * a
 *	\return void
 *
 */
...
/**
 *
 *	\fn void compile_4k_table64(unsigned __int8 *a, GfCtx4k64 *ctx)
 *	\brief Compile 4k table of 64 bits
 *	\param[in] GfCtx4k64 * ctx
 *	\param[in] unsigned __int8 * a
 *	\return void
 *
 */
...
/**
 *
 *	\fn static int IsBitSet128 (unsigned int bit, unsigned __int8 *a)
 *	\brief Check if it is a 128 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned int bit
 *	\return static int a[(127 - bit) / 8] & (0x80 >> ((127 - bit) % 8))
 *
 */
...
/**
 *
 *	\fn static int IsBitSet64 (unsigned int bit, unsigned __int8 *a)
 *	\brief Check if it is a 64 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned int bit
 *	\return static int a[(63 - bit) / 8] & (0x80 >> ((63 - bit) % 8))
 *
 */
...
/**
 *
 *	\fn static void SetBit128 (unsigned int bit, unsigned __int8 *a)
 *	\brief Set to 128 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned int bit
 *	\return static void
 *
...
/**
 *
 *	\fn static void SetBit64 (unsigned int bit, unsigned __int8 *a)
 *	\brief Set to 64 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned int bit
 *	\return static void
 *
...
/**
 *
 *	\fn void MirrorBits128 (unsigned __int8 *a)
 *	\brief Mirror a 128 bits
 *	\param[in] unsigned __int8 * a
 *	\return void
 *
 */
...
/**
 *
 *	\fn void MirrorBits64 (unsigned __int8 *a)
 *	\brief Mirror a 64 bits
 *	\param[in] unsigned __int8 * a
 *	\return void
 *
 */
...
/**
 *
 *	\fn int Gf128Tab64Init (unsigned __int8 *a, GfCtx *ctx)
 *	\brief Allocate and initialize speed optimization table
...
/**
 *
 *	\fn int Gf64TabInit (unsigned __int8 *a, GfCtx *ctx)
 *	\brief Allocate and initialize speed optimization table
 *          for multiplication by 64-bit operand in MSB-first mode
 *	\param[in] GfCtx * ctx
 *	\param[in] unsigned __int8 * a
 *	\return int FALSE if (!ctx4k)
 *			TRUE otherwise
 *
 */
...
/**
 *
 *	\fn void Gf128MulBy64Tab (unsigned __int8 a[8], unsigned __int8 p[16], GfCtx *ctx)
 *	\brief Multiply a 128-bit number by a 64-bit number in the finite field GF(2^128)
 *	\param[in] GfCtx * ctx
...
/**
 *
 *	\fn void Gf64MulTab (unsigned char a[8], unsigned char p[8], GfCtx *ctx)
 *	\brief Multiply two 64-bit numbers in the finite field GF(2^64)
 *	\param[in] GfCtx * ctx
 *	\param[in] unsigned char a[8]
 *	\param[in] unsigned char p[8]
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void xor128 (uint64 *a, uint64 *b)
 *	\brief Apply xor on 128 bits
 *	\param[in] uint64 * b
 *	\param[in] uint64 * a
 *	\return static void
 *
...
/**
 *
 *	\fn static void shl128 (unsigned __int8 *a)
 *	\brief Shift of the indicated register (128 bits) bits by 1 bit to the left. (The outgoing bits are transferred to the retaining flag CF but are not reintroduced to the right).
 *	\param[in] unsigned __int8 * a
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void GfMul128Basic (unsigned __int8 *a, unsigned __int8 *b, unsigned __int8 *p)
 *	\brief Basic application of GfMultiplication on 128 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned __int8 * b
 *	\param[in] unsigned __int8 * p
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void xor64 (uint64 *a, uint64 *b)
 *	\brief Apply xor on 64 bits
 *	\param[in] uint64 * b
 *	\param[in] uint64 * a
 *	\return static void
 *
...
/**
 *
 *	\fn static void shl64 (unsigned __int8 *a)
 *	\brief Shift of the indicated register (64 bits) bits by 1 bit to the left. (The outgoing bits are transferred to the retaining flag CF but are not reintroduced to the right).
 *	\param[in] unsigned __int8 * a
 *	\return static void
 *
 */
...
/**
 *
 *	\fn static void GfMul64Basic (unsigned __int8 *a, unsigned __int8 *b, unsigned __int8* p)
 *	\brief Basic application of GfMultiplication on 64 bits
 *	\param[in] unsigned __int8 * a
 *	\param[in] unsigned __int8 * b
 *	\param[in] unsigned __int8 * p
 *	\return static void
...
/**
 *
 *	\fn BOOL GfMulSelfTest ()
 *	\brief Test GfMul
 *	\return BOOL FALSE if (!gfCtx)
 *			result otherwise
 *
 */
BOOL GfMulSelfTest ()
