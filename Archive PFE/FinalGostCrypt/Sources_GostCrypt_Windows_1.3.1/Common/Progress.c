 ...
 *
 *	\file Progress.c
 *	\brief Manage progress bar in windows
 *	\version 1.3.1
 *	\date 20/01/2017
 *
 */
...
/**
 *
 *	\fn void InitProgressBar (__int64 totalBytes, __int64 bytesDone, BOOL bReverse, BOOL bIOThroughput, BOOL bDisplayStatus, BOOL bShowPercent)
 *	\brief If bIOThroughput is TRUE, the speed reflects the amount of data read AND written per second (rather than
 *			the speed of the "transform cursor").
 *	\param[in] BOOL bShowPercent
 *	\param[in] BOOL bDisplayStatus
 *	\param[in] BOOL bIOThroughput
 *	\param[in] BOOL bReverse
 *	\param[in] __int64 bytesDone
 *	\param[in] __int64 totalBytes
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL UpdateProgressBar (__int64 byteOffset)
 *	\brief Call the function UpdateProgressBarProc (byteOffset)
 *	\param[in] __int64 byteOffset
 *	\return BOOL UpdateProgressBarProc (byteOffset)
 *
 */
...
/**
 *
 *	\fn BOOL UpdateProgressBarProc (__int64 byteOffset)
 *	\brief Return if the progress was cancel or not. update the progress bar depending on how much byte are treated. update the window containing the bar.
 *			Show some information about the speed and the remaining approximative remaining time.
 *	\param[in] __int64 byteOffset
 *	\return BOOL bVolTransformThreadCancel
 *
 */
