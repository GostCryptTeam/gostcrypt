 ...
 *
 *	\file Format.c
 *	\brief Manage formatting functions (formatting threads)
 *	\version 1.3.1
 *	\date 23/01/2017
 *
 */
...
/**
 *
 *	\fn uint64 GetVolumeDataAreaSize (BOOL hiddenVolume, uint64 volumeSize)
 *	\brief Get the size of volume data area
 *	\param[in] uint64 volumeSize
 *	\param[in] BOOL hiddenVolume
 *	\return uint64 0 if (volumeSize < reservedSize)
 *			volumeSize - reservedSize else
 *
 */
...
/**
 *
 *	\fn int GSTFormatVolume (volatile FORMAT_VOL_PARAMETERS *volParams)
 *	\brief Format a volume as GST
 ...
 *	\return int ERR_DONT_REPORT if (FormatSectorSize < GST_MIN_VOLUME_SECTOR_SIZE || FormatSectorSize > GST_MAX_VOLUME_SECTOR_SIZE || FormatSectorSize % ENCRYPTION_DATA_UNIT_SIZE != 0)
 *			ERR_VOL_SIZE_WRONG if (!volParams->hiddenVol && (volParams->size <= GST_TOTAL_VOLUME_HEADERS_SIZE))
 *			nStatus if (nStatus != 0)
 *			ERR_OS_ERROR if (FakeDosNameForDevice (volParams->volumePath, dosDev, devName, FALSE) != 0)
 *			nStatus goto fv_end
 *
 */
...
/**
 *
 *	\fn int FormatNoFs (unsigned __int64 startSector, __int64 num_sectors, void * dev, PCRYPTO_INFO cryptoInfo, BOOL quickFormat)
 *	\brief Seek to start sector
 ...
 *	\param[in] BOOL quickFormat
 *	\param[in] PCRYPTO_INFO cryptoInfo
 *	\param[in] void * dev
 *	\param[in] __int64 num_sectors
 *	\param[in] unsigned __int64 startSector
 *	\return int ERR_OS_ERROR if (!SetFilePointerEx ((HANDLE) dev, startOffset, &newOffset, FILE_BEGIN) || newOffset.QuadPart != startOffset.QuadPart)
 *			ERR_OUTOFMEMORY if (!write_buf)
 *			(retVal ? retVal : ERR_OS_ERROR) goto fail
 *			0 else
 *
 */
...
/**
 *
 *	\fn BOOLEAN __stdcall FormatExCallback (int command, DWORD subCommand, PVOID parameter)
 *	\brief Check FormatExResult global variable state
 *	\param[in] PVOID parameter
 *	\param[in] DWORD subCommand
 *	\param[in] int command
 *	\return BOOLEAN __stdcall TRUE
 *
 */
...
/**
 *
 *	\fn BOOL FormatNtfs (int driveNo, int clusterSize)
 *	\brief Format to NTFS
 *	\param[in] int clusterSize
 *	\param[in] int driveNo
 *	\return BOOL FALSE if (hModule == NULL)
 *			FALSE if (!(FormatEx = (PFORMATEX) GetProcAddress (GetModuleHandle ("fmifs.dll"), "FormatEx")))
 *			FormatExResult else
 *
 */
...
/**
 *
 *	\fn BOOL WriteSector (void *dev, char *sector, char *write_buf, int *write_buf_cnt, __int64 *nSecNo, PCRYPTO_INFO cryptoInfo)
 *	\brief Write sector
 *	\param[in] PCRYPTO_INFO cryptoInfo
 *	\param[in] __int64 * nSecNo
 *	\param[in] int * write_buf_cnt
 *	\param[in] char * write_buf
 *	\param[in] char * sector
 *	\param[in] void * dev
 *	\return BOOL FALSE if (*write_buf_cnt == FormatWriteBufferSize && !FlushFormatWriteBuffer (dev, write_buf, write_buf_cnt, nSecNo, cryptoInfo))
 *			FALSE if (GetTickCount () - updateTime > 25 && (UpdateProgressBar (*nSecNo * FormatSectorSize)))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static void __cdecl FormatWriteThreadProc (void *arg)
 *	\brief Manage the formatting write thread (set priority/exit/handle errors)
 *	\param[in] void * arg
...
/**
 *
 *	\fn static BOOL StartFormatWriteThread ()
 *	\brief Start the formatting write thread
 *	\return static BOOL FALSE goto err
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static void StopFormatWriteThread ()
 *	\brief Stop the formatting write thread
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL FlushFormatWriteBuffer (void *dev, char *write_buf, int *write_buf_cnt, __int64 *nSecNo, PCRYPTO_INFO cryptoInfo)
 *	\brief Flush the formatting write buffer
 *	\param[in] PCRYPTO_INFO cryptoInfo
 *	\param[in] __int64 * nSecNo
 *	\param[in] int * write_buf_cnt
 *	\param[in] char * write_buf
 *	\param[in] void * dev
 *	\return BOOL TRUE if (*write_buf_cnt == 0)
 *			FALSE if (WriteThreadRunning && (WaitForSingleObject (WriteBufferEmptyEvent, INFINITE) == WAIT_FAILED))
 *			FALSE if (WriteThreadRunning && (WriteRequestResult != ERROR_SUCCESS))
 *			FALSE if (WriteThreadRunning && (!SetEvent (WriteBufferFullEvent)))
 *			FALSE if (!WriteThreadRunning && !WriteFile ((HANDLE) dev, write_buf, *write_buf_cnt, &bytesWritten, NULL)))
 *			TRUE else
 *
 */
