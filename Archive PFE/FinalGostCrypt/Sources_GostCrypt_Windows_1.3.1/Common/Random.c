 ...
 *
 *	\file Random.c
 *	\brief Manage the requirements for using the mixPool (defined in this file) function that will add random into the crypting process.
 *	\version 1.3.1
 *	\date 20/01/2017
 *
 */
...
/**
 *
 *	\fn void RandAddInt (unsigned __int32 x)
 *	\brief Add random byte in all bytes of x value (in argument)
 ...
 *
 *	\fn int Randinit ()
 *	\brief  Init the random number generator, setup the hooks, and start the thread
 *	\return int 0 if (bRandDidInit)
 *			1 if (pRandPool == NULL && (pRandPool == NULL))
 *			1 if (hMouse == 0)
 *			1 if (!(PeriodicFastPollThreadHandle = (HANDLE) _beginthreadex (NULL, 0, PeriodicFastPollThreadProc, NULL, 0, NULL)))
 *			0 else
 *
 */
int Randinit ()
...
/**
 *
 *	\fn void RandStop (BOOL freePool)
 *	\brief Close everything down, including the thread which is closed down by
 *			setting a flag which eventually causes the thread function to exit 
 *	\param[in] BOOL freePool
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL IsRandomNumberGeneratorStarted ()
 *	\brief Return the status of RandomNumberGenerator thread
 *	\return BOOL bRandDidInit
 *
 */
...
/**
 *
 *	\fn void RandSetHashFunction (int hash_algo_id)
 *	\brief Set the hash function that will be used by the default one if none was chosen otherwise set it the the hash ID
 *	\param[in] int hash_algo_id
 *	\return void
 *
 */
...
/**
 *
 *	\fn int RandGetHashFunction (void)
 *	\brief Getter for the hash function
 *	\return int HashFunction
 *
 */
...
/**
 *
 *	\fn void SetRandomPoolEnrichedByUserStatus (BOOL enriched)
 *	\brief Set the random pool to enriched
 *	\param[in] BOOL enriched
 *	\return void
 *
 */
...
/**
 *
 *	\fn BOOL IsRandomPoolEnrichedByUser ()
 *	\brief Getter for the enriched status of the pool
 *	\return BOOL RandomPoolEnrichedByUser
 *
 */
 ...
 *
 *	\fn BOOL Randmix ()
 *	\brief The random pool mixing function. Set the digest size according to the hash,
 *			compute the digest message with the seleted hash function, mix and delete for avoiding leaks.
 *	\return BOOL TRUE
 *
 */
BOOL Randmix ()
...
/**
 *
 *	\fn void RandaddBuf (void *buf, int len)
...
/**
 *
 *	\fn BOOL RandpeekBytes (unsigned char *buf, int len)
 *	\brief Enter the critical section and copy some data of a given length.
 *	\param[in] int len
 *	\param[in] unsigned char
 *	\return BOOL FALSE if (!bRandDidInit)
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn BOOL RandgetBytes (unsigned char *buf, int len, BOOL forceSlowPoll)
 *	\brief Get len random bytes from the pool (max. RNG_POOL_SIZE bytes per a single call) 
 *	\param[in] BOOL forceSlowPoll
 *	\param[in] int len
 *	\param[in] unsigned char * buf
 *	\return BOOL FALSE if (bDidSlowPoll == FALSE || forceSlowPoll && (!SlowPoll ()))
 *			FALSE if (!FastPoll ()) 
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn LRESULT CALLBACK MouseProc (int nCode, WPARAM wParam, LPARAM lParam)
 *	\brief Capture the mouse, and as long as the event is not the same as the last
 ...
 *	\param[in] int nCode
 *	\return LRESULT CallNextHookEx (hMouse, nCode, wParam, lParam) if (nCode < 0)
 *			0 else
 *
 */
...
/**
 *
 *	\fn LRESULT CALLBACK KeyboardProc (int nCode, WPARAM wParam, LPARAM lParam)
 *	\brief Capture the keyboard, as long as the event is not the same as the last two
 ...
 *	\param[in] int nCode
 *	\return LRESULT CallNextHookEx (hMouse, nCode, wParam, lParam) if (nCode < 0)
 *			CallNextHookEx (hMouse, nCode, wParam, lParam) else
 *
 */
...
/**
 *
 *	\fn static unsigned __stdcall PeriodicFastPollThreadProc (void *dummy)
 *	\brief This is the thread function which will poll the system for randomness
 *	\param[in] void * dummy
 *	\return static unsigned __stdcall
 *
 */
...
/**
 *
 *	\fn BOOL SlowPoll (void)
 *	\brief This is the slowpoll function which gathers up network/hard drive. It is only used for NT server like : 
...
/** 
 *
 *	\fn BOOL FastPoll (void)
 *	\brief This is the fastpoll function which gathers up info by calling various api's. 
 *			It gets the system information in order to avoid problem for the processes.
 *	\return BOOL TRUE
 *
 */
BOOL FastPoll (void)
