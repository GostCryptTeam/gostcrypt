...
#define EX2MSG(exception, message) do { if (ex == typeid (exception)) return (message); } while (false)
		...
		EX2MSG (NextDeprecated, 					_("In the next version of GostCrypt, those archives won't be allowed. Please reencrypt your data with a newer container."));
		EX2MSG (OlderVersionRequired, 				_("The archive you are trying to open is no longer supported. Please open it in an previous version of GostCrypt and reencrypt the data in a new container."));
