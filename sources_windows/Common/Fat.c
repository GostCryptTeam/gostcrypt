...
/**
 *
 *	\fn void GetFatParams (fatparams * ft)
 *	\brief Get FAT parameters
 *	\param[in] fatparams * ft
 *	\return void
 *
 */
void
...
/**
 *
 *	\fn void PutBoot (fatparams * ft, unsigned char *boot)
 *	\brief Put boot procedure
 *	\param[in] unsigned char * boot
 *	\param[in] fatparams * ft
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void PutFSInfo (unsigned char *sector, fatparams *ft)
 *	\brief Put FAT32 FSInfo
 *	\param[in] fatparams * ft
 *	\param[in] unsigned char * sector
 *	\return static void
 *
 */
...
/**
 *
 *	\fn int FormatFat (unsigned __int64 startSector, fatparams * ft, void * dev, PCRYPTO_INFO cryptoInfo, BOOL quickFormat)
 *	\brief Format to FAT
 ...
 *	\return int ERR_VOL_SEEKING if (!SetFilePointerEx ((HANDLE) dev, startOffset, &newOffset, FILE_BEGIN) || newOffset.QuadPart != startOffset.QuadPart)
 *			ERR_OUTOFMEMORY if (!write_buf)
 *			retVal if (retVal != ERR_SUCCESS)
 *			ERR_MODE_INIT_FAILED if (!EAInitMode (cryptoInfo))
 *			ERR_OS_ERROR goto fail
 *			0 else
 *
 */
int
