 ...
 *
 *	\file Keyfiles.c
 *	\brief Manage a window that allow to create a file a directory or a token file.
 *			They will load into a list that can be used to point to the file on the computer.
 *			then the file created can be read to load the file parameters.
 *	\version 1.3.1
 *	\date 23/01/2017
 *
 */
...
/**
 *
 *	\fn KeyFile *KeyFileAdd (KeyFile *firstKeyFile, KeyFile *keyFile)
 *	\brief return the last keyfile of the keyfile list or and create the next at NULL 
 *	\param[in] KeyFile * keyFile
 *	\param[in] KeyFile * firstKeyFile
 *	\return KeyFile firstKeyFile
 *
 */
...
/**
 *
 *	\fn static KeyFile *KeyFileRemove (KeyFile *firstKeyFile, KeyFile *keyFile)
 *	\brief Returns first keyfile, NULL if last keyfile was removed
 *	\param[in] KeyFile * keyFile
 *	\param[in] KeyFile * firstKeyFile
 *	\return static KeyFile firstKeyFile
 *
 */
...
/**
 *
 *	\fn void KeyFileRemoveAll (KeyFile **firstKeyFile)
 *	\brief Remove all the keyfiles in the list.
 *	\param[in] KeyFile ** firstKeyFile
 *	\return void
 *
 */
...
/**
 *
 *	\fn KeyFile *KeyFileClone (KeyFile *keyFile)
 *	\brief Clone the keyfile in parameter
 *	\param[in] KeyFile * keyFile
 *	\return KeyFile
 *
 */
...
/**
 *
 *	\fn KeyFile *KeyFileCloneAll (KeyFile *firstKeyFile)
 *	\brief Clone all the keyfiles of the list
 *	\param[in] KeyFile * firstKeyFile
 *	\return KeyFile * cloneFirstKeyFile
 *
 */
...
/**
 *
 *	\fn static BOOL KeyFileProcess (unsigned __int8 *keyPool, KeyFile *keyFile)
 *	\brief Read the keyfile name and add in the keypool the UPDC32 of the crc and the character read in the file.
 *	\param[in] KeyFile * keyFile
 *	\param[in] unsigned __int8
 *	\return static BOOL status TRUE
 *	while ((bytesRead = fread (buffer, 1, sizeof (buffer), f)) > 0)
 *		FALSE if (ferror (f))
 *	FALSE if (ferror (f))
 *	FALSE else if (totalRead == 0)
 *
 */
...
/**
 *
 *	\fn BOOL KeyFilesApply (Password *password, KeyFile *firstKeyFile)
 *	\brief apply a security token keyfile, verify if it is a path or a file, If it is a folder,
 *			it finds and processes of the keyfile in the directory.
 *			the function skip the hidden files.
 *			apply keyfile to the pool.
 *			Mix the keyfile pool contents into the password 
 *	\param[in] KeyFile * firstKeyFile
 *	\param[in] Password * password
 *	\return BOOL FALSE if (keyfileData.empty())
 *			FALSE if (stat (kf->FileName, &statStruct) != 0)
 *			FALSE if ((searchHandle = _findfirst(searchPath, &fBuf)) == -1)
 *			FALSE if (!KeyFileProcess(keyPool, kfSub))
 *			FALSE if (keyfileCount == 0)
 *			FALSE if (stat(kfSub->FileName, &statStruct) != 0)
 *			FALSE else if (!KeyFileProcess (keyPool, kf))
 *			TRUE else
 *
 */
...
/**
 *
 *	\fn static void LoadKeyList (HWND hwndDlg, KeyFile *firstKeyFile)
 *	\brief Load the parameters of the firstKeyFile to display them on the GUI interface.
 *	\param[in] KeyFile * firstKeyFile
...
/**
 *
 *	\fn BOOL CALLBACK KeyFilesDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief manage theGUI interface for the keyFiles
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT msg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 if a command known is pressed
 *			0 otherwise
 *
 */
...
/**
 *
 *	\fn BOOL KeyfilesPopupMenu (HWND hwndDlg, POINT popupPosition, KeyFilesDlgParam *param)
 *	\brief Show a popup menu that return True if a action have to be done, False otherwise.
 *			The commands are :
 *			Add file , Add directory, add token files
 *	\param[in] KeyFilesDlgParam * param
 *	\param[in] POINT popupPosition
 *	\param[in] HWND hwndDlg
 *	\return BOOL switch(sel)
 *				case IDM_KEYFILES_POPUP_ADD_FILES status = TRUE if (SelectMultipleFiles (hwndDlg, "SELECT_KEYFILE", kf->FileName, bHistory))
 *				case IDM_KEYFILES_POPUP_ADD_DIR status = TRUE if (BrowseDirectories (hwndDlg,"SELECT_KEYFILE_PATH", kf->FileName))
 *				case IDM_KEYFILES_POPUP_ADD_TOKEN_FILES status = TRUE if (DialogBoxParamW (hInst, MAKEINTRESOURCEW (IDD_TOKEN_KEYFILES), hwndDlg, (DLGPROC) SecurityTokenKeyfileDlgProc, (LPARAM) &selectedTokenKeyfiles) == IDOK)
 *			status else
 *
 */
