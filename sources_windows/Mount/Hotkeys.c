 ...
 *
 *	\file Hotkeys.c
 *	\brief Manage the interface for the hotkey assigment and resolv the hotkeys when pressed
 *	\version 1.3.1
 *	\date 14/12/2016
 *
 */
...
/**
 *
 *	\fn static void ScanAndProcessKey (UINT *vKeyCode, wchar_t *keyName)
 *	\brief Scan in the key pressed asynchronously and if the key can be 
			resolv as an action in the interface set the vKetCode variable to the resolved key
 *	\param[in] wchar_t * keyName
 *	\param[in] UINT * vKeyCode
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL GetKeyName (UINT vKey, wchar_t *keyName)
 *	\brief Returns TRUE if the key is allowed and its name is resolved. 
 *	\param[in] wchar_t * keyName
 *	\param[in] UINT vKey
 *	\return BOOL result
 *
...
/**
 *
 *	\fn static BOOL ShortcutInUse (UINT vKeyCode, UINT modifiers, GSTHOTKEY hotkeys[])
 *	\brief Return true if the vKeyCode variable exists in hotkeys table
 *	\param[in] GSTHOTKEY hotkeys[]
 *	\param[in] UINT modifiers
 *	\param[in] UINT vKeyCode
 *	\return static BOOL TRUE if (hotkeys[i].vKeyCode == vKeyCode && hotkeys[i].vKeyModifiers == modifiers)
 *			FALSE else
 *
 */
...
/**
 *
 *	\fn void UnregisterAllHotkeys (HWND hwndDlg, GSTHOTKEY hotkeys[])
 *	\brief Unregistr=er all the hotkeys in the hotkey table.
 *	\param[in] GSTHOTKEY hotkeys[]
...
/**
 *
 *	\fn BOOL RegisterAllHotkeys (HWND hwndDlg, GSTHOTKEY hotkeys[])
 *	\brief Register all hotkeys in the hotkeys table.
 *	\param[in] GSTHOTKEY hotkeys[]
 *	\param[in] HWND hwndDlg
 *	\return BOOL result
 *
...
/**
 *
 *	\fn static void DisplayHotkeyList (HWND hwndDlg)
 *	\brief Display the hotkeys in the hotkeys table.
 *	\param[in] HWND hwndDlg
 *	\return static void
 *
 */
...
/**
 *
 *	\fn BOOL CALLBACK HotkeysDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
 *	\brief Manage the window of hotkeys for assigning new one, seeing the existing one or deleting.
 ...
 *			1 switch (msg) case WM_COMMAND if (msg == WM_NOTIFY && wParam == IDC_HOTKEY_LIST)
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_HOTKEY_ASSIGN)
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_HOTKEY_REMOVE)
 *			1 switch (msg) case WM_COMMAND if (lw == IDC_RESET_HOTKEYS)
 *			1 switch (msg) case WM_COMMAND if (lw == IDCANCEL || lw == IDCLOSE)
 *			1 switch (msg) case WM_COMMAND if (lw == IDOK)
 *			0 switch (msg) case WM_COMMAND else
 *			1 switch (msg) case WM_NOTIFY if (msg == WM_NOTIFY && wParam == IDC_HOTKEY_LIST)
 *			1 switch (msg) case WM_NOTIFY if (lw == IDC_HOTKEY_ASSIGN)
 *			1 switch (msg) case WM_NOTIFY if (lw == IDC_HOTKEY_REMOVE)
 *			1 switch (msg) case WM_NOTIFY if (lw == IDC_RESET_HOTKEYS)
 *			1 switch (msg) case WM_NOTIFY if (lw == IDCANCEL || lw == IDCLOSE)
 *			1 switch (msg) case WM_NOTIFY if (lw == IDOK)
