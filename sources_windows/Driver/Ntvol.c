 ...
 *
 *	\file Ntvol.c
 *	\brief contains functions that open a volume / close a volume, and update the accessed time.
 *	\version 1.3.1
 *	\date 13/12/2016
 *
 */
...
/**
 *
 *	\fn GSTOpenVolume (PDEVICE_OBJECT DeviceObject, PEXTENSION Extension, MOUNT_STRUCT *mount, PWSTR pwszMountVolume, BOOL bRawDevice)
 *	\brief Query the size of the device to open. Open the device and return error messages. Then check the file format we want to open
 *	(NTFS / FAT ...) then check volume size and type (hidden ...) and read the header. Attempt to decrypt the header.
 *	Decrypt the sectors. Protect the hidden volume (don't decrypt them).
 *	\param[in] BOOL bRawDevice
 *	\param[in] PWSTR pwszMountVolume
...
/**
 *
 *	\fn void GSTCloseVolume (PDEVICE_OBJECT DeviceObject, PEXTENSION Extension)
 *	\brief Close the volume decrypted thanks to the previous function
 *	\param[in] PEXTENSION Extension
 *	\param[in] PDEVICE_OBJECT DeviceObject
 *	\return void
 *
...
/**
 *
 *	\fn void GSTSendHostDeviceIoControlRequest (PDEVICE_OBJECT DeviceObject, PEXTENSION Extension, ULONG IoControlCode, void *OutputBuffer, ULONG OutputBufferSize)
 *	\brief Call the driver with an irp to return the block status
 *	\param[in] PDEVICE_OBJECT DeviceObject
 *	\param[in] PEXTENSION Extension
...
/**
*
*	\fn void COMPLETE_IRP (PDEVICE_OBJECT DeviceObject, PIRP Irp, NTSTATUS IrpStatus, ULONG_PTR IrpInformation)
*	\brief Complete the IRP with data (extra info provide the adresses completed)
*	\param[in] PDEVICE_OBJECT DeviceObject
*	\param[in] PIRP Irp
*	\param[in] NTSTATUS IrpStatus
*	\param[in] ULONG_PTR IrpInformation
*	\return NTSTATUS IrpStatus
*
*/
...
/**
 *
 *	\fn static void RestoreTimeStamp (PEXTENSION Extension)
 *	\brief When accessing a file set the accessed / modified time and set up to the actual time.
 *	\param[in] PEXTENSION Extension
 *	\return static void
 *
 */
