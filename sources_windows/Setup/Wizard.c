...
/**
 *
 *	\fn void localcleanupwiz (void)
 *	\brief Clean up local allocations (bitmaps...)
 *	\return void
 *
 */
...
/**
 *
 *	\fn static void InitWizardDestInstallPath (void)
 *	\brief Initialize the install path wizard
 *	\return static void
 *
 */
...
/**
 *
 *	\fn void LoadPage (HWND hwndDlg, int nPageNo)
 *	\brief Load a page
...
/**
 *
 *	\fn static int GetDonVal (int minVal, int maxVal)
 *	\brief Get don value
 *	\param[in] int maxVal
 *	\param[in] int minVal
 *	\return static int ((int) ((double) *((uint16 *) buffer) / (0xFFFF+1) * (maxVal + 1 - minVal)) + minVal) if (OsPrngAvailable && CryptGenRandom (hCryptProv, sizeof (buffer), buffer) != 0) 
 *			((int) ((double) rand() / (RAND_MAX+1) * (maxVal + 1 - minVal)) + minVal) else
 *
 */
 ...
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 1 switch (nCurPageNo) case INTRO_PAGE
 *			1 switch (nCurPageNo) case WIZARD_MODE_PAGE
 *			1 switch (nCurPageNo) case EXTRACTION_OPTIONS_PAGE
 *			1 switch (nCurPageNo) case EXTRACTION_PROGRESS_PAGE
 *			1 switch (nCurPageNo) case INSTALL_OPTIONS_PAGE
 *			1 switch (nCurPageNo) case INSTALL_PROGRESS_PAGE
 *			1 switch (nCurPageNo) case DONATIONS_PAGE
 *			0 switch (uMsg) case WM_INITDIALOG  if switch (nCurPageNo) didnt return 1
 *			1 switch (uMsg) case WM_HELP
 *			0 switch (uMsg) case WM_ENDSESSION
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_AGREE && nCurPageNo == INTRO_PAGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_WIZARD_MODE_EXTRACT_ONLY && nCurPageNo == WIZARD_MODE_PAGE)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_WIZARD_MODE_INSTALL && nCurPageNo == WIZARD_MODE_PAGE)
 *			1 switch (uMsg) case WM_COMMAND if ( nCurPageNo == EXTRACTION_OPTIONS_PAGE && hw == EN_CHANGE )
 *			1 switch (uMsg) case WM_COMMAND if ( nCurPageNo == INSTALL_OPTIONS_PAGE && hw == EN_CHANGE )
 *			1 switch (uMsg) case WM_COMMAND switch (lw)
 *			0 switch (uMsg) case WM_COMMAND if switch (lw) didnt return 1
 *			0 switch (uMsg) case WM_PAINT
 *			((LONG)(HBRUSH)(GetStockObject(NULL_BRUSH))) switch (iMsg) case WM_CTLCOLORSTATIC		
 *			0 switch (uMsg) case WM_ERASEBKGND 0
 *
 */
...
/**
 *
 *	\fn BOOL UpdateProgressBarProc (int nPercent)
 *	\brief Update the Proc progress bar
 *			This function must always return TRUE
 *	\param[in] int nPercent)
 *	\return BOOL TRUE
 *
 */
 ...
 *			should return nonzero if it processes the message, and zero if it does
 *			not. - see DialogProc
 *			CALLBACK : window management, called when a button is pressed or the window receives some directive from the program (example : extraction finished)
 *	\param[in] LPARAM lParam
 *	\param[in] WPARAM wParam
 *	\param[in] UINT uMsg
 *	\param[in] HWND hwndDlg
 *	\return BOOL 0 switch (uMsg) case WM_INITDIALOG
 *			1 switch (uMsg) case WM_SYSCOMMAND if (lw == IDC_ABOUT)
 *			0 switch (uMsg) case WM_SYSCOMMAND else
 *			1 switch (uMsg) case WM_HELP
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDHELP)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDCANCEL)
 *			1 switch (uMsg) case WM_COMMAND if (lw == ID_NEXT)
 *			1 switch (uMsg) case WM_COMMAND if (lw == IDC_PREV)
 *			0 switch (uMsg) case WM_COMMAND else
 *			0 switch (uMsg) case WM_PAINT
 *			((LONG) (HBRUSH) (GetStockObject (NULL_BRUSH))) switch (uMsg) case WM_CTLCOLORSTATIC if ((HWND) lParam != GetDlgItem (MainDlg, IDC_MAIN_CONTENT_CANVAS))
 *			0 switch (uMsg) case WM_ERASEBKGND
 *			1 switch (uMsg) case GST_APPMSG_INSTALL_SUCCESS
 *			1 switch (uMsg) case GST_APPMSG_INSTALL_FAILURE
 *			1 switch (uMsg) case GST_APPMSG_EXTRACTION_SUCCESS
 *			1 switch (uMsg) case GST_APPMSG_EXTRACTION_FAILURE
 *			1 switch (uMsg) case WM_CLOSE if (!bDevm)
 *			0 switch (uMsg) case WM_CLOSE else
 *
 */
