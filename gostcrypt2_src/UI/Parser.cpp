...
void Parser::parseMount(QCommandLineParser& parser,
                        QSharedPointer <GostCrypt::Core::MountVolumeRequest> options)
{
    parser.addPositionalArgument("mount", "Mounts a volume.", "mount");
    parser.addPositionalArgument("volumepath", "Path of the volume or the device to mount.",
                                 "volumepath");
    parser.addOptions(
    {
    ...
    if (parser.isSet("no-filesystem"))   // nofilesystem
    {
        ...
        if (parser.isSet("filesystem") || parser.isSet("options") || parser.isSet("protection"))
        {
            throw Parser::ParseException(
                QString("--nofilesystem cannot be used with --filesystem, --protection or --options."));
        }
    }
    else
    {
...
void Parser::parseDismount(QCommandLineParser& parser,
                           QSharedPointer <GostCrypt::Core::DismountVolumeRequest> volume)
{
...
void Parser::parseCreate(QCommandLineParser& parser,
                         QSharedPointer <GostCrypt::Core::CreateVolumeRequest> options)
{
    parser.addPositionalArgument("create", "Creates a volume.", "create");
    parser.addPositionalArgument("volumepath", "Path of the volume to create",
                                 "path"); // TODO add default values to description
    parser.addOptions(
    {
    ...
    if (options->type != GostCrypt::Volume::VolumeType::Hidden)
    {
        ...
        {
            throw Parser::ParseException(
                QString("Options for hidden volumes should only be used with --type=Hidden"));
        }
    }
    else
    {
...
void Parser::parseCreateKeyFiles(QCommandLineParser& parser, QStringList& files)
{
    ...
    {
        throw Parser::ParseException(
            QString("Please Specify the path of at least one keyfile to create."));
    }
    if (positionalArguments.size() >= 2)
...
void Parser::parseBenchmark(QCommandLineParser& parser,
                            QSharedPointer<GostCrypt::Core::BenchmarkAlgorithmsRequest> options)
{

    parser.addPositionalArgument("benchmark",
                                 "execute the benchmark of the differents cipher algorithms", "benchmark");
