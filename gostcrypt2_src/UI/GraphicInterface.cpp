...
QString GraphicInterface::formatSize(quint64 sizeInByte, bool choice)
{
    return UserInterface::formatSize(sizeInByte, choice);
}
    ...
    case UI::benchmark: //"benchmark"
        {
            ...
            convertedSize = Parser::parseSize(size, &ok);
            if(ok)
            {
