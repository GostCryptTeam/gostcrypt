...
void Volume::CheckProtectedRange(quint64 writeHostOffset, quint64 writeLength)
{
    ...
    if ((writeHostOffset < ProtectedRangeStart) ? (writeHostEndOffset >= ProtectedRangeStart) :
            (writeHostOffset <= ProtectedRangeEnd - 1))
    {
                  ...
                  bool useBackupHeaders)
{
    ...
    if (protection == VolumeProtection::HiddenVolumeReadOnly)
    {
        ...
        try
        {
            ...
            {
                throw InvalidParameterException("password/keyfiles",
                                                "The given password/keyfiles to open the inner hidden volume instance are the ones of the outer volume instance");
            }
    ...
    try
    {
        ...
        for (QSharedPointer <VolumeLayout> layout : VolumeLayout::GetAvailableLayouts(volumeType))
        {
            ...
            if (header->Decrypt(headerBuffer, *passwordKey, layout->GetSupportedKeyDerivationFunctions(),
                                layoutEncryptionAlgorithms))
            {
                // Header decrypted
                ...
                if (Protection == VolumeProtection::HiddenVolumeReadOnly)
                {
                    if (Type == VolumeType::Hidden)
                    {
                        throw PasswordOrKeyfilesIncorrectException(
                            volumePath);    // the password of the inner volume was put instead of the one of the outer volume.
                    }
...
void Volume::ReEncryptHeader(bool backupHeader, const BufferPtr& newSalt,
