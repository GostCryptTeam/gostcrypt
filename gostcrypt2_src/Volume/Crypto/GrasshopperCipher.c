...
void grasshopper_set_key(unsigned long long* master_key, grasshopper_kds* subkeys)
{
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        L(blocktmp);
        c_values[i][0] = blocktmp[0];
        c_values[i][1] = blocktmp[1];
    }

    /* Change endian */

    /* Compute subkeys K_1 and K_2  */
    subkeys->x00 =  master_key[0];
    subkeys->x01 =  master_key[1];
    subkeys->x10 =  master_key[2];
    subkeys->x11 =  master_key[3];

    changeEndian(&subkeys->x00);
    changeEndian(&subkeys->x01);
    changeEndian(&subkeys->x10);
    changeEndian(&subkeys->x11);

    /* Compute subkeys K_3 and K_4  */
    blocktmp[0] = subkeys->x00;
    blocktmp[1] = subkeys->x01;
    blocktmp2[0] = subkeys->x10;
    blocktmp2[1] = subkeys->x11;
    F(c_values[0], blocktmp, blocktmp2);
    F(c_values[1], blocktmp, blocktmp2);
    F(c_values[2], blocktmp, blocktmp2);
    F(c_values[3], blocktmp, blocktmp2);
    F(c_values[4], blocktmp, blocktmp2);
    F(c_values[5], blocktmp, blocktmp2);
    F(c_values[6], blocktmp, blocktmp2);
    F(c_values[7], blocktmp, blocktmp2);

    subkeys->x20 = blocktmp[0];
    subkeys->x21 = blocktmp[1];
    subkeys->x30 = blocktmp2[0];
    subkeys->x31 = blocktmp2[1];

    /* Compute subkeys K_5 and K_6  */
    F(c_values[ 8], blocktmp, blocktmp2);
    F(c_values[ 9], blocktmp, blocktmp2);
    F(c_values[10], blocktmp, blocktmp2);
    F(c_values[11], blocktmp, blocktmp2);
    F(c_values[12], blocktmp, blocktmp2);
    F(c_values[13], blocktmp, blocktmp2);
    F(c_values[14], blocktmp, blocktmp2);
    F(c_values[15], blocktmp, blocktmp2);

    subkeys->x40 = blocktmp[0];
    subkeys->x41 = blocktmp[1];
    subkeys->x50 = blocktmp2[0];
    subkeys->x51 = blocktmp2[1];

    /* Compute subkeys K_7 and K_8  */
    F(c_values[16], blocktmp, blocktmp2);
    F(c_values[17], blocktmp, blocktmp2);
    F(c_values[18], blocktmp, blocktmp2);
    F(c_values[19], blocktmp, blocktmp2);
    F(c_values[20], blocktmp, blocktmp2);
    F(c_values[21], blocktmp, blocktmp2);
    F(c_values[22], blocktmp, blocktmp2);
    F(c_values[23], blocktmp, blocktmp2);

    subkeys->x60 = blocktmp[0];
    subkeys->x61 = blocktmp[1];
    subkeys->x70 = blocktmp2[0];
    subkeys->x71 = blocktmp2[1];

    /* Compute subkeys K_9 and K_{10} */
    F(c_values[24], blocktmp, blocktmp2);
    F(c_values[25], blocktmp, blocktmp2);
    F(c_values[26], blocktmp, blocktmp2);
    F(c_values[27], blocktmp, blocktmp2);
    F(c_values[28], blocktmp, blocktmp2);
    F(c_values[29], blocktmp, blocktmp2);
    F(c_values[30], blocktmp, blocktmp2);
    F(c_values[31], blocktmp, blocktmp2);

    subkeys->x80 = blocktmp[0];
    subkeys->x81 = blocktmp[1];
    subkeys->x90 = blocktmp2[0];
    subkeys->x91 = blocktmp2[1];
                         ...
                         unsigned long long* ciphertext)
{
    ...
    changeEndian(&plaintext[0]);
    changeEndian(&plaintext[1]);

    ciphertext[0] = plaintext[0] ^ subkeys->x00;
    ciphertext[1] = plaintext[1] ^ subkeys->x01;
    S(ciphertext);
    L(ciphertext);

    /* Round 2 with subkey 2 */
    ciphertext[0] ^= subkeys->x10;
    ciphertext[1] ^= subkeys->x11;
    S(ciphertext);
    L(ciphertext);

    /* Round 3 with subkey 3 */
    ciphertext[0] ^= subkeys->x20;
    ciphertext[1] ^= subkeys->x21;
    S(ciphertext);
    L(ciphertext);

    /* Round 4 with subkey 4 */
    ciphertext[0] ^= subkeys->x30;
    ciphertext[1] ^= subkeys->x31;
    S(ciphertext);
    L(ciphertext);

    /* Round 5 with subkey 5 */
    ciphertext[0] ^= subkeys->x40;
    ciphertext[1] ^= subkeys->x41;
    S(ciphertext);
    L(ciphertext);

    /* Round 6 with subkey 6 */
    ciphertext[0] ^= subkeys->x50;
    ciphertext[1] ^= subkeys->x51;
    S(ciphertext);
    L(ciphertext);

    /* Round 7 with subkey 7 */
    ciphertext[0] ^= subkeys->x60;
    ciphertext[1] ^= subkeys->x61;
    S(ciphertext);
    L(ciphertext);

    /* Round 8 with subkey 8 */
    ciphertext[0] ^= subkeys->x70;
    ciphertext[1] ^= subkeys->x71;
    S(ciphertext);
    L(ciphertext);

    /* Round 9 with subkey 9 */
    ciphertext[0] ^= subkeys->x80;
    ciphertext[1] ^= subkeys->x81;
    S(ciphertext);
    L(ciphertext);

    /* Round 10 with subkey 10 */
    ciphertext[0] ^= subkeys->x90;
    ciphertext[1] ^= subkeys->x91;

    changeEndian(&ciphertext[0]);
    changeEndian(&ciphertext[1]);
                         ...
                         unsigned long long* plaintext)
{
    ...
    plaintext[0] = ciphertext[0] ^ subkeys->x90;
    plaintext[1] = ciphertext[1] ^ subkeys->x91;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 2 with subkey 9 */
    plaintext[0] ^= subkeys->x80;
    plaintext[1] ^= subkeys->x81;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 3 with subkey 8 */
    plaintext[0] ^= subkeys->x70;
    plaintext[1] ^= subkeys->x71;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 4 with subkey 7 */
    plaintext[0] ^= subkeys->x60;
    plaintext[1] ^= subkeys->x61;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 5 with subkey 6 */
    plaintext[0] ^= subkeys->x50;
    plaintext[1] ^= subkeys->x51;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 6 with subkey 5 */
    plaintext[0] ^= subkeys->x40;
    plaintext[1] ^= subkeys->x41;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 7 with subkey 4 */
    plaintext[0] ^= subkeys->x30;
    plaintext[1] ^= subkeys->x31;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 8 with subkey 3 */
    plaintext[0] ^= subkeys->x20;
    plaintext[1] ^= subkeys->x21;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 9 with subkey 2 */
    plaintext[0] ^= subkeys->x10;
    plaintext[1] ^= subkeys->x11;
    L_inv(plaintext);
    S_inv(plaintext);

    /* Round 10 with subkey 1 */
    plaintext[0] ^= subkeys->x00;
    plaintext[1] ^= subkeys->x01;
                        ...
                        unsigned int len)
{
    ...
    out_ks->x00 = ks->x00 ^ in[0];
    out_ks->x01 = ks->x90 ^ in[0];
    out_ks->x10 = ks->x10 ^ in[0];
    out_ks->x11 = ks->x90 ^ in[0];
    out_ks->x20 = ks->x20 ^ in[0];
    out_ks->x21 = ks->x90 ^ in[0];
    out_ks->x30 = ks->x30 ^ in[0];
    out_ks->x31 = ks->x90 ^ in[0];
    out_ks->x40 = ks->x40 ^ in[0];
    out_ks->x41 = ks->x90 ^ in[0];
    out_ks->x50 = ks->x50 ^ in[0];
    out_ks->x51 = ks->x90 ^ in[0];
    out_ks->x60 = ks->x60 ^ in[0];
    out_ks->x61 = ks->x90 ^ in[0];
    out_ks->x70 = ks->x70 ^ in[0];
    out_ks->x71 = ks->x90 ^ in[0];
    out_ks->x80 = ks->x80 ^ in[0];
    out_ks->x81 = ks->x90 ^ in[0];
    out_ks->x90 = ks->x90 ^ in[0];
    out_ks->x91 = ks->x90 ^ in[0];
    for (i = 1; i < len; i++)
    {
        out_ks->x00 = ks->x00 ^ in[i];
        out_ks->x01 = ks->x90 ^ in[i];
        out_ks->x10 = ks->x10 ^ in[i];
        out_ks->x11 = ks->x90 ^ in[i];
        out_ks->x20 = ks->x20 ^ in[i];
        out_ks->x21 = ks->x90 ^ in[i];
        out_ks->x30 = ks->x30 ^ in[i];
        out_ks->x31 = ks->x90 ^ in[i];
        out_ks->x40 = ks->x40 ^ in[i];
        out_ks->x41 = ks->x90 ^ in[i];
        out_ks->x50 = ks->x50 ^ in[i];
        out_ks->x51 = ks->x90 ^ in[i];
        out_ks->x60 = ks->x60 ^ in[i];
        out_ks->x61 = ks->x90 ^ in[i];
        out_ks->x70 = ks->x70 ^ in[i];
        out_ks->x71 = ks->x90 ^ in[i];
        out_ks->x80 = ks->x80 ^ in[i];
        out_ks->x81 = ks->x90 ^ in[i];
        out_ks->x90 = ks->x90 ^ in[i];
        out_ks->x91 = ks->x90 ^ in[i];
    }
}
