...
namespace Volume {
	...
	protected:
        ...
        template <typename T> static T DeserializeEntry (const ConstBufferPtr &header, size_t &offset);
        template <typename T> static T DeserializeEntryAt (const ConstBufferPtr &header, const size_t &offset);
		void Init ();
		void Serialize (const BufferPtr &header) const;
        template <typename T> static void SerializeEntry (const T &entry, const BufferPtr &header, size_t &offset);
