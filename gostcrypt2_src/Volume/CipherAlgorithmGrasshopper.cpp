...
{
    grasshopper_decrypt ((grasshopper_kds*) ScheduledKey.Get(), (gst_ludword *) data, (gst_ludword *) data);
}

void CipherAlgorithmGrasshopper::Encrypt (quint8 *data) const
{
    grasshopper_encrypt ((grasshopper_kds*) ScheduledKey.Get(), (gst_ludword *) data, (gst_ludword *) data);
}
...
{
    grasshopper_set_key((gst_ludword*)key, (grasshopper_kds *) ScheduledKey.Get());
}

void CipherAlgorithmGrasshopper::XorCipherKey (quint8 *ks, quint8 *data, int len) const
{
    grasshopper_xor_ks((grasshopper_kds *) ScheduledKey.Get(), (grasshopper_kds *) ks, (gst_ludword *)data, len / sizeof(unsigned long long));
}
