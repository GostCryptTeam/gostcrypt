...
class Service : public QObject
{
 ...
 private:
    ...
    QDataStream
    inputStream; /**< QDataStream corresponding to the inputFile used to deserialize requests */
    QDataStream
    outputStream; /**< QDataStream corresponding to the outputFile used t serialize responses */
    QSharedPointer<QSocketNotifier>
    inputFileMonitor; /**< Pointer to QSocketNotifier used in async mode to detect when there is new data to read in the inputFile */
...
class UnknowRequest : public GostCryptException
{
 public:
    ...
    */
    UnknowRequest(QString fonction, QString filename, quint32 line,
                  const char* requestTypeName) : GostCryptException(fonction, filename, line),
        requestTypeName(requestTypeName) {}
    DEF_EXCEPTION_WHAT(UnknowRequest, GostCryptException,
                       "Child process received an unknown request (" + requestTypeName + ")")
 protected:
...
class UnknowResponse : public GostCryptException
{
 public:
    ...
    */
    UnknowResponse(QString fonction, QString filename, quint32 line,
                   const char* responseTypeName) : GostCryptException(fonction, filename, line),
        responseTypeName(responseTypeName) {}
    DEF_EXCEPTION_WHAT(UnknowResponse, GostCryptException,
                       "Unknow reponse received from child process (" + responseTypeName + ")")
 protected:
...
struct ProgressUpdateResponse : CoreResponse
{
     ...
     */
    ProgressUpdateResponse(quint32 requestId, qreal progress) : requestId(requestId),
        progress(progress) {}
