    ...
    try
    {
        ...
        try
        {
            ...
            QList<QSharedPointer<Volume::VolumeInformation>> volumeInfoList =
                        getMountedVolumesResponse->volumeInfoList;
...
void CoreRoot::writeHeaderToFile(std::fstream& file,
                                 QSharedPointer<CreateVolumeRequest::VolumeParams> params,
                                 QSharedPointer<Volume::VolumeLayout> layout, quint64 containerSize)
{
    ...
    QSharedPointer<GostCrypt::Volume::EncryptionAlgorithm> ea(getEncryptionAlgorithm(
    ...
    {
        throw InvalidParameterException("params->size",
                                        "Primary volume should always use 100% of the container");
    }
    ...
    else
    {
        if ((int64)layout->GetHeaderSize() + layout->GetHeaderOffset() < 0)
            throw InvalidParameterException("Header size and header offset",
                                            "Header size (" + QString::number(layout->GetHeaderSize()) + ") not compatible with header offset ("
                                            +
    ...
    file.write((char*)headerBuffer.get(),
               headerBuffer.size());  // writing backup header crypted with new salt

}

void CoreRoot::mountFormatVolume(QFileInfo volume, QSharedPointer<QByteArray> password,
                                 ...
                                 QSharedPointer<CreateVolumeResponse> parentResponse, ProgressTrackingParameters id)
{
    ...
    mountparams->passThrough.insert("volumeCreationFormatId",
                                    QVariant::fromValue(ProgressTrackingParameters(id, 0.25, 1.0)));
...
void CoreRoot::continueMountFormat(QSharedPointer<MountVolumeResponse> mountResponse)
{
    ...
    ProgressTrackingParameters id =
        mountResponse->passThrough.value("volumeCreationFormatId").value<ProgressTrackingParameters>();
    ...
    dismountparams->passThrough.insert("parentParams",
                                       mountResponse->passThrough.value("parentParams"));
    dismountparams->passThrough.insert("parentResponse",
                                       mountResponse->passThrough.value("parentResponse"));
...
{
    emit formatVolumeDone(
        dismountResponse->passThrough.value("parentParams").value<QSharedPointer<CreateVolumeRequest>>(),
        dismountResponse->passThrough.value("parentResponse").value<QSharedPointer<CreateVolumeResponse>>());
}
...
void CoreRoot::createVolume(QSharedPointer<CreateVolumeRequest> params)
{
    ...
    try
    {
        ...
        */
        ...
        createRandomFile(params->path, params->size, fillFileId, params->outerVolume->encryptionAlgorithm,
                         false); // no random to create the file faster.

        // opening file (or device)
        volumefile.open(params->path.absoluteFilePath().toStdString(),
        ...
        innerlayout.reset(new
                          Volume::VolumeLayoutV2Hidden()); // we ALWAYS have a hidden volume header, it can just be a fake one
        ...
        {
            connect(this, SIGNAL(formatVolumeDone(QSharedPointer<CreateVolumeRequest>,
                                                  QSharedPointer<CreateVolumeResponse>)), this,
                    SLOT(continueFormatHidden(QSharedPointer<CreateVolumeRequest>,
        ...
        else
        {
            connect(this, SIGNAL(formatVolumeDone(QSharedPointer<CreateVolumeRequest>,
                                                  QSharedPointer<CreateVolumeResponse>)), this,
                    SLOT(finishCreateVolume(QSharedPointer<CreateVolumeRequest>,
                                            QSharedPointer<CreateVolumeResponse>)));
            mountFormatId = ProgressTrackingParameters(params->id, 0.65, 1.0);
        }

        mountFormatVolume(params->path, params->outerVolume->password, params->outerVolume->keyfiles,
                          params->outerVolume->filesystem, params, response, mountFormatId);
...
void CoreRoot::continueFormatHidden(QSharedPointer<CreateVolumeRequest> params,
                                    QSharedPointer<CreateVolumeResponse> response)
{
    try
    {
        ...
        connect(this, SIGNAL(formatVolumeDone(QSharedPointer<CreateVolumeRequest>)), this,
                SLOT(finishCreateVolume(QSharedPointer<CreateVolumeRequest>)));
        updateProgress(0.95, params->id);
        mountFormatVolume(params->path, params->innerVolume->password, params->innerVolume->keyfiles,
                          params->innerVolume->filesystem, params, response, ProgressTrackingParameters(params->id, 0.825,
                                  1.0));
        updateProgress(1, params->id);
    }
    catch (GostCryptException& e)
...
void CoreRoot::finishCreateVolume(QSharedPointer<CreateVolumeRequest> params,
                                  QSharedPointer<CreateVolumeResponse> response)
{
    updateProgress(1, params->id);
    if (params->emitResponse)
