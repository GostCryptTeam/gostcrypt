...
void FuseService::mountRequestHandler(QVariant r)
{
    ...
    QSharedPointer<Core::MountVolumeRequest> params =
        r.value<QSharedPointer<Core::MountVolumeRequest>>();
        ...
        if (params->isDevice)
        {
            if (mountedVolume->GetFile()->GetDeviceSectorSize() != mountedVolume->GetSectorSize())
            {
                throw InvalidParameterException("sectorSize",
                                                "Device sector size does not correspond to sectorSize indicated in volume header");
            }
            if (mountedVolume->GetSectorSize() != 512)
            {
                throw InvalidParameterException("sectorSize",
                                                "GostCrypt only support sectorSize of 512 bytes for volume contained in device");
            }
        }
...
static int fuse_service_read(const char* path, char* buf, size_t size, off_t offset,
...
static int fuse_service_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset,
                                struct fuse_file_info* fi)
{
    (void)fi;
    (void)offset;
...
static int fuse_service_write(const char* path, const char* buf, size_t size, off_t offset,
                              struct fuse_file_info* fi)
{
    (void)fi;
    try
