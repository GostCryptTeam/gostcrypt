...
typedef struct CRYPTO_INFO_t
{
    ...
    unsigned __int8
    ks[MAX_EXPANDED_KEY];   /* Primary key schedule (if it is a cascade, it conatins multiple concatenated keys) */
    unsigned __int8
    ks2[MAX_EXPANDED_KEY];  /* Secondary key schedule (if cascade, multiple concatenated) for XTS mode. */
    ...
    unsigned __int8
    master_keydata[MASTER_KEYDATA_SIZE];    /* This holds the volume header area containing concatenated master key(s) and secondary key(s) (XTS mode). */
    unsigned __int8
    k2[MASTER_KEYDATA_SIZE];                /* For XTS, this contains the secondary key (if cascade, multiple concatenated). */
...
void EncryptDataUnits(unsigned __int8* buf, const quint64* structUnitNo, quint32 nbrUnits,
                      PCRYPTO_INFO ci);
void EncryptDataUnitsCurrentThread(unsigned __int8* buf, const quint64* structUnitNo,
                                   quint64 nbrUnits, PCRYPTO_INFO ci);
void DecryptDataUnits(unsigned __int8* buf, const quint64* structUnitNo, quint32 nbrUnits,
                      PCRYPTO_INFO ci);
void DecryptDataUnitsCurrentThread(unsigned __int8* buf, const quint64* structUnitNo,
                                   quint64 nbrUnits, PCRYPTO_INFO ci);
