...
namespace GostCrypt
{
	...
	void GraphicUserInterface::InitSecurityTokenLibrary () const
	{
		...
		try
		{
			SecurityToken::InitLibrary (Preferences.SecurityTokenModule, shared_ptr <GetPinFunctor> (new PinRequestHandler), shared_ptr <SendExceptionFunctor> (new WarningHandler));
		}
		catch (Exception &e)
		{
			ShowError (e);
	...
	{
		wxString url = wxString (StringConverter::ToWide (secure ? GST_APPLINK_SECURE : GST_APPLINK));
		...
		if(linkId == L"help")
			...
			url += L"contact.php";
		else if (linkId == L"history")
			url += L"gostcrypt.php";
		else if(linkId == L"tutorial")
#ifdef GST_LINUX
			url += L"wiki/doku.php?id=utilisation:linux";
#elif defined (GST_MACOSX)
			url += L"wiki/doku.php?id=utilisation:mac";
#endif
...
#	error GST_RESOURCE_DIR undefined
...
#endif
		{
			wildcards = LangString["ALL_FILES"] + L"|*";
	...
	FilePath GraphicUserInterface::SelectVolumeFile (wxWindow *parent, bool saveMode, const DirectoryPath &directory) const
	{
		...
		extensions.push_back (make_pair (wstring(L"gst"), wstring(LangString["GST_VOLUMES"])));
